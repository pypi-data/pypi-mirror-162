...
from pure_sql.types import QueryDatum
...
def get_query_by_name(query_name: str, filename: str) -> QueryDatum | None:
    queries = from_path(filename)
    q = [q for q in queries if q.query_name == query_name]
    if len(q) == 0:
        return
    return q[0]
