...
def regulator(f1, f2, norm_method, CHRM_SIZE, outdir, bed="",
              res=5000,
              sigma0=1.6,
              s=10,
              pt=0.1,
    ...
    print(f'length before normalization:{len(v1)}')
    ...
    if chromosome == chromosome2:
        ...
        pval_weights1 = normalize_sparse(x1, y1, v1, res, distance_in_px)
        pval_weights2 = normalize_sparse(x2, y2, v2, res, distance_in_px)
        print(f'length after normalization:{len(v1)}')
        ...
        with Manager() as manager:
            ...
            for i in range(len(start)):
                ...
                p = Process(target=process_block, args=(
                    i, start, end, overlap_size, cc2, chromosome,chromosome2, res, pval_weights2, distance_in_px, octave_values, o, st, pt))
                p.start()
                processes.append(p)
...
def main():
    ...
    for i, (chromosome,chromosome2) in enumerate(zip(chr_list,chr_list2)):
        ...
        print("{0} loops found for chrmosome={1}, fdr<{2} in {3}sec".format(len(o),chromosome,args.pt,"%.2f" % (time.time()-start_time)))
        with open(args.outdir+'1', 'a') as out_file1, open(args.outdir+'2', 'a') as out_file2:
