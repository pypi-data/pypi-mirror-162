import requests, random, platform, sys, os, json
...
class short:
   def cuttly(content:str):
      myurl, name=content.split('!')
      protocol="http://"
      ...
      r1 = requests.get('http://cutt.ly/api/api.php?key={}&short={}&name={}'.format(key, myurl, name))
      r = (r1.text)
      if r == '{"url":{"status":1}}':
        ...
        print(bcolors.OKGREEN + 'Developer - Misha Korzhik', bcolors.ENDC)
        print(bcolors.OKGREEN + 'Link succesfully created! url: '+domain+r, bcolors.ENDC)
   def dagd(myurl:str):
      domain = 'http://'
      link = domain + myurl
      r1 = requests.get('https://da.gd/s/?url='+link).text
      print(bcolors.OKGREEN + 'Developer - Misha Korzhik', bcolors.ENDC)
      print(bcolors.LITBU + 'Link succesfully created! url: '+r1, bcolors.ENDC)
   def isgd(myurl:str):
      domain = 'http://'
      link = domain + myurl
