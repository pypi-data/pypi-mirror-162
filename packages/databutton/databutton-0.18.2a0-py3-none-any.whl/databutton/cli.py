...
@click.group()
@click.option("--verbose", "-v", count=True, help="Enable verbose logging", default=3)
@click.pass_context
...
def download(project_id: str, project_dir: str):
    ...
    if project_dir.exists():
        if len(list(project_dir.iterdir())) > 0:
            click.secho("❌ The target directory isn't empty.")
            exit(1)
    click.echo("Downloading project")
    click.secho(f"id: {project_id}", fg="cyan")
    click.secho(f"directory: {str(project_dir)}", fg="cyan")
    download_url = f"https://p{project_id}.dbtn.app/dbtn/projects/download"
    ...
    if not response.ok:
        ...
        click.secho("If it is, the following url should work:")
    ...
    if project_dir.resolve() != Path.cwd().resolve():
        if project_dir.relative_to(Path.cwd()):
            click.secho(f"  cd {project_dir}", fg="cyan")
        else:
            click.secho(f"  cd {project_dir.resolve()}", fg="cyan")
    click.secho("  pip install -r requirements.txt", fg="cyan")
    click.secho("  databutton start", fg="cyan")
