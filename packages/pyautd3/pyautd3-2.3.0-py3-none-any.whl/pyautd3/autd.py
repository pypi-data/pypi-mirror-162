...
class SOEM:
    def __init__(self, ifname: str, dev_num: int):
        self.ifname = ifname
    ...
    def send_cycle(self, cycle: int):
        self.send_cycle = cycle
        return self

    def sync0_cycle(self, cycle: int):
        self.sync0_cycle = cycle
        return self
    ...
    def build(self):
        onlost = ErrorHandlerFunc(self.on_lost) if self.on_lost is not None else None
        LinkSOEM().init_dll()
        link = c_void_p()
        LinkSOEM().dll.AUTDLinkSOEM(byref(link), self.ifname.encode('mbcs'),
                                    self.dev_num, self.sync0_cycle, self.send_cycle, self.freerun, onlost, self.high_presicion)
        return Link(link)
...
class RemoteTwinCAT:
    def __init__(self, remote_ip_addr, remote_ams_net_id, local_ams_net_id):
        ...
        self.remote_ams_net_id = remote_ams_net_id
    ...
    def build(self):
        link = c_void_p()
        LinkRemoteTwinCAT().dll.AUTDLinkRemoteTwinCAT(byref(link), self.remote_ip_addr.encode('mbcs'),
                                                      self.remote_ams_net_id.encode('mbcs'),
                                                      self.local_ams_net_id.encode('mbcs'))
        return Link(link)
...
class Emulator:
    ...
    def build(self):
        link = c_void_p()
        LinkEmulator().dll.AUTDLinkEmulator(byref(link), self.port, self.autd.p_cnt)
        return Link(link)
