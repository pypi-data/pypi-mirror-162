...
def __assert(row, data):
    ...
    for info in assert_info:
        column = info['column']
        expect_value_type = info['expect_value_type']
        expect_value = info['expect_value']
        if 'json' == expect_value_type:
            is_same, detail = json_util.is_same_json(json.loads(getattr(row, column), encodings='utf-8'),
                                                     json.loads(expect_value, encodings='utf-8'))
            assert is_same
        else:
