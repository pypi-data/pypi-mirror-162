...
######### Internal
def extract_parameter_space_data(data_dir, config_file, parameter_start_key, parameter_end_key, parameter_increment_key):
    ...
    for sub_dir in sub_dirs:
        all_spaces.append(
            extract_parameters(sub_dir + '/' + config_file, parameter_start_key, parameter_end_key, parameter_increment_key))


    return merge_spaces(all_spaces)

def get_param_condition(parameter_key, j):
    sub_entry = j
    param_condition = {}
...
def generate_parameter_space_values(start_value, end_value, increment_value, end_point = True):
    try:
...
def extract_parameters(config_file, parameter_start_key, parameter_end_key, parameter_increment_key):
    
    param_space = {}
    import json
    with open(config_file) as f:
        j = json.load(f)
    param_starts = get_param_condition(parameter_start_key,j)
    param_ends = get_param_condition(parameter_end_key,j)
    param_increments = get_param_condition(parameter_increment_key,j)
    common_params = []
    ...
    for param in common_params:
        ...
        if type(param_starts[param]) == dict:
            for key, value in param_starts[param].items():
                space = generate_parameter_space_values(param_starts[param][key], param_ends[param][key], param_increments[param][key])
                if space is not None:
                    param_space[param + f'({key})'] = {"space": space}
        else:
            space = generate_parameter_space_values(param_starts[param], param_ends[param], param_increments[param])
            if space is not None:
                param_space[param] = {"space": space}
    return param_space

def merge_spaces(all_spaces):
    merged_space = {}
...
if __name__ == "__main__":
    # To test, run from one directory up: python -m tinc.utils 
