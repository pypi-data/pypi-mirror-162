    ...
    def add(self, obj: LTComponent) -> None:  # type: ignore[override]
        ...
        if length_objs > 0:  # 0, 1
            ...
            if length_objs == 1:
                ...
                space_count = now_char_distance / max(max_char_distance, agv_char_width)
                for _ in range(math.ceil(space_count)):  # add at least one LTAnno
                    LTContainer.add(self, LTAnno(" "))
    ...
    def add(self, obj: LTComponent) -> None:  # type: ignore[override]
        ...
        if length_objs > 0:  # 0, 1
            ...
            if length_objs == 1:
                ...
                space_count = now_char_distance / max(max_char_distance, agv_char_height)
                for _ in range(math.ceil(space_count)):  # add at least one LTAnno
                    LTContainer.add(self, LTAnno(" "))
            self._sum_char_distance += now_char_distance
