...
class Namespace(Enum):
    SANTA_CRUZ_ELECTION_2022 = "santa-cruz-election-2022"
...
def load_dataset(
    race=Measure.D,
    namespace=Namespace.SANTA_CRUZ_ELECTION_2022,
):
    if namespace != Namespace.SANTA_CRUZ_ELECTION_2022:
        raise ValueError(
            f"Invalid namespace. Only {Namespace.SANTA_CRUZ_ELECTION_2022} is supported."
        )

    if race.lower() == Measure.D:
        url = f"{BASE_URL}/{ELECTION_DATA}/{MEASURE_D}"
    elif race.lower() == Measure.C:
