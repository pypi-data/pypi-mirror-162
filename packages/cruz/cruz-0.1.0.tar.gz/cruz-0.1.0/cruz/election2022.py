...
def load_dataset(name="D"):
    if name.upper() == "D":
        ...
        raise ValueError("Invalid dataset name. Pick either D or C.")
...
def craft_visual_index(df):
    df["Visual Index"] = df.index.map(rework_spot).astype(int)
    df2 = df.sort_values(by="Visual Index")
    df2["Visual Index 2"] = np.linspace(0, 1, num=len(df.index))
    df2[df2["Ballots Cast"] > 0]
