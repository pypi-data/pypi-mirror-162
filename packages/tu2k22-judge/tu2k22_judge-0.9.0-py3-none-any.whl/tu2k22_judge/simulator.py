...
class Simulator:
    ...
    def run(self, bot_urls: List[str], bot_data: List[Any], player_moves: List[PlayerMove]):
        ...
        while len(player_moves) < MAX_MOVES_ALLOWED:
            ...
            response = requests.post(bot_urls[player], json=bot_input, timeout=self.move_timeout)
            ...
            try:
                bot_response = BotResponse.validate(json.loads(response.content))
                ...
                if self.verbose:
                    ...
                    print(color + Style.BRIGHT + str(bot_response.move))
                    print(Fore.RESET + Style.RESET_ALL)
                ...
                raise InvalidMovesException(
                    reason=f"Incorrect schema. Expected schema: {BotResponse.schema()}", player=player)
