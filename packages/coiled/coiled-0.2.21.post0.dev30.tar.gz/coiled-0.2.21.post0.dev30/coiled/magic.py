...
async def create_wheel(pkg_name: str, src: str) -> Path:
    outdir = Path(f"./.data/test/{pkg_name}")
    p = await asyncio.create_subprocess_shell(
        cmd=f"pip wheel --wheel-dir {outdir} --no-deps --no-binary :all: {src}"
    )
    await p.wait()
    return next(file for file in outdir.iterdir() if file.suffix == ".whl")
...
async def handle_dist(
    ...
    if egg_links:
        wheel_file = await create_wheel(
            pkg_name=dist.name, src=dist._path.parent  # type:ignore
        )
        return {
            "name": name,
            "client_version": dist.version,
            ...
            "specifier": "",
            "include": True,
            "issue": "Editable install",
            "sdist": wheel_file.open(mode="rb"),
        }
    if installer:
            ...
            if direct_url_metadata:
                ...
                if url_metadata.get("vcs_info"):
                    ...
                    if vcs == "git":
                        ...
                        wheel_file = await create_wheel(pkg_name=dist.name, src=pip_url)
                        return {
                            "name": name,
                            "client_version": dist.version,
                            "direct_url": None,
...
async def create_environment_approximation(
    ...
    path_deps = {
        ...
        if package["sdist"] or package["direct_url"]
    }
    conda_env = {
