...
from craft_text_detector import craft_utils as craft_utils
from craft_text_detector import image_utils as image_utils
from craft_text_detector import torch_utils as torch_utils
...
def get_prediction(
    ...
    if refine_net is not None:
        ...
        score_link_line = y_refiner[0, :, :, 0].cpu().data.numpy()
    ...
    boxes_line, polys_line = craft_utils.getDetBoxes(
    score_text, score_link_line, text_threshold, link_threshold, low_text, poly
    )

    # coordinate adjustment
    boxes = craft_utils.adjustResultCoordinates(boxes, ratio_w, ratio_h)
    polys = craft_utils.adjustResultCoordinates(polys, ratio_w, ratio_h)
    boxes_line = craft_utils.adjustResultCoordinates(boxes_line, ratio_w, ratio_h)
    polys_line = craft_utils.adjustResultCoordinates(polys_line, ratio_w, ratio_h)
    for k in range(len(polys)):
        if polys[k] is None:
            polys[k] = boxes[k]
    for k in range(len(polys_line)):
        if polys_line[k] is None:
            polys_line[k] = boxes_line[k]
    ...
    return {
        "image":image,
        "boxes": boxes,
        "boxes_line":boxes_line,
        "boxes_as_ratios": boxes_as_ratio,
        "polys": polys,
        "polys_line": polys_line,
