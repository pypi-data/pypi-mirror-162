...
class MCSchematic:
    ...
    def save(self, outputFolderPath: str, schemName: str, version: 'Version'):
        ...
        cleanBlockPalette = \
            {
                blockNbt: Int(blockPaletteId)
                for blockNbt, blockPaletteId in self._blockPalette.items()
                if type(blockNbt) == str
            }
    ...
    '''
    def setBlock(self, position: tuple[int, int, int], blockNbt: str):
        # We have a different treatment between block entities
        # and normal blocks.
        if not self._isBlockNBTBlockEntity(blockNbt):
            self._setBlockState(position, blockNbt)
        else:
            self._setBlockEntity(position, blockNbt)

    '''
    Returns the blockNbt at the inputed position, None if there is 
    '''
    ...
    '''
    Returns the blockNbt associated to the inputed ID in the palette.
    Returns None if no blockNbt is attached to the inputed blockPaletteId
    ...
    '''
    def _isBlockNBTBlockEntity(self, blockNbt: str) -> bool:
        return '{' in blockNbt
    ...
    def _setBlockEntity(self, position: tuple[int, int, int], blockEntity: str):
        ...
        self._blockEntities[position] = blockEntity
