...
class Parser:
    ...
    def p_term_topic_exists(self, p):
        'term : TOPIC EXISTS'
        topic = p[1]
        p[0] = self.Expr(set([topic]), lambda s: topic in s)
    ...
    def p_term_topic_equal_literal(self, p):
        ...
        lhs, rhs = p[1], p[3]
        conv = float if type(rhs) is float else str
        p[0] = self.Expr(set([lhs]), lambda s: conv(s[lhs]) == rhs)
