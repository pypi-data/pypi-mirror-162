...
def To_Generate_Key_Word(number):
    ...
    if wd == 0:
        ...
        w.to_file(f'The Short Article Trigram_figure_WorldCloud {xml_papersw[80][:-5]}.png')
    ...
    if wd == 1:
        ...
        w = wordcloud.WordCloud(background_color="white")  # 把词云当做一个对象
        w.generate_from_frequencies(dic)
        w.to_file(f'The Regular Article Trigram_figure_WorldCloud {xml_papersw[80][:-5]}.png')
    #
    # i=0
    ...
    if wd == 0:
        ...
        b=list(reversed(b))
        plt.barh(a, b)
        plt.title(f'The Short Article Top 10 trigrams "weighted" for {xml_papersw[80][:-5]}')
        plt.xticks(rotation=45, fontsize=11)

        # Saving the figures in result path:
        plt.savefig(os.path.join(f'The Short Article Trigram_figure_{xml_papersw[80][:-5]}'),
                    bbox_inches="tight")
        plt.close()
    ...
    if wd == 1:
        ...
        b=list(reversed(b))
        plt.barh(a, b)
        plt.title(f'The Regular Article Top 10 trigrams "weighted" for {xml_papersw[80][:-5]}')
        plt.xticks(rotation=45, fontsize=11)

        # Saving the figures in result path:
        plt.savefig(os.path.join(f'The Regular Article Trigram_figure_{xml_papersw[80][:-5]}'),
                    bbox_inches="tight")
        plt.close()
...
# To_Generate_Disease(7824075)
# To_Generate_Location(7824075)
