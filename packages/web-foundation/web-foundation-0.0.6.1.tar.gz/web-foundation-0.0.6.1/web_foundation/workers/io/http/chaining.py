...
ProtectIdentity = TypeVar("ProtectIdentity")


@dataclass
class InputContext(Generic[DtoType, ProtectIdentity]):
    request: Request
    dto: DtoType | None
...
Protector = Callable[[Request], Coroutine[Any, Any, ProtectIdentity | None]]
...
def chain(protector: Protector = protect,
    ...
    def called_method(target: HandlerType):
        ...
        async def f(*args, **kwargs):
            req: Request = args[0]
            prot_identity = await protector(req)
            if in_struct:
