...
class DictRouter(ExtRouter):
    ...
    def apply_routes(self, app: Sanic, **kwargs):
        ...
        for app_route in self._router_conf.get("apps"):
            ...
            for endpoint, params in app_route.get("endpoints").items():
                methods_confs = []
                endpoint_version = params.pop("version", None)
                if not endpoint_version:
                    raise AttributeError("Need to specify endpoint version")
                ...
                for method_name, method_params in params.items():
                    ...
                    app.add_route(uri=endpoint, methods={method_name.upper()}, handler=chain,
                                  version_prefix=version_prefix, version=endpoint_version)
