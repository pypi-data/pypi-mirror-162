...
export class MapModel extends widgets.DOMWidgetModel {
  defaults() {
    return {
      ...
      _model_module: '@here/map-widget-ext-for-jupyter',
      _view_module: '@here/map-widget-ext-for-jupyter',
...
export class MapView extends widgets.DOMWidgetView {
  ...
  mapjs_events() {
    this.obj.addEventListener('mapviewchangeend', (evnt) => {
      console.log('HERE EVENT CHANGE CENTER ');
    ...
    this.obj.addEventListener('tap', function (evnt) {
      var coord = evnt.target.screenToGeo(evnt.currentPointer.viewportX,
                                          evnt.currentPointer.viewportY);
      this.model.set('tap_pos', [coord.lat, coord.lng]);
      this.model.save_changes();
    });
