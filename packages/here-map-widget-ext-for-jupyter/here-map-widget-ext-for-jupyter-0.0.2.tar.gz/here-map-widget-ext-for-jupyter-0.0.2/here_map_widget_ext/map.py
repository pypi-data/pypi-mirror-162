...
def basemap_to_tiles(basemap, opacity=1, **kwargs):
    ...
    basemap : class:`xyzservices.lib.TileProvider`
        Basemap description coming from here_map_widget_ext.basemaps.
...
class Element(Widget):
    ...
    _view_module = Unicode("@here/map-widget-ext-for-jupyter").tag(sync=True)
    _model_module = Unicode("@here/map-widget-ext-for-jupyter").tag(sync=True)
...
class Map(DOMWidget, InteractMixin):
    ...
    tap_pos = List([]).tag(sync=True, o=True)
