...
from torcheval.metrics import Metric, BinaryPrecisionRecallCurve
...
### INPUT GENERATION
def generate_update_args_binary(num_batches:int, batch_size:int, device: torch.device) -> Dict[str, Any]:
...
def generate_update_args_multiclass(num_batches:int, batch_size:int, device: torch.device, num_classes: int) -> Dict[str, Any]:
    input = generate_random_input(InputType.MULTICLASS_PROB, device, num_batches, batch_size, num_classes=num_classes)
...
def main(num_batches, batch_size, compute_interval, num_classes):
    ...
    metric = BinaryPrecisionRecallCurve().to(device)
