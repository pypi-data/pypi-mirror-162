...
from torcheval.metrics import Metric, MulticlassAccuracy, MultilabelAccuracy
...
def generate_update_args_multilabel(num_batches:int, batch_size:int, device: torch.device, num_classes: int) -> Dict[str, Any]:
    input = generate_random_input(InputType.MULTILABEL, device, num_batches, batch_size, num_classes=num_classes)
    target = generate_random_input(InputType.MULTILABEL, device, num_batches, batch_size, num_classes=num_classes)
    return {"input": input, "target": target}
...
def main(num_batches, batch_size, compute_interval, num_classes):
    ...
    metric = MultilabelAccuracy().to(device)
    update_kwargs = generate_update_args_multilabel(num_batches, batch_size, device, num_classes)
    get_benchmark_stats(metric, num_batches, compute_interval, update_kwargs)
