...
def plot_fitting_result(MODEL, datafile, data, par):
    ...
    * If MODEL.PPAR.ax == None:
      a single output graph is shown (plt.show) and saved (to output_file)
    * If MODEL.PPAR.ax == axes_object
    ...
    if MODEL.PPAR.ax == None:
        ...
        plt.tight_layout()
        # (c) Save the plot
        # (In Spyder, the plot is shown during saving in Plot window
        # (In CLI, the plot is saved, not shown => not to be closed manually
