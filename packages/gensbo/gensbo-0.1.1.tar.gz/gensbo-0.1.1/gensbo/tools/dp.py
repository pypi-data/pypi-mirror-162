...
def write_to_file(algo_name, result, filename, var_name = []):
    ...
    '''
    if (algo_name == 'sopso'):
        np.save(filename + '_so.npy',[result,['f','x','violate_cons','f_history',
                                              'feasible_f','feasible_x']])
        f_value,x_value,vio_cons_index,f_history,fea_f,fea_x = result
        # 创建便于直观查看结果的excel文档
        wb = xlwt.Workbook()
        ...
        ws.write(0, 3, 'violate_cons')
        if (len(vio_cons_index) == 0):
            ws.write(1, 3, 'None')
        else:
            for i in range(len(vio_cons_index)):
                ws.write(1+i, 3, vio_cons_index[i])

        ws.write(0, 4, 'f_history')
        for i in range(len(f_history)):
            ws.write(1+i, 4, f_history[i])

        if len(fea_f) > 0:
            ws.write(0, 5, 'feasible_f')
            ws.write(0, 6, 'feasible_x')
            for i in range(len(fea_f)):
                ws.write(1+i, 5, fea_f[i])
                ws.write(1 + i, 6, str(fea_x[i]))
