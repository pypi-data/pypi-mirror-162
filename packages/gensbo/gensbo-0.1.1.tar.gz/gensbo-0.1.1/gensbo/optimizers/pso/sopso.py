...
class Algorithm:
    ...
    def __init__(self, problem, simulator, swarm_size=30, w=0.8, c1=2, c2=2,
                 ...
                 print_info=True,time_max=24*3600):
    ...
    def run(self, pso_model='standard_pso', neighborhood='star',
        ...
        if self.if_ini_cons == True:
            ...
            if (self._constraint_judge == True):
                ...
                while num_feasible_x < self.ini_feasible_x_num:
                    ...
                    dt_max = time_max_1 - time_max_0
                    if dt_max > self.time_max:
                        if self.print_info == True:
                            print('达到最大设定运行时间(初始化内点):先寻优试一试','\n')
                        break
                ...
                if len(self.feasible_x) >= self.feasible_x_num:
                    ...
                    if self.if_get_feasible_x_only == True:
                        ...
                        return self.f_value, self.x_gbest_step,\
                               _constraint_violate_index, self.f_history,\
        ...
        for step in range(1,self.step_max):
            ...
            if self.print_info == True:
                self.progress_bar(step,self.step_max,dt)

            time_max_2 = time.time()
            dt_max = time_max_2 - time_max_0
            if dt_max > self.time_max:
                if self.print_info == True:
                    print('达到最大设定运行时间,优化结束', '\n')
                break
    ...
    ## 进度条输出
    def progress_bar(self, num, total, dt):
        rate = float(num + 1) / total
        ratenum = int(20 * rate)
        if dt >= 3600:
            ti = dt / 3600  # h
            tall = ti * total
            r = '\r{:.2f} h/{:.2f} h [{}{}]{}%'.format(ti, tall, '*' * ratenum, ' ' * (20 - ratenum), ratenum * 5)
        elif 3600 > dt >= 60:
            ti = dt / 60
            tall = ti * total
            r = '\r{:.2f} min/{:.2f} min [{}{}]{}%'.format(ti, tall, '*' * ratenum, ' ' * (20 - ratenum), ratenum * 5)
        else:
            ti = dt
            tall = ti * total
            r = '\r{:.2f} s/{:.2f} s [{}{}]{}%'.format(ti, tall, '*' * ratenum, ' ' * (20 - ratenum), ratenum * 5)
