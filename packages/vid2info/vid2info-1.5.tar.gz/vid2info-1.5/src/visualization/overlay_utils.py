...
from src.visualization.config import DEFAULT_OVERLAY_CORNER, OVERLAY_CORNER_PAD, GENERAL_OVERLAY_FONT_SIZE, \
    ...
    MIN_BBOX_ALPHA
from src.visualization.general_utils import time_as_str, apply_pad_on_shortest_axis
...
def overlay_general_annotations(image: np.ndarray, scene_state: SceneState, is_bgr: bool = True,
    ...
    assert type(scene_state) is SceneState, f"scene_state must be of type SceneState, not {type(scene_state)}"
...
def draw_element_bbox(image: np.ndarray, element_state: ElementState, track_id: int,
    ...
    assert image.ndim == 3, f"image must be of dimension 3, not {image.ndim}"
    assert image.shape[2] == 3, f"image must have 3 channels, not {image.shape[2]}"
    assert type(element_state) is ElementState, f"element_state must be of type ElementState, not {type(element_state)}"
