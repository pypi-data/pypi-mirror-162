...
from src.inference.segmentation.config import WEIGTHS_PATH, PARAMS_YAML, \
    MODEL_NAME, PIDNET_SCALE_FACTOR
from src.inference.config import INFERENCE_DEVICE, CLASS_NAMES_LIST, SEGMENTATION_MASK, BACKGROUND_CLASS_IDX
from src.inference.utils import image_to_float_tensor, get_l_r_pad
...
class PidNetSegmenter:
    def __init__(self, weights : str = WEIGTHS_PATH, params_yml : str = PARAMS_YAML,
        ...
        self.load_params(params_yml=params_yml)
    ...
    def load_params(self, params_yml : str = PARAMS_YAML):
        """
        Load the parameters from the given yml file. That params are mean, std and class_names.

        :param params_yml: str. Path to the yml file.
        """
        assert os.path.isfile(params_yml), f'Dataset yml file {params_yml} not found.'
