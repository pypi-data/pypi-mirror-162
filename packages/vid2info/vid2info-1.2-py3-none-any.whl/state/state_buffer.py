...
from src.state.config import BUFFER_SIZE
...
class StateBuffer:
    def __init__(self, buffer_size : int = BUFFER_SIZE, running_online = False):
        ...
        Args:
            buffer_size: int. The size of the history buffer to keep.
            running_online: bool. If True, it will assume that the video is comming from an online stream
    ...
    def state_from_tracker_out(self, tracker_out : dict, add_to_buffer : bool = True,
        ...
        :return: SceneState. The new state. If add_to_buffer was True it can be also found in the last element of the
                current self.buffer.
        """
        state = SceneState(tracker_out=tracker_out,
