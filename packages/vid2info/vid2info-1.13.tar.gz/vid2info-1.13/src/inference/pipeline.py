...
from src.inference.detection.yolo_detector import YoloDetector
from src.inference.tracking.tracker import Tracker
from src.inference.segmentation.pidnet_segmenter import PidNetSegmenter

from src.state.state_buffer import StateBuffer
from src.state.scene_state import SceneState

from src.inference.detection.config import WEIGTHS_PATH, DETECTOR_MIN_CONFIDENCE_TH, IOU_NMS_TH, \
    MAX_DETECTIONS_PER_IMAGE, HALF_MODE, DATASET_YML, NORMALIZE_OUTPUT
from src.inference.segmentation.config import PARAMS_YAML as SEGMENTER_PARAMS_YML,\
    MODEL_NAME as SEGMENTER_MODEL_NAME, WEIGTHS_PATH as SEGMENTER_WEIGTHS_PATH
from src.inference.tracking.config import TRACKING_TH, TRACKING_MATCH_TH, TRACKING_MAX_BUFFER
from src.inference.config import INFERENCE_DEVICE

from src.video.config import DEFAULT_VIDEO_HW, DEFAULT_VIDEO_FRAME_RATE
...
class Pipeline:
    ...
    def process_frame(self, frame: np.ndarray, use_only_high_confidence_detection : bool = True,
                      is_bgr : bool = False) -> SceneState:
        """
