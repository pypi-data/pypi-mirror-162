...
# %%
def BrewResponsibleAI(model,x_test,sensitive_data,y_test):
  import numpy as np 
  overall_acc,grouped_acc=AML.BrewFairnessEvaluator(model,x_test,sensitive_data_col,y_test)
  ...
  if (perc_diff_minmax_acc > 50):
    ...
    Mitigated_model,Mitigated_Pred=AML.BrewDisparityMitigation(x_test,sensitive_data_col,y_test)
...
def MagicBrewer(input_dataframe,label_col,sensitive_data_col):
  ...
  print("\n","******************** CLEANSING STARTING ********************")
  cleansed_df=AML.BrewDataCleanser(input_dataframe)


  print("\n","******************** SAMPLING STARTING ********************")
  subsample_final,subsample_StratifiedSampling,subsample_RandomSampling,subsample_SystematicSampling,subsample_ClusterSampling_Oversampling=AML.BrewDataSampling(cleansed_df,label_col)


  print("\n","******************** TRAIN-TEST SPLIT STARTING ********************")
  x_train, x_test, y_train, y_test = AML.BrewTrainTestSplit(subsample_final,label_col,0.2)
  print("Train-Test Split completed in the ratio of Train:Test :: 80:20")

  print("\n","******************** MODELLING STARTING ********************")
  model,Final_df_actual_pred=AML.BrewAutoML_Classifier (x_train,y_train,x_test,y_test)


  print("\n","******************** RESPONSIBLE-AI STARTING ********************")
  FinalModel,FinalPredictedData=BrewResponsibleAI(model,x_test,sensitive_data_col,y_test)
