...
def AnomalyDetection(df,target_variable,variables_to_analyze,outliers_fraction):
...
def FairnessEvaluator(model,x_test,sensitivefeatures,y_test):
    ...
    y_test2=y_test.astype(np.int32).tolist()
    #y_true=pd.DataFrame(y_test,columns=['survived'])
    #y_true.reset_index(drop=True,inplace=True)
    #y_true['survived']=y_true['survived'].astype(int)
    #y_true2=y_true
    print(x_test)
    gm = MetricFrame(metrics=accuracy_score, y_true=y_test2, y_pred=y_pred,sensitive_features=sensitivefeatures)
    #gm = mean_prediction(y_true=y_test2, y_pred=y_pred)    
...
# df=x_test['sex']
# y_pred = FairnessEvaluator(model,x_test,df,y_test)
