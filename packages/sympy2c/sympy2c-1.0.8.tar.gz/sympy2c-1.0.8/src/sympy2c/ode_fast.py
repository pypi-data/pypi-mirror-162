...
def _write_json(path, data):
    with portalocker.Lock(
        path, "w", timeout=1, encoding="utf-8", flags=portalocker.LOCK_EX
    ) as fh:
...
def update_new_traces(unique_id, new_traces):
    ...
    with portalocker.Lock(
        path + ".lock", "w", timeout=1, encoding="utf-8", flags=portalocker.LOCK_EX
    ) as fh:
        ...
        with open(path, "w") as fh:
            json.dump(traces, fh, indent=4)
    # print(f"process {os.getpid()}: {open(path).read()}")
