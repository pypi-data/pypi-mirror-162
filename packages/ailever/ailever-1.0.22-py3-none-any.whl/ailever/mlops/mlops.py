...
class MLTrigger(PredictResult):
    ...
    def get_dataset(self, feature_store_path):
        dataset = pd.read_csv(feature_store_path)
        dataset_name = os.path.split(feature_store_path)[-1]
        dataset.columns.name = dataset_name
        return dataset
...
class MLOps(MLTrigger):
    ...
    def model_choice(self, idx:int=-1):
        ...
        if hasattr(self, '_user_model_names'):
            ...
            logger['mlops'].info(f'The {self._model_name} model is selected from model registry!')
        return self
    ...
    def drawup_dataset(self, name:str):
        ...
        logger['mlops'].info(f'The {name} model is selected from feature store!')
    ...
    def codecommit(self, entry_point, upload=True):
        ...
        reserving_path = os.path.join(self.core['SR'].path, self.entry_point.mlops_entry_point)
        copyfile(entry_point, reserving_path)
        logger['mlops'].info(f'The {self.entry_point.mlops_entry_point} code is reserved into source repository!')
