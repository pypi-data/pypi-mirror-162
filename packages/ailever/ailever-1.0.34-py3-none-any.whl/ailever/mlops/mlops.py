...
class MLOps(MLTrigger):
    ...
    def codecommit(self, entry_points, upload=True):
        ...
        if isinstance(entry_points, str):
            ...
            if len(list(filter(lambda x: isinstance(x, str), entry_points))) == len(entry_points):
                if len(entry_points) == 1:
                    # >>> entry_points = ['*.py', ]
                    entry_point, d_comment, t_comment = (entry_points[0], None, None)
        ...
        for entry_point_set in entry_point_sets:
            ...
            # define user dataset
            dataset = entry_point_obj.preprocessing()                                   # mlops.entry_point.preprocessing

            # define user model
            model = entry_point_obj.architecture()                                      # mlops.entry_point.architecture
            
            self._user_datasets.append((dataset, d_comment))
            self._user_models.append((model, t_comment))
    
        self.__dataset = self.preprocessing(entry_point=self.entry_point_obj.mlops_entry_point)
        self.__model = self.learning(entry_point=self.entry_point_obj.mlops_entry_point)             # mlops.entry_point.train
