...
class CableTemplateForm(BootstrapMixin, forms.ModelForm):
    class Meta:
        model = CableTemplate
        fields = (
            'cable_number', 'tenant', 'type', 'plug', 'label', 'color', 'length', 'length_unit'
        )
...
class CableTemplateCSVForm(CSVModelForm):
    ...
    tenant = CSVModelChoiceField(queryset=Tenant.objects.all(), required=False, to_field_name="name", help_text="Assigned tenant")
