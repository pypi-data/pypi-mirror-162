...
from typing import Any, Callable, Iterable, List, Mapping, Tuple, Type, TypeVar, Union
...
def pformat(obj: Any, specifier: str = "", *, depth: int = 0, indent: int = 4, shorten: bool = True) -> str:
    ...
    if hasattr(cls, "__pformat__"):
        ...
        if len(obj) == 0:
            return f"{cls.__name__}()"
        return f"{cls.__name__}([{pformat_iterable(obj.items(), **with_indent)}])"
        ...
        if obj.maxlen is None:
            return f"{cls.__name__}({pformat_iterable(obj, **with_indent)})"
    ...
    s = pformat_iterable(obj, specifier, depth, indent, shorten)
...
def pformat_iterable(obj: Iterable[Any], specifier: str, depth: int, indent: int, shorten: bool) -> str:
