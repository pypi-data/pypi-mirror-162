...
class Interleaving:
	...
	def get_mode(self) -> enums.InterleaveMode:
		...
		Snippet: value: enums.InterleaveMode = driver.scenario.localized.interleaving.get_mode() \n
		...
		"""
		response = self._core.io.query_str('SCENario:LOCalized:INTerleaving:MODE?')
		return Conversions.str_to_scalar_enum(response, enums.InterleaveMode)

	def set_mode(self, mode: enums.InterleaveMode) -> None:
		"""SCPI: SCENario:LOCalized:INTerleaving:MODE \n
		Snippet: driver.scenario.localized.interleaving.set_mode(mode = enums.InterleaveMode.DROP) \n
		Select the mode for interleaving. \n
		...
		self._core.io.write(f'SCENario:LOCalized:INTerleaving:MODE {param}')
