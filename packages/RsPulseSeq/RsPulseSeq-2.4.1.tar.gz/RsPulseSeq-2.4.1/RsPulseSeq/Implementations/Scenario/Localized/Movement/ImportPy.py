...
class ImportPy:
	...
	def set(self) -> None:
		...
		Snippet: driver.scenario.localized.movement.importPy.set() \n
		Imports the selected waypoint and vehicle description files into the repository and applies them. \n
		"""
		self._core.io.write(f'SCENario:LOCalized:MOVement:IMPort')

	def set_with_opc(self, opc_timeout_ms: int = -1) -> None:
		"""SCPI: SCENario:LOCalized:MOVement:IMPort \n
		Snippet: driver.scenario.localized.movement.importPy.set_with_opc() \n
		Imports the selected waypoint and vehicle description files into the repository and applies them. \n
		...
		self._core.io.write_with_opc(f'SCENario:LOCalized:MOVement:IMPort', opc_timeout_ms)
