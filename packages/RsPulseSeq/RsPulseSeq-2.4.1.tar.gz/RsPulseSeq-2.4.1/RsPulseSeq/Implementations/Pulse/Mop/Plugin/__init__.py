...
class Plugin:
	...
	def get_name(self) -> str:
		...
		Snippet: value: str = driver.pulse.mop.plugin.get_name() \n
		...
		"""
		response = self._core.io.query_str('PULSe:MOP:PLUGin:NAME?')
		return trim_str_response(response)

	def set_name(self, name: str) -> None:
		"""SCPI: PULSe:MOP:PLUGin:NAME \n
		Snippet: driver.pulse.mop.plugin.set_name(name = '1') \n
		Renames the selected repository element. \n
			:param name: string Must be unique for the particular type of repository elements. May contain empty spaces.
		"""
		param = Conversions.value_to_quoted_str(name)
		self._core.io.write(f'PULSe:MOP:PLUGin:NAME {param}')
