...
class Path:
	...
	def get_list_py(self) -> List[int]:
		...
		Snippet: value: List[int] = driver.cpanel.used.path.get_list_py() \n
		Queries the available RF paths of the selected signal generators. \n
			:return: list_py: 'Path#1','Paht#2',...
		"""
		response = self._core.io.query_bin_or_ascii_int_list('CPANel:USED:PATH:LIST?')
		return response

	def get_select(self) -> str:
		"""SCPI: CPANel:USED:PATH:SELect \n
		Snippet: value: str = driver.cpanel.used.path.get_select() \n
		...
		"""
		response = self._core.io.query_str('CPANel:USED:PATH:SELect?')
		return trim_str_response(response)

	def set_select(self, select: str) -> None:
		"""SCPI: CPANel:USED:PATH:SELect \n
		Snippet: driver.cpanel.used.path.set_select(select = '1') \n
		Selects the element to which the subsequent commands apply. \n
		...
		self._core.io.write(f'CPANel:USED:PATH:SELect {param}')
