from ....Internal.Core import Core
from ....Internal.CommandsGroup import CommandsGroup


# noinspection PyPep8Naming,PyAttributeOutsideInit,SpellCheckingInspection
class Reject:
	"""Reject commands group definition. 1 total commands, 0 Subgroups, 1 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("reject", core, parent)

	def set(self) -> None:
		"""SCPI: PROGram:SETTings:REJect \n
		Snippet: driver.program.settings.reject.set() \n
		Reject changes to program settings. \n
		"""
		self._core.io.write(f'PROGram:SETTings:REJect')

	def set_with_opc(self, opc_timeout_ms: int = -1) -> None:
		"""SCPI: PROGram:SETTings:REJect \n
		Snippet: driver.program.settings.reject.set_with_opc() \n
		Reject changes to program settings. \n
		Same as set, but waits for the operation to complete before continuing further. Use the RsPulseSeq.utilities.opc_timeout_set() to set the timeout value. \n
			:param opc_timeout_ms: Maximum time to wait in milliseconds, valid only for this call."""
		self._core.io.write_with_opc(f'PROGram:SETTings:REJect', opc_timeout_ms)
