...
class Time:
	...
	def get_post(self) -> float:
		...
		"""
		response = self._core.io.query_str('SCENario:CEMit:MARKer:TIME:POST?')
	...
	def set_post(self, post: float) -> None:
		...
		self._core.io.write(f'SCENario:CEMit:MARKer:TIME:POST {param}')
	...
	def get_pre(self) -> float:
		...
		"""
		response = self._core.io.query_str('SCENario:CEMit:MARKer:TIME:PRE?')
		return Conversions.str_to_float(response)
	...
	def set_pre(self, pre: float) -> None:
		...
		self._core.io.write(f'SCENario:CEMit:MARKer:TIME:PRE {param}')
