...
class Select:
	...
	def get_id(self) -> float:
		...
		Selects a plugin variable ID. \n
			:return: idn: float
		"""
		response = self._core.io.query_str('DESTination:PLUGin:VARiable:SELect:ID?')
	...
	def set_id(self, idn: float) -> None:
		...
		self._core.io.write(f'DESTination:PLUGin:VARiable:SELect:ID {param}')
	...
	def get_value(self) -> str:
		...
		"""
		response = self._core.io.query_str('DESTination:PLUGin:VARiable:SELect?')
		return trim_str_response(response)

	def set_value(self, select: str) -> None:
		"""SCPI: DESTination:PLUGin:VARiable:SELect \n
		Snippet: driver.destination.plugin.variable.select.set_value(select = '1') \n
		Selects a plugin variable. \n
			:param select: string
		"""
		param = Conversions.value_to_quoted_str(select)
		self._core.io.write(f'DESTination:PLUGin:VARiable:SELect {param}')
