...
def processCustomTransactionOnChain(block,name,parameters:dict,pool:dict):
    ...
    parameters: key value pair of rpc parameters. 
    pool: pool parameters - reserveA, reserveB, totalLiquidity, poolSymbol (e.g. TSLA-DUSD)

    RETURN:
    addpoolliquidity: amount liquidity tokens
    removepoolliquidity: amount of TokenA and TokenB (tuple)
    poolswap: amount of new reserveA, new reserveB and amount TokenB (tuple)
    '''
    ...
    if name == 'addpoolliquidity':
        ...
        return chain.addPoolLiquidity(parameters['amountA'],parameters['amountB'],pool['reserveA'],pool['reserveB'],pool['totalLiquidity'])
        ...
        return chain.removePoolLiquidity(parameters['liquidity'],pool['reserveA'],pool['reserveB'],pool['totalLiquidity'])
    elif name == 'poolswap':
        ...
        return chain.poolSwap(block,parameters['tokenFrom'],pool['symbol'],parameters['fromAmount'],pool['reserveA'],pool['reserveB'],parameters['commission'],parameters['dexFeeInPct'])
...
#     hex ='6a2c44665478706a0f400d0300000000001976a9148f83f4f005ba70f0c5d7a59d4696daee13a6e43b88ac010000'
