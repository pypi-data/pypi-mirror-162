...
def play(update: Update, context: CallbackContext):
    ...
    game_name = unidecode.unidecode(context.args[0]) if context.args else None
...
def word(update: Update, context):
    ...
    if opponent.current_word:
        ...
        if opponent.current_word == query:
            ...
            broadcast(game, f"It took *{game.rounds_count + 1} round{'s' if game.rounds_count else ''}* for {escape(players)} to settle\.\n{escape(summary)}\nSend /play to {escape(bot_username)} to play Intersection\.", parse_mode=ParseMode.MARKDOWN_V2)
            game.terminate()
        else:
            for chat_id, opponent in game.get_broadcast_against():
                bot.send_message(chat_id, f"✏ Oups, {escape(opponent.user_name)} entered *__{escape(opponent.current_word)}__*\. Try again\!", parse_mode=ParseMode.MARKDOWN_V2)
            game.new_round()
        return
