...
def _graph(name, value = None) :
    if value is None :
        return _null(name)
    attr = onnx.AttributeProto()
    attr.name = name
    attr.type = onnx.AttributeProto.AttributeType.GRAPH
    attr.g = value
    return attr

def _tensor(name, value = None) :
    if value is None :
        return _null(name)
    attr = onnx.AttributeProto()
    attr.name = name
...
__all__ = ['_int', '_float', '_string', '_ints', '_null', '_floats', '_strings', '_graph', '_tensor', 'Iterator', '_index_to_offset', '_offset_to_index']
