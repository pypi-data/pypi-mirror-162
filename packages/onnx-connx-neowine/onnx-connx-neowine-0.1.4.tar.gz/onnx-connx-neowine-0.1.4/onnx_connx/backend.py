    ...
    for input_path in input_paths:
        with open(input_path, 'rb') as f:
            try :
                ...
                try :
                    tensor = onnx.SequenceProto()
                    tensor.ParseFromString(f.read())
                    inputs.append(numpy_helper.to_list(tensor))
                except :
                    try :
                        tensor = onnx.OptionalProto()
                        tensor.ParseFromString(f.read())
                        inputs.append(numpy_helper.to_optional(tensor))
                    except :
                        raise Exception('Not support Tensor Type (only Tesnor, Sequence, Optional)')                    
