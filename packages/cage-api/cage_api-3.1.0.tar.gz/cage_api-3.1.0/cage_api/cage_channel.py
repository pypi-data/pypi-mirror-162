...
#   file_create (f_create), 
#   file_remove (f_remove),
#   open (ch_open), 
#   close (ch_close), 
#   is_active (is_open),
#   write (w_cage), 
...
def f_close(self, fchannel=-1, Kerr=[]):
    ...
    if fchannel not in self.cage_ch:
        ...
        if closed_channel == True:
            if not self.NO_SERVER: # file was closed virtually (only for this cage
...
def f_rename(self, server, path, new_name, Kerr):
    ...
    if self.NO_SERVER:
            path_head_old = os.path.split(str(path))[0]       #  https://docs.python.org/3/library/os.path.html
            file_name_old=  os.path.split(str(path))[1]
            if not file_name_old:
                ...
                full_path_old= os.path.join(os.sep, self.local_root, path_head_old)  
            else:
                full_path_old= self.local_root  
            if not os.path.isdir(full_path_old):
                message='Directory " %s " not exist .'% str( full_path_old)
                set_warn_int( Kerr,
                              Mod_name,
                              "f_rename",
                              11,
            ...
            path_file_old = os.path.join(os.sep, full_path_old, file_name_old)
            # ? already_opened
            for ch in self.cage_ch:
                elem= self.cage_ch[ch]    # ("local", file, "wm", path_file)
                if elem[3]== path_file_old:
                    message='File " %s " opened. Rename impossible'% str( path_file_old)
                    set_warn_int( Kerr,
                                  Mod_name,
                                  "f_rename",
                                  12,
            ...
            if os.path.exists(str(path_file_old)):
                if os.path.isfile(str(path_file_old)):  # file with specified name exist
                    file_new_name=  os.path.split(str(new_name))[1]
                    path_file_new = os.path.join(os.sep, full_path_old, file_new_name)
                    ...
                    if os.path.exists(path_file_new):
                        if os.path.isfile(path_file_new):
                            set_warn_int(
                                ...
                                "f_rename",
                                12,
                                message="File with new name " + str(renamed) + " already exists.",
                            )
                            return False
                    try:
                        ...
                        set_err_syst(
                            ...
                            str(path_file_old),
                            err,
                            message="File " + str(path_file_old) + " rename error.",
                        )
                        return False

                    return True

            message='Path " %s " not exist .'% str(path_file_old )
            set_warn_int( Kerr,
                              Mod_name,
                              "f_rename",
                              14,
...
def ch_open(self, server, path, Kerr, mod):
    ...
    if self.NO_SERVER:
            ...
            if not os.path.isdir(full_path):
                message='Directory " %s " not exist .'% str( full_path)
                set_warn_int( Kerr,
                              Mod_name,
                              "ch_open",
                              11,
            ...
            if os.path.exists(str(path_file)):
                if os.path.isfile(str(path_file)):  # file with specified name exist

                    try:
                        file = open(str(path_file), "r+b", 0)
                    except OSError as err:
                        set_err_syst(
                            Kerr,
                            Mod_name,
                            ...
                            str(path_file),
                            err,
                            message="File " + str(path_file) + " open error.",
                        )
                        return False
                    """
                    if not file.closed:
                        message="File " + str(path_file) + " already opened.",
                        set_warn_int( Kerr,
                              Mod_name,
                              "ch_open",
                              14,
                    ...
                    """
                    self.num_cage_ch += 1
                    self.cage_ch[self.num_cage_ch] = ("local", file, "wm", path_file)
                    return self.num_cage_ch

            message='Path " %s " not exist .'% str(path_file )
            set_warn_int( Kerr,
                              Mod_name,
                              "ch_open",
                              12,
    ...
    if self.NO_SERVER:
        ...
        try:
            ...
            set_err_syst(
                    ...
                    16,
                    "I/O",
                    str(self.cage_ch[fchannel][3]),
                    err,
                    message='Channel %d File " %s " close error.'%
                        (fchannel,str(self.cage_ch[fchannel][3]))
                )
            return False
        else:
...
def f_remove(self, server, path, Kerr):
    ...
    if self.NO_SERVER:
            ...
            for ch in self.cage_ch:
                elem= self.cage_ch[ch]    # ("local", file, "wm", path_file)
                if elem[3]== path_file:
                    message=' File " %s " opened. '% str( path_file)
                    set_warn_int( Kerr,
                                  Mod_name,
                                  "f_remove",
                                  12,
                    ...
                    pr ('\n'+message)
                    if self.cage_ch[ch][3] == path_file:
                        if not self.close(ch, Kerr):
                            return False
                        else:
                            break
            if os.path.exists(str(path_file)):
                if os.path.isfile(str(path_file)):  # file with specified name exist
                    ...
                    try:
                        ...
                        set_err_syst(
                            ...
                            str(path_file),
                            err,
                            message="File " + str(path_file) + " remove error.",
                        )
                        return False
...
def is_open(self, fchannel, Kerr, stat):
    ...
    if self.NO_SERVER:
            file = self.cage_ch[fchannel][1]
            #path_head = os.path.split(self.cage_ch[fchannel][3])[0]       #  https://docs.python.org/3/library/os.path.html
            #file_name=  os.path.split(self.cage_ch[fchannel][3])[1]

            already_opened =False
            for ch in self.cage_ch:
                elem= self.cage_ch[ch]    # ("local", file, "wm", path_file)
                if elem[3]== self.cage_ch[fchannel][3]:
                    already_opened =True     

                    try:
                        file = open(elem[3], "r+b", 0)
                    except OSError as err:
                        set_err_syst(
                            Kerr,
                            Mod_name,
                            ...
                            str(path_file),
                            err,
                            message="File " + str(self.cage_ch[fchannel][3]) + " open error.",
                        )
                        return False
                    if not file.closed:
...
def statis(self, Kerr):  #   statistic printing (test method)
    ...
    if self.NO_SERVER:
        print( "\n   Channels info (local files)"  )
        Channels_info = self.remote("local", Kerr)
        print( str(Channels_info) )
    else:
