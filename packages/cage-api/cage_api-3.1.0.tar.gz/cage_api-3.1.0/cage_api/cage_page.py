...
Mod_name = "*" + __name__
zero_page = b"\x00" * (PAGESIZE + 1)

#--------------------------------------------------------------

def read_f(self, channel, pos, length, Kerr=[]):

    file= self.cage_ch[channel][1]  # ("local", file, "wm", path_file)
    ...
    try:
        ...
        set_err_syst(
            ...
            1,
            "I/O",
            self.cage_ch[channel][3],
            err,
            message="File " + self.cage_ch[channel][3] + " EOF seek error.",
        )

        return False

    while (pos + length) > file.tell():
        # file extending
        try:
            # pr ('zero_page ='+ zero_page[:length+1].decode("Cp1251","backslashreplace")+'=' )
            file.write(zero_page[:length])
        except OSError as err:
            set_err_syst(
                Kerr,
                Mod_name,
                ...
                self.cage_ch[channel][3],
                err,
                message="File " + self.cage_ch[channel][3] + " write error.",
            )
            return False
        try:
            file.seek(0, os.SEEK_END)
    ...
    try:
        ...
        # pr("\n ------  after LOAD PAGE from file "+ str(self.cage_ch[channel][3])+ \
        #    " data= " + str( data.decode("Cp1251","backslashreplace")))
    except OSError as err:
        #
        set_err_syst(
            Kerr,
            ...
            self.cage_ch[channel][3],
            err,
            message="File " + self.cage_ch[channel][3] + "read error.",
        )
        return False
...
def write_f(self, channel, pos, data, Kerr=[]):
    ...
    if len_data == 0:
        set_warn_int(
            ...
            "write_f",
            1,
            message="Write to file channel #"
            + str(channel)
            + " not possible. \
                    Length of data equal 0.",
        )
        return False
    ...
    try:
        ...
        set_err_syst(
            ...
            2,
            "I/O",
            self.cage_ch[channel][1],
            err,
            message="File " + self.cage_ch[channel][1] + " EOF seek error.",
        )
        return False

    while (pos + len_data) > file.tell():
        # file extending
        try:
            # pr ('zero_page ='+ zero_page[:len_data].decode("Cp1251","backslashreplace")+'=' )
            file.write(zero_page[:len_data])
        except OSError as err:
            #
            set_err_syst(
                Kerr,
                ...
                self.cage_ch[channel][1],
                err,
                message="File " + self.cage_ch[channel][1] + " write error.",
            )
            return False
        try:
            file.seek(0, os.SEEK_END)
    ...
    try:
        ...
        set_err_syst(
            ...
            self.cage_ch[channel][1],
            err,
            message="File " + self.cage_ch[channel][1] + " seek error.",
        )
        return False
    try:
        file.write(data)
    ...
    try:
        ...
        set_err_syst(
            ...
            self.cage_ch[channel][1],
            err,
            message="File " + self.cage_ch[channel][1] + " flush error.",
        )
        return False
...
def get_p(self, fchannel, fpage, Kerr):
    ...
    if self.WRITE_THREAD and  thread_socket != False:
        # pr(  "*** Get_P   *** 02")
    ...
    # ------------------------------------ write thread  end ----------------------------------------------------
        ...
        if zero_page1 >= 0:
            ...
            if (nbls_push, nf_push) not in self.hash2nat:  
                # check pushing page in dict.
            ...
            if self.binout[nmin1]["prmod"]:  # if pushing page was modified
                ...
                if not self.NO_SERVER:
                    ...
                    write_f(self, nf_push, smstr, self.masstr[nmin1], Kerr=[])
    ...
    if not self.NO_SERVER:
        ...
        if event == -1:
            ...
            #if self.WRITE_THREAD and self.lock_write.locked():
                #self.lock_write.release()                                                ###   RELEASE  lock write
            return -1
        ...
        self.masstr[page_update] = read_f(self, fchannel, smstr, self.pagesize, Kerr=[])
    ...
    if not self.NO_SERVER:
        ...
        for page in range(self.numpages):
            ...
            if self.binout[page]["prmod"] and self.binout[page]["nf"] == fchannel:
                ...
                write_f(self, fchannel, smstr, self.masstr[page], Kerr=[])
    ...
    for page in range(self.numpages):
        if self.binout[page]["prmod"]:
            ...
            if not self.NO_SERVER:
                ...
                write_f(self, nf, smstr, self.masstr[page], Kerr=[])
