...
class BalanceUtility:
    ...
    # def getDOTBalance(self, nodes: list, dotAddress):
        ...
        urls = kwargs['nodes']
        for url in urls:
            balance = self.getDOTBalanceViaNode(url, kwargs['dotAddress'])
            if balance:
    ...
    # def getXRPBalance(self, nodes: list, xrpAddress):
        ...
        urls = kwargs['nodes']
        for url in urls:
            balance = self.getXRPBalanceViaNode(url,kwargs['xrpAddress'])
            if balance:
...
if __name__ == '__main__':
    ...
    args = {'nodes':['wss://nodes.wandevs.org/polkadot'],'dotAddress':'13NB3m2P3nw1wfzbHqMHQ3Y6cKZrFRk18r3Z8yt4fGKMb5Gx'}
