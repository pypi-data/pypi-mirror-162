...
class DependencyResolutionFailure:
    ...
    def __repr__(self):
        return f"DependencyResolutionFailure(key:{self.key}\t,trace:{' => '.join(self.trace)},cause: ({self.cause})"
...
class DIGraph:
    ...
    def di_dfs(self, src):
        def dfs(node: str, trace=[]):
            try:
                ...
                raise MissingDependencyError(f"failed to get neighbors of {node} at {' => '.join(trace)}.", node,
                                             trace) from e
            for n in nexts:
                yield node, n, trace
    ...
    def find_missing_dependencies(self, roots: Union[str, List[str]]):
        if isinstance(roots, str):
            roots = [roots]
        failures = []
        for r in roots:
