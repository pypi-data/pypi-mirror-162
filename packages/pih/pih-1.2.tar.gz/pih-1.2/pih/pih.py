...
from pih.const import AD_CONST, DATA_EXTRACTOR, FACADE, EXECUTOR, FIELD_NAME_COLLECTION, LOCAL_COMMAND_LIST_DICT, NAME_POLICY_CONST, FIELD_COLLECTION, PASSWORD, SITE
from pih.rpc_commnads import RPC_COMMANDS
from pih.tools import DataTools, DataUnpack, FullNameTool, PasswordTools
...
class PIH:
    ...
    class DATA:
        ...
        class EXTRACT:
            ...
            @staticmethod
            def active_users(data: List) -> List:
                return filter(lambda x: AD_CONST.ACTIVE_STATUS in x, data)

            @staticmethod
            def tab_number(mark: dict) -> str:
                return mark[FIELD_NAME_COLLECTION.TAB_NUMBER] if FIELD_NAME_COLLECTION.TAB_NUMBER in mark else None

            @staticmethod
            def telephone(user: dict) -> str:
                return user[FIELD_NAME_COLLECTION.TELEPHONE] if FIELD_NAME_COLLECTION.TELEPHONE in user else None

            @staticmethod
            def name(mark: dict) -> str:
                return mark[FIELD_NAME_COLLECTION.NAME] if FIELD_NAME_COLLECTION.NAME in mark else None

            @staticmethod
            def dn(user: dict) -> str:
                return user[FIELD_NAME_COLLECTION.DN] if FIELD_NAME_COLLECTION.DN in user else None

            @staticmethod
            def full_name(mark: dict) -> str:
                name = PIH.DATA.EXTRACT.name(mark)
                return FullNameTool.from_string(name)
    ...
    class VISUAL:
        ...
        def show_table_with_caption_first_title_is_centered(data: Dict, caption: str, use_index: bool = False, modify_data_handler: Callable = None) -> None:
            ...
            PIH.VISUAL.show_table_with_caption(
                data, caption, modify_table, modify_data_handler, use_index)
        ...
        def show_table_with_caption_last_title_is_centered(data: Dict, caption: str, use_index: bool = False) -> None:
            ...
            PIH.VISUAL.show_table_with_caption(
                data, caption, modify_table, None, use_index)

        @staticmethod
        def show_table_with_caption(data: Dict, caption: str = None, modify_table_handler: Callable = None, modify_data_handler: Callable = None, use_index: bool = False) -> None:
            PIH.VISUAL.init()
