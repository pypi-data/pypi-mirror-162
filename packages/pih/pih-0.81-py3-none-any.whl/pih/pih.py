...
from pih.collection import CommandChainItem, CommandLinkItem, FieldItem, CommandItem, FullName
from pih.const import DATA_EXTRACTOR, FACADE, EXECUTOR, FIELD_NAME_COLLECTION, LOCAL_COMMAND_LIST_DICT, NAME_POLICY_CONST, FIELD_COLLECTION
from pih.rpc import RPC
from pih.tools import DataTools, DataUnpack
...
class PIH:
    ...
    class DATA:
        ...
        def generate_password(type: str) -> str:
            return RPC.AD.generate_password(type)
    ...
    class INPUT:
        ...
        @staticmethod
        def password(secret: bool = True, check: bool = False) -> str:
            PR.cyan("Set password:")
            return getpass(" ") if secret else input()

        @staticmethod
        def generate_password(type: str = None) -> str:
            while True:  
                password = PIH.DATA.generate_password(type)
                PR.blue(f"Gnerated password is {password}")
