...
from pih.collection import CommandChainItem, CommandLinkItem, FieldItem, CommandItem, FieldItemList, FullName, PasswordSettings
from pih.const import AD_CONST, DATA_EXTRACTOR, FACADE, EXECUTOR, FIELD_NAME_COLLECTION, LOCAL_COMMAND_LIST_DICT, NAME_POLICY_CONST, FIELD_COLLECTION, PASSWORD, SITE, USER_PROPERTY
...
class PIH:
    ...
    class DATA:
        ...
        class EXTRACT:
            ...
            def tab_number(mark: dict) -> str:
                return PIH.DATA.EXTRACT(mark, FIELD_NAME_COLLECTION.TAB_NUMBER)

            @staticmethod
            def telephone(user: dict) -> str:
                return PIH.DATA.EXTRACT(user, FIELD_NAME_COLLECTION.TELEPHONE)

            @staticmethod
            def name(mark: dict) -> str:
                return PIH.DATA.EXTRACT(mark, FIELD_NAME_COLLECTION.NAME)

            @staticmethod
            def dn(user: dict) -> str:
                return PIH.DATA.EXTRACT(user, FIELD_NAME_COLLECTION.DN)

            @staticmethod
            def full_name(mark: dict) -> FullName:
                return FullNameTool.from_string(PIH.DATA.EXTRACT.parameter(mark, FIELD_NAME_COLLECTION.FULL_NAME))

            @staticmethod
            def description(data: dict) -> str:
                result = data[FIELD_NAME_COLLECTION.DESCRIPTION] if FIELD_NAME_COLLECTION.DESCRIPTION in data else None
                if result is None:
                    return ""
            ...
            def container_dn(user_data: dict) -> str:
                return ",".join(PIH.DATA.EXTRACT.dn(user_data).split(",")[1:])
        
        class FILTER:

            def users_by_dn(data: List, dn: str) -> List:
                return list(filter(lambda x: PIH.DATA.EXTRACT.container_dn(x).find(dn) != -1, data ))
        ...
        @staticmethod
        def user_by_full_name(value: FullName) -> dict:
    ...
    class INPUT:
        ...
        @staticmethod
        def indexed_list(list: FieldItemList, caption: str) -> str:
            for index, item in enumerate(list.list):
                print(f"{index + 1}: {item.caption}")
            return list.get_name_list()[PIH.INPUT.index(
                caption, list.length())]
        ...
        def templated_user() -> dict:
            result = PIH.DATA.templated_users()
            data = ResultUnpack.unpack_data(result)
            PIH.VISUAL.show_templated_users_for_data(result, True)
            return data[PIH.INPUT.index("Choose templated user:", len(data))]

        @staticmethod
        def yes_no(text: str = " ", enter_for_yes: bool = False) -> bool:

            answer = input(f"{PR.blue_str(text)}{' ' if text != '' else ''}{PR.green_str(' Yes (Yes|Y|1|Enter) ')} {PR.red_str(' No (Another) ')}:" if enter_for_yes else
    ...
    class VISUAL:
        ...
        @staticmethod
        def show_free_marks(use_index: bool = False) -> None:
            PIH.VISUAL.show_table_with_caption_first_title_is_centered(
                PIH.DATA.free_marks(), "Free marks:", use_index)
        ...
        @staticmethod
        def show_free_marks_group_statistics(use_index: bool = False) -> None:
            PIH.VISUAL.show_free_marks_group_statistics_for_data(
        ...
        @staticmethod
        def show_table_with_caption_first_title_is_centered(data: Dict, caption: str, use_index: bool = False, modify_data_handler: Callable = None) -> None:
        ...
        @staticmethod
        def show_table_with_caption_last_title_is_centered(data: Dict, caption: str, use_index: bool = False, modify_data_handler: Callable = None) -> None:
            def modify_table(table: PrettyTable, caption_list: List[str]):
        ...
        @staticmethod
        def show_table_with_caption(data: Dict, caption: str = None, use_index: bool = False, modify_table_handler: Callable = None, modify_data_handler: Callable = None) -> None:
            PIH.VISUAL.init()
            if caption is not None:
                PR.cyan(caption)
            field_list, result = ResultUnpack.unpack(data)
            if use_index:
        ...
        @staticmethod
        def show_templated_users_for_data(data: dict, use_index: bool = False) -> None:
            def data_handler(field_item: FieldItem, item: Any) -> Any:
                ...
                if filed_name == FIELD_NAME_COLLECTION.DESCRIPTION:
                    return item[filed_name][0] if item is not None and filed_name in item and len(item[filed_name]) > 0 else ""
                return None
            PIH.VISUAL.show_table_with_caption(data, "Templated users:", None, data_handler, use_index)
    ...
    class ACTION:
        ...
        class AD:
            ...
            def create_user_from_template(templated_user: dict,
                ...
                try:
                    template_user_dn = PIH.DATA.EXTRACT.dn(templated_user)
                    RPC_COMMANDS.AD.create_user_from_template(
                        template_user_dn, full_name, login, password, description, telephone, email)
                except:
                    pass
        ...
        class ORION:
            ...
            @staticmethod
            def make_mark_as_free_by_tab_number(tab_number: dict) -> bool:
        ...
        def generate_password(once: bool = False, settings: PasswordSettings = None) -> str:
            ...
            while True:  
                ...
                PR.green(f"Generated password is {password}")
