...
class PIH:
    ...
    class DATA:
        ...
        def generate_login(full_name: FullName) -> str:
            ...
            login_string: str = login.to_string()
            if PIH.CHECK.user_is_exsits_by_login(login_string):
                ...
                login_alt = NamePolicy.convert_to_alternative_login(
                    login)
                login_string = login_alt.to_string()
                if PIH.CHECK.user_is_exsits_by_login(login_string):
                    PR.red(f"Login '{login_string}' is not free")
                    login_reversed = NamePolicy.convert_to_reverse_login(login)
                    login_string = login_reversed.to_string()
                    if PIH.CHECK.user_is_exsits_by_login(login_string):
                        PR.red(f"Login '{login_string}' is not free")
                        while True:
    ...
    class ACTION:
        ...
        class ORION:
            ...
            def update_full_name_by_tab_number(full_name: FullName, tab_number: dict) -> bool:
                return DataTools.rpc_unrepresent(RPC_COMMANDS.ORION.update_full_name_by_tab_number(full_name, tab_number))
