...
@decorator
def cache4batch(func, location='cachedir', verbose=0, *args, **kwargs):
    """disk_cache 存中存"""

    @disk_cache(location=location)
    def wrap(batch):  # 函数最好支持 one and batch
        if isinstance(batch, Iterable) and not isinstance(batch, str):
            if verbose:
                batch = tqdm(batch, desc='miss')
            return np.row_stack(list(map(wrap, batch)))  # 递归
        return func([batch]) # batch => [batch] func 支持 batch
