def BIN_to_BEAN(_bin_: str, encryption_key, remainder=None) -> str:
    bean_text = ''

    if (len(_bin_))%len(encryption_key) != 0:
        beans = encryption_key*(((len(_bin_))//len(encryption_key))+1)
    else:
        beans = encryption_key*((len(_bin_))//len(encryption_key))
...
def beanory(plain_text: str, encryption_key: str, encrypt=True, bit_length=8) -> str:
    ...
    if encrypt:
        ...
        bean_text = BIN_to_BEAN(bin_nbit, encryption_key)
        ...
        if len(bean_text)%encryption_key_len != 0:
            ...
            bean_rem = BIN_to_BEAN(remainder_bin_kwn_unk, encryption_key, remainder=_remainder_)
        else:
            bean_rem = ''
