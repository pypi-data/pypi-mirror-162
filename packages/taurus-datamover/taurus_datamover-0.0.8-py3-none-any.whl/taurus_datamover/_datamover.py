...
class Workspace(Datamover):
    ...
    def mk_cache(self, name: str = 'cache', expire_in_days: int = 1) -> None:
        ...
        '''
        res = subprocess.run(
            [str(self.path_to_exe / 'ws_allocate'), name, str(expire_in_days), '-c', 'temporary_files'], capture_output=True)
        return res.stdout.strip()


class CacheWorkspaceContext:
    ...
    def __enter__(self) -> None:
        if self.cache_dir is None:
            self.cache_dir = self.ws.mk_cache(self.name)
