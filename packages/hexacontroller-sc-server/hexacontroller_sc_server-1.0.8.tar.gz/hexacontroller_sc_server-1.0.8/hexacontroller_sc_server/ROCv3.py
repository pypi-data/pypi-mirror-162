...
class ROCv3():
    ...
    def _translate(self, valid_config: dict,
                   ...
                   translation_output_registers=[],
                   transaction_log_written=False) -> list:
        # do this to avoid weird referencing and static variable behaviour
        translation_output_registers = translation_output_registers.copy()
        if not transaction_log_written:
            self.transaction_logger.debug(
        ...
        for key, val in valid_config.items():
            if isinstance(val, dict):
                translation_output_registers += self._translate(
                        val,
                        translation_dictionary[key],
                        translation_output_registers,
                        transaction_log_written=True)
            else:
                ...
                for r, m in zip(param_registers, masks):
                    aligned_val = \
                        ...
                        << m['reg_minbit']
                    translation_output_registers.append(
                            (r[0], r[1], aligned_val, m['reg_mask']))
        return translation_output_registers
