...
def get_factor_return(
    start_date, end_date, factors=None, universe="whole_market", method="implicit", industry_mapping=True, model="cne5", market="cn"
):
    """获取因子收益率数据
    ...
    :param industry_mapping(bool): 是否使用 申万2021 行业分类, 若为True, 使用申万2021行业分类, 若为False, 则使用2014申万行业分类;
        默认为 True
    :param model: 选用的模型, 可选模型包括 cne5, cne6; 默认为 cne5
    ...
    check_items_in_container(model, ["cne5", "cne6"], "model")

    model_to_api = {
        "cne5": "get_factor_return_v2",
        "cne6": "get_factor_return_cne6",
    }
...
@export_as_api
def get_factor_exposure(order_book_ids, start_date=None, end_date=None, factors=None, industry_mapping='sw2021', model="cne5", market="cn"):
    """获取因子暴露度
    ...
    if factors is not None:
        ...
        if model == "cne5":
            ...
            check_items_in_container(factors, exposure_factors_cne6, "factors")

    model_to_api = {
        "cne5": "get_factor_exposure_v2",
        "cne6": "get_factor_exposure_cne6"
    }
...
exposure_factors_cne6.extend(SHENWAN_INDUSTRY_2021)
...
@export_as_api
def get_style_factor_exposure(order_book_ids, start_date, end_date, factors=None, model="cne5", market="cn"):
    """获取个股风格因子暴露度
    ...
    if factors is not None:
        ...
        if model == "cne5":
            ...
            check_items_in_container(factors, _STYLE_FACTORS_CNE6, "factors")

    model_to_api = {
        "cne5": "get_style_factor_exposure",
        "cne6": "get_style_factor_exposure_cne6",
    }
    df = get_client().execute(
...
@export_as_api
def get_descriptor_exposure(order_book_ids, start_date, end_date, descriptors=None, model="cne5", market="cn"):
    """获取个股细分因子暴露度
    ...
    if descriptors is not None:
        if descriptors == "all":
            ...
            if model == "cne5":
                ...
                check_items_in_container(descriptors, _DESCRIPTORS_CNE6, "descriptors")

    model_to_api = {
        "cne5": "get_descriptor_exposure",
        "cne6": "get_descriptor_exposure_cne6"
    }
    df = get_client().execute(
...
def get_eigenfactor_adjusted_covariance(date, horizon='daily', model="cne5"):
    """ 获取因子协方差矩阵（特征因子调整）
    ...
    model_to_api = {
        "cne5": 'get_eigenfactor_adjusted_covariance',
        "cne6": 'get_eigenfactor_adjusted_covariance_cne6',
    }
...
@export_as_api
def get_factor_covariance(date, horizon='daily', model="cne5"):
    """ 获取因子协方差矩阵
    ...
    model_to_api = {
        "cne5": "get_factor_covariance",
        "cne6": "get_factor_covariance_cne6"
    }
    df = get_client().execute(model_to_api[model], date, horizon)
    if not df:
        return
...
@export_as_api
def get_specific_return(order_book_ids, start_date, end_date, model="cne5"):
    """ 获取个股特异收益率
    ...
    model_to_api = {
        "cne5": "get_specific_return",
        "cne6": "get_specific_return_cne6"
    }
...
@export_as_api
def get_specific_risk(order_book_ids, start_date, end_date, horizon='daily', model="cne5"):
    """ 获取个股特异方差
    ...
    model_to_api = {
        "cne5": "get_specific_risk",
        "cne6": "get_specific_risk_cne6"
    }
...
def get_cross_sectional_bias(start_date, end_date, type='factor', model="cne5"):
    """ 获取横截面偏差系数
    ...
    model_to_api = {
        "cne5": "get_cross_sectional_bias",
        "cne6": "get_cross_sectional_bias_cne6"
    }
