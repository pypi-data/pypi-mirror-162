    ...
    with open(filepath) as file:
        ...
        while line:
            ...
            if read_until_all_Y(IDs) < 0:
                print("ERROR in recieving acknowlege from Boards, rebooting accessible boards")
...
for id in ids:
    try:
        print("\nRecieved RDY! from board: " + board_ids[id - broadcast_id - 1] + " with FW version: " + str(
            fw_versions[id]))
    except:
        print("\nRecieved RDY! from following ID: " + hex(id))
...
if res == 'R':
    ...
    linecount = send_hex_file(filepath, ids, broadcast_id)
    # Writing FW Flash Command
    print("\nWriting Flash Command ... Done")
