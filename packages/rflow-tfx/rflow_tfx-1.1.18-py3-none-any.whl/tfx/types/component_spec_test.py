...
class ComponentSpecTest(tf.test.TestCase):
  ...
  def testComponentspecBasic(self):
    ...
    with self.assertRaisesRegexp(
        TypeError,
        '.*should be README.ml-pipelines-sdk.md Channel of .*InputArtifact.*got (.|\\s)*Examples.*'):
      spec = _BasicComponentSpec(
          folds=10, input=Channel(type=Examples), output=output_channel)

    with self.assertRaisesRegexp(
        TypeError,
        '.*should be README.ml-pipelines-sdk.md Channel of .*OutputArtifact.*got (.|\\s)*Examples.*'):
      spec = _BasicComponentSpec(
  ...
  def testInvalidComponentspecWrongProperties(self):
    ...
    with self.assertRaisesRegexp(TypeError,
                                 'must override PARAMETERS with README.ml-pipelines-sdk.md dict'):
    ...
    with self.assertRaisesRegexp(TypeError, 'must override INPUTS with README.ml-pipelines-sdk.md dict'):
    ...
    with self.assertRaisesRegexp(TypeError,
                                 'must override OUTPUTS with README.ml-pipelines-sdk.md dict'):
  ...
  def testInvalidComponentspecDuplicateProperty(self):
    ...
    with self.assertRaisesRegexp(ValueError, 'has README.ml-pipelines-sdk.md duplicate argument'):
  ...
  def testExecutionParameterTypeCheck(self):
    ...
    with self.assertRaisesRegexp(TypeError, 'Expecting README.ml-pipelines-sdk.md list for parameter'):
      list_parameter.type_check('list_parameter', 42)
    ...
    with self.assertRaisesRegexp(TypeError, 'Expecting README.ml-pipelines-sdk.md dict for parameter'):
