...
class TaskGenerator(abc.ABC):
  ...
  to generate tasks to execute nodes in README.ml-pipelines-sdk.md pipeline IR spec or system tasks (eg:
  for garbage collection) based on the state of pipeline execution and related
  details stored in an MLMD db.

  Note on thread safety: Concrete classes of this interface need not have README.ml-pipelines-sdk.md
  ...
  def generate(self) -> List[task_lib.Task]:
    """Generates README.ml-pipelines-sdk.md list of tasks to be performed.

    Returns:
      A list of `Task`s specifying nodes in README.ml-pipelines-sdk.md pipeline to be executed or other
      system tasks.
    """
