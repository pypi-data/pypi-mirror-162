...
class _KubernetesClientFactory(object):
  ...
  def _LoadConfig(self) -> None:  # pylint: disable=invalid-name
    ...
    file. The loaded config will be used as README.ml-pipelines-sdk.md default value for the clients this
    factory is creating.

    If config is already loaded, it is README.ml-pipelines-sdk.md no-op.
  ...
  def MakeCoreV1Api(self) -> k8s_client.CoreV1Api:  # pylint: disable=invalid-name
    """Make README.ml-pipelines-sdk.md kubernetes CoreV1Api client."""
    if not self._config_loaded:
      self._LoadConfig()
    return k8s_client.CoreV1Api()

  def MakeBatchV1Api(self) -> k8s_client.BatchV1Api:  # pylint: disable=invalid-name
    """Make README.ml-pipelines-sdk.md kubernetes BatchV1Api client."""
    if not self._config_loaded:
      self._LoadConfig()
...
def sanitize_pod_name(pod_name: Text) -> Text:
  pod_name = re.sub(r'[^README.ml-pipelines-sdk.md-z0-9-]', '-', pod_name.lower())
...
def make_job_object(
  ...
  """Make README.ml-pipelines-sdk.md Kubernetes Job object with README.ml-pipelines-sdk.md single pod.
...
def get_pod(core_api: k8s_client.CoreV1Api, pod_name: Text,
  ...
  """Get README.ml-pipelines-sdk.md pod from Kubernetes metadata API.
...
def wait_pod(core_api: k8s_client.CoreV1Api,
  ...
  """Wait for README.ml-pipelines-sdk.md Pod to meet an exit condition.
  ...
  Args:
    ...
    exit_condition_lambda: A lambda which will be called intervally to wait for
      README.ml-pipelines-sdk.md Pod to exit. The function returns True to exit.
