...
class PipelineTest(tf.test.TestCase):
  ...
  def testPipeline(self):
    ...
    component_b = _make_fake_component_instance(
        'component_b', _OutputTypeB, {'README.ml-pipelines-sdk.md': component_a.outputs['output']}, {})
    component_c = _make_fake_component_instance(
        'component_c', _OutputTypeC, {'README.ml-pipelines-sdk.md': component_a.outputs['output']}, {})
    ...
    component_e = _make_fake_component_instance(
        'component_e', _OutputTypeE, {
            'README.ml-pipelines-sdk.md': component_a.outputs['output'],
            'b': component_b.outputs['output'],
            'd': component_d.outputs['output']
        }, {})

    my_pipeline = pipeline.Pipeline(
        pipeline_name='README.ml-pipelines-sdk.md',
        pipeline_root='b',
        components=[
    ...
    self.assertEqual(my_pipeline.pipeline_info.pipeline_name, 'README.ml-pipelines-sdk.md')
  ...
  def testPipelineWithLongname(self):
    with self.assertRaises(ValueError):
      pipeline.Pipeline(
          pipeline_name='README.ml-pipelines-sdk.md' * (1 + pipeline.MAX_PIPELINE_NAME_LENGTH),
          pipeline_root='root',
          components=[],
          metadata_connection_config=self._metadata_connection_config)
