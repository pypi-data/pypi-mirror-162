...
class Accounts(Resource):
    ...
    def serialize(self, payload=None, operation=None):
        ...
        elif operation == super().PDF_TO_JSON:
            data = payload
    ...
    def response(self):
        ...
        elif super().get_operation() == super().PDF_TO_JSON:
            std_data_list = []
            std_data = {}
            if super().get_bank_id() == super().SAFCOM:
                if isinstance(response_data,dict):
                    ...
                    if statement_type == 'personal':
                        for i in range(len(data)):
                            std_data = {
                                "date_time": data[i].get("completion_time", ""),
                                "transaction_details": data[i].get("details", ""),
                                "reference": data[i].get("receipt_no", ""),
                                "balance": data[i].get("balance", "")
                            }

                            if float(data[i].get("paid_in", -1).replace(',','')) > 0:
                                std_data['type'] = 'credit'
                                std_data['amount'] = data[i].get("paid_in", -1)
                            if float(data[i].get("withdraw", -1).replace(',','')) > 0:
                                std_data['type'] = 'debit'
                                std_data['amount'] = data[i].get("withdraw", -1)
                ...
                if isinstance(response_data,dict):
                    ...
                    if statement_type == 'personal':
                        for i in range(len(data)):
                            std_data = {
                                "date_time": data[i].get("transaction", ""),
                                "transaction_details": f'{data[i].get("transaction_type", "")} {data[i].get("transaction_details", "")}',
                            ...
                            if float(data[i].get("debits", -1).replace(',','')) > 0:
                                std_data['type'] = 'debit'
                                std_data['amount'] = data[i].get("debits", -1)
                ...
                if isinstance(response_data,dict):
                    ...
                    if statement_type == 'personal':
                        for i in range(len(data)):
                            std_data = {
                                "date_time": data[i].get("date", ""),
                                "transaction_details": f'{data[i].get("transaction_details", "")}',
                                "reference": "",
                                "balance": data[i].get("balance", ""),
                ...
                if isinstance(response_data,dict):
                    ...
                    if statement_type == 'personal':
                        for i in range(len(data)):
                            std_data = {
                                "date_time": data[i].get("transaction_date", ""),
                                "transaction_details": f'{data[i].get("transaction_details", "")}',
                            ...
                            try:
                                if float(data[i].get("credit", -1).replace(',','')) > 0:
                                    std_data['type'] = 'credit'
                                    std_data['amount'] = data[i].get("credit", -1)
                            except:
                                pass
                            ...
                            try:
                                if float(data[i].get("debit", -1).replace(',','')) > 0:
                                    std_data['type'] = 'debit'
                                    std_data['amount'] = data[i].get("debit", -1)
                            except:
                                pass
                ...
                if isinstance(response_data,dict):
                    ...
                    if statement_type == 'personal':
                        for i in range(len(data)):
                            std_data = {
                                "date_time": data[i].get("entry_date", ""),
                                "transaction_details": f'{data[i].get("description", "")}',
                                "reference": data[i].get("reference", ""),
                                "balance": data[i].get("balance", ""),
                            ...
                            try:
                                if float(data[i].get("deposit", -1).replace(',','')) > 0:
                                    std_data['type'] = 'credit'
                                    std_data['amount'] = data[i].get("deposit", -1)
                            except:
                                pass
                            ...
                            try:
                                if float(data[i].get("withdrawal", -1).replace(',','')) > 0:
                                    std_data['type'] = 'debit'
                                    std_data['amount'] = data[i].get("withdrawal", -1)
                            except:
                                pass
                    ...
                    if statement_type == 'business':
                        ...
                        for i in range(len(data)):
                            ...
                            if float(data[i].get("money_in", -1).replace(',','')) > 0:
                                std_data['type'] = 'credit'
                                std_data['amount'] = data[i].get("money_in", -1)
                            if float(data[i].get("money_out", -1).replace(',','')) < 0:
                                std_data['type'] = 'debit'
                                std_data['amount'] = data[i].get("money_out", -1).replace('-','')
