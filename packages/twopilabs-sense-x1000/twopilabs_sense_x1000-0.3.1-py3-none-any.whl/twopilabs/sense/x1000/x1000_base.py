...
class SenseX1000Base(object):
    ...
    class AcqHeader(AcqBaseHeader):
        ...
        def from_stream(cls, stream: BinaryIO) -> 'SenseX1000Base.AcqHeader':
            ...
            subheaders_dict = {a: c.from_stream(stream) for a, c in cls.subheaders().items()
                               if baseheader.subheader_flags & c.flag()}

            # Sanity check for total header length
            header_length = baseheader.struct().size + sum([i.struct().size for i in subheaders_dict.values()])

            # Silently discard remaining header bytes (may be unsupported subtypes)
            stream.read(baseheader.header_length - header_length)
