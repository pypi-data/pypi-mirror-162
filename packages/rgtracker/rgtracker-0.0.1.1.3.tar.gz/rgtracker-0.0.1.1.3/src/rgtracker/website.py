...
class Website:
    ...
    def write_metadata(self, stream_name, previous_keys=None):
        if previous_keys is None:
            ...
            execute('XADD', stream_name, '*', 'dimension', Dimension.WEBSITE.value, 'id', self.id, 'ts',
                    parsed_key.get('ts'), 'merged_key', self.dimension_ts_key)
        else:
...
def load_website(website, section, page, device):
    if website.is_exists() != 1:
        ...
        website.write_metadata(stream_name=create_key_name(type=Type.STREAM.value, name='BLOOM-ROTATE-1MINUTE'))
    else:
