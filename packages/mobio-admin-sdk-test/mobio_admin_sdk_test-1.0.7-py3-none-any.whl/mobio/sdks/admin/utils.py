...
from .config import SystemConfigKeys, UrlConfig, KafkaTopic, CodeErrorDecrypt, CodeErrorEncrypt
import datetime
import requests
...
@MobioAdminSDK.lru_cache.add()
def get_info_fields_config(merchant_id, module, field):
    field_config = {}
    try:
...
def encrypt_field_by_config(merchant_id, module, field, values):
    field_config = get_info_fields_config(merchant_id, module, field)
    kms_id, normalize_config = None, None
    if field_config and isinstance(field_config, dict):
        kms_id = field_config.get("kms_id")
        normalize_config = field_config.get("normalize_config")
    ...
    if kms_id:
        ...
        if kms_info:
            if kms_info.get("kms_type") == "kms_mobio":
                ...
                data_response = {"data": data, "data_error": data_error}
            else:
        ...
        data_response = build_response_from_list(values)
    if normalize_config:
        data_response.update({"normalize_config": normalize_config})
