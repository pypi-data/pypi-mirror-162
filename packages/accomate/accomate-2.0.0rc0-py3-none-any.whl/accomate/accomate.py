...
def init(store_name: str, version: str, store_email: str, environment: str = "production", template: str = "accomate-subscription-template"):
    ...
    base_dns_response = add_dns_record(
        hosted_zone_id, "A", base_domain, machine_ip_address)

    if not base_dns_response:
    ...
    with open(f"{repository_clone_path}/accomate.json", "w+", encoding="utf-8") as accomate:
        json_data = {
            ...
            "urls": {
                "base": base_domain,
                "store_service": f"{base_domain}/store",
                "auth_service": f"{base_domain}/auth",
            },
            "secrets": {
...
def setup(store_url: str = "https://store-new.accomate.com", auth_url: str = "https://auth.accomate.com", plan: str = "RedirectionProcess"):
    ...
    try:
        ...
        if not project_config:
            ...
            if verify_dns_record(hosted_zone_id, 'A', base_domain, machine_ip_address):
                echo(
                    f"✅ Client domain {base_domain} is verified.")
            else:
                echo(
                    f"❌ Client domain {base_domain} is not verified.")
                raise typer.Abort()
            ...
            echo(f"Creating bucket: {parsed_store_name_dir}")
            ...
            if bucket is not None:
                ...
                if scope_config is not None:
                    reward_token_scopes = scope_config['plans'][plan]['token_scopes']
                ...
                CSRF_ALLOWED_ORIGINS.append(f"https://{base_domain}")
        ...
        if nginx:
            ...
            try:
                with open(f"{absolute_package_path}/conf/nginx/site.conf", "r") as site_template:
                    ...
                    client_nginx_site_config = template
                    ...
                    client_nginx_site_config = client_nginx_site_config.replace(
                        "#domain#", base_domain)
                    client_nginx_site_config = client_nginx_site_config.replace(
                        "#static_files_path#", auth_service_static_files_path)
                    client_nginx_site_config = client_nginx_site_config.replace(
                        "#proxy_pass_uri_auth#", auth_service_proxy_pass)
                    client_nginx_site_config = client_nginx_site_config.replace(
                        "#proxy_pass_uri_store#", store_service_proxy_pass)
                    client_nginx_site_config = client_nginx_site_config.replace(
                        "#access_log#", f"{base_nginx_site_log_path}/access.log")
                    client_nginx_site_config = client_nginx_site_config.replace(
                        "#error_log#", f"{base_nginx_site_log_path}/error.log")

                    echo("Writing client site nginx conf...")
                    with open(f"{base_nginx_path}/{base_domain}.conf", "w+") as f:
                        f.write(client_nginx_site_config)
            ...
            client_nginx_site_config_exists = os.path.exists(
                f"{base_nginx_path}/{base_domain}.conf")

            if client_nginx_site_config_exists:
...
def config_ssl():
    try:
        ...
        add_lets_encrypt_certificate(base_domain, store_email)
...
def health(exec = ""):
    try:
        ...
        echo(f"Health check for {base_domain}")

        is_up = False

        while not is_up:
            try:
                response = httpx.get(base_domain)
                if response is not None:
                    p_status = response.status_code // 500
                    if p_status != 1:
                        is_up = True
                        echo("✅ It's up and running")
            except Exception as e:
                echo("🔄 Waiting for 1 sec...", end="")
                sleep(1)
        ...
        if exec != "":
            ...
            services_by_tags = assign_service_type_tag(
                service_containers, parsed_dir)
            auth_container_id = services_by_tags['auth'].get('id')
            res = exec_run(auth_container_id, "ls")
            if res.exit_code == 0:
                echo("Executed !!")
            else:
