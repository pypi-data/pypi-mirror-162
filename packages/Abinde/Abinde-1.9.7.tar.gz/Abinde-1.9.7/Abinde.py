...
class sprite:
    class Player(pygame.sprite.Sprite):
        def __init__(self, image, pos=[20, 20], title="Sprite", FRIC=0.9, ACC=1, GRAV=1, jumpheight=10, health=10):
            super().__init__()
            global players
            players.append(self)
        ...
        def draw(self, game):
            if pygame.sprite.spritecollideany(self, enemies):
                if self.bleedout_timer == 40:
                    game.root.fill(color.RED4)
                    self.bleedout_timer_2 +=1
                    if self.bleedout_timer_2 == 20:
                        self.bleedout_timer = 0
                        self.bleedout_timer_2 = 0
                        self.health -= 1
                else:
    ...
    class Enemy(pygame.sprite.Sprite):
        def __init__(self, image, line=[0, 100], pos=[20, 20], title="Enemy", FRIC=0.9, ACC=1, GRAV=1):
            super().__init__()
            global enemies, _enemies
            enemies.add(self)
