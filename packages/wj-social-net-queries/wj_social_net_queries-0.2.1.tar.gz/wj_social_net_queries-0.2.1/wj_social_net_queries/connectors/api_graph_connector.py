...
from social_net_queries.wj_social_net_queries.connectors.api_connector import (
    ApiConnector,
)
from social_net_queries.wj_social_net_queries.utils import utils
from social_net_queries.wj_social_net_queries.utils.constants.api_graph_constants import (  # Fields; Operations
    ACCESS_TOKEN,
    GET_ACCOUNTS,
...
class MetaApiConnector(ApiConnector):
    ...
    def __add_token__(self, request_url: str, token:str, prefix: Optional[str]=None) -> str:
        if 'fields' in request_url:
            token_field = self.api_token.format(
                prefix='&',
                token=token
            )
        else:
    ...
    def get_long_lived_token(self, access_token: Optional[str]=None):
        ...
        request_url += LONG_LIVED_TOKEN.format(
            app_id=META_APP_ID,
            api_key_secret=META_API_SECRET_KEY,
            token=self.token
        )
