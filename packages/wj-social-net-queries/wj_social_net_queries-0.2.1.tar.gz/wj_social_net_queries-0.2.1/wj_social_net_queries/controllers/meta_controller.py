...
from social_net_queries.wj_social_net_queries.connectors.api_graph_connector import (
    MetaApiConnector,
)
from social_net_queries.wj_social_net_queries.utils.constants.api_graph_constants import (
    MEDIA_FIELDS,
    RECENT_MEDIA_OPTION,
    TOP_MEDIA_OPTION,
)
from social_net_queries.wj_social_net_queries.utils.constants.constants import (
    CSV,
    JSON,
    MEDIA_DIR,
)
from social_net_queries.wj_social_net_queries.utils.ig_utils import (
    get_shortcode_from_permalink,
)
from social_net_queries.wj_social_net_queries.utils.utils import (
    download_image,
    safe_data_on_csv,
    save_data_on_json,
)
...
def generate_long_lived_token(access_token:str) -> str:
    ...
    """
    meta_connector = MetaApiConnector(
        token=access_token
    )
...
def download_recent_media_ig_by_post(query:str, token:str, download_path:Optional[str]=None):
    """
    Description
    ----------
    ...
    if found_bs_ac:
        ...
        for post in resent_media_response:
            if post["media_type"] !="IMAGE":
                continue
            shortcode = get_shortcode_from_permalink(
...
def get_hashtag_media_ig(query:str, token:str, hashtag_content:str) -> dict:
    """
    Description
    ----------
    Given a query, this function downloads the images and metadata from IG API.
