...
class AnalysisBox(FeaturesSet):
    ...
    def normality_check(self, features_to_plot: list=[], p_thresh: float = 0.05):
        ...
        for feature in num_features:
            shapiro_test = stats.shapiro(np.array(list(self._feature_dataframe[feature]))).pvalue
    ...
    def calculate_basic_stats(self, volume_feature: str=''):
        ...
        p_shapiro_test = []
        for feature in num_features:
            p_shapiro_test.append(stats.shapiro(np.array(list(self._feature_dataframe[feature]))).pvalue)
        stats_dataframe = pd.concat([stats_dataframe,
