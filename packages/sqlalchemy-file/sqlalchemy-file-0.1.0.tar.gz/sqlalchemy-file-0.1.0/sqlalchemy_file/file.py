...
class File(BaseFile):
    ...
    using libcloud while saving around most file information.
    ...
    Default attributes provided for all ``File`` include:
        ...
        - path              - This is a upload_storage/file_id path which can
                             be used with :meth:`StorageManager.get_file` to retrieve the file
        - content_type      - This is the content type of the uploaded file
        - uploaded_at       - This is the upload date in ISO format
        - url               - Public url of the uploaded file provided by libcloud
                                meth:`Object.get_cdn_url`
        - file              - Only available for saved content, internally call
                              :meth:`StorageManager.get_file` on path and return
                              an instance of NamedTemporaryFile
    """
