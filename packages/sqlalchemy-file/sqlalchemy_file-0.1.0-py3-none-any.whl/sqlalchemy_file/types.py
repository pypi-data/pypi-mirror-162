...
class FileField(types.TypeDecorator):  # type: ignore
    ...
    ``FileField`` can be used as a Column type to store files
    into the model. The actual file itself will be uploaded to a specific
     :class:`libcloud.storage.base.Container`, and only the :class:`File`
    information will be stored on the database as JSON.

    The ``FileField`` is transaction aware, so it will delete
    ...
    associate storage :class:`libcloud.storage.base.Container` and can go through different
    processors (ex: :class:`ThumbnailGenerator`) before being saved in the database.

    Set multiple=True to save multiple files.
...
class ImageField(FileField):
    """Add ImageValidator to FileField"""
