...
def chol_sample_1D(
    ...
    Args:
        ...
        seed: Optional integer seed passed to `numpy.random.default_rng()`
    ...
    X = rng.normal(loc=0.0, scale=1.0, size=(size, N))
    S = rng.normal(loc=0.0, scale=std_noise, size=(size, N))
...
def kron_sample_2D(
    ...
    # Sample from std. normal and Gaussian with vector noise
    rng = np.random.default_rng(seed)
    X = rng.normal(loc=0.0, scale=1.0, size=(size, Nx * Nt))
    S = rng.normal(loc=0.0, scale=std_noise, size=(size, Nx * Nt))
...
def kron_sample_ND(
    ...
    # Sample from std. normal and Gaussian with vector noise
    rng = np.random.default_rng(seed)
    X = rng.normal(loc=0.0, scale=1.0, size=(size, Npts))
    S = rng.normal(loc=0.0, scale=std_noise, size=(size, Npts))
