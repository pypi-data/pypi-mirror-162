...
def _napi_threads(dev):
    # type: (str) -> list[int]
    output = _exec(
        """ps -aef | grep "\[napi\/%s" | awk '{print $2 " "}'""" % dev, 
        shell=True
    )
...
class Config(object):
        ...
        for name, section in self:
            if name != 'globals':
                if (section.addrs is not None and len(section.addrs)) \
                    ...
                    return True
                if (section.remote_addrs is not None \
                        and len(section.remote_addrs)) \
        ...
        for name, section in self:
            if name != 'globals':
                if section.mode == 'exclusive' \
                        and self.settings.flow_director is None:
                    devlink = Devlink(self.globals.dev, self.log)
                ...
                for port in section.ports:
                    if section.addrs is not None and len(section.addrs):
                        for addr in section.addrs:
                            ...
                            if self.globals.priority \
                                    and self.globals.priority == 'skbedit':
                                tc.filter_add(
    ...
    def _set_affinity(self): # type () -> None
        ...
        if threaded:
            ...
            if not os.path.exists("/sys/class/net/%s/threaded" % dev):
                raise Exception(
                    "the current driver does not support napi threads")
            ...
            if name != 'globals' and section.pollers > 0:
                try:
                    ...
                    devlink.param('tc%d_poller_timeout' 
                        % tc_idx, int(section.poller_timeout))
                except:
                    raise Exception(
    ...
    def apply(self): # type(bool) -> None
        ...
        self.settings.apply()
        self._set_tcs()
        if sum([section.pollers if name != 'globals' \
                else 0 for name, section in self]):
            self._set_pollers()
        self._set_options()
