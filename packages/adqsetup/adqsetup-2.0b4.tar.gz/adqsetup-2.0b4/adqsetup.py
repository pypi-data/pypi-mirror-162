...
if sys.version_info.major == 3:
    ...
    def sched_setaffinity(pid, cpus): # type: (int, list[int]) -> None
        cpus = set(cpus)
        mask = create_string_buffer(_mask(cpus, 1024))  ## 128 byte mutable buffer
        if libc.sched_setaffinity(c_uint32(pid), c_size_t(len(mask)), byref(mask)):
...
def _irqs(dev):
    ...
    output = _exec("grep -i 'ice-%s-TxRx-' /proc/interrupts | cut -f1 -d:" % dev, shell=True)
    return [int(t) for t in output.split()]

def _napi_threads(dev):
    # type: (str) -> list[int]
    output = _exec("""ps -aef | grep "\[napi\/%s" | awk '{print $2 " "}'""" % dev, shell=True)
    return list(reversed([int(t) for t in output.split()]))
    ...
    def _validate(self, inv): # type: (Inventory) -> None
        ...
        if isinstance(self.cpus, list):
            if len(self.cpus) != self.queues:
                raise Exception("[globals] the number of cpus must be equal to queues")
...
class ConfigSection(ConfigBase):
    ...
    def __init__(self, name="", source=None): # type (str, dict) -> None
        '''
...
class Config(object):
    ...
    def _assign_auto_cpus(self, inv):
    ...
    def _set_affinity(self): # type () -> None
        ...
        if threaded:
            ...
            if not os.path.exists("/sys/class/net/%s/threaded" % dev):
                raise Exception("the current driver does not support napi threads")
    ...
    def _set_pollers(self): # type () -> None
        ...
        for name, section in self:
            if name != 'globals' and section.pollers > 0:
                try:
                    devlink = Devlink(self.globals.dev, self.log)
                    devlink.param('tc%d_qps_per_poller' % tc_idx, section._qpp)
