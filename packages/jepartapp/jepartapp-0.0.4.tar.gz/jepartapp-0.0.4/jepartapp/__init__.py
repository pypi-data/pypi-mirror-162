...
class AlegriaDeBrincar:
    ...
    def configAPP(self):
        ...
        if "config.json" not in check:
            json = '{ "idLoja": 0, "cacheDay": 0 "firebird": { "dbLoja": "", "dbUser": "", "dbPass": "" }, "mysql": { "sqlIp": "", "sqluser": "", "sqlPass": "", "sqlDb": "" } }'
    ...
    def selectFDB(self,string):
        db = self.c["firebird"]
        con = fdb.connect(database=db["dbLoja"], user=db["dbUser"], password= db["dbPass"])
        cur = con.cursor()
        cur.execute(string)
        result = cur.fetchall()
        return result
    ...
    def insertDay(self):
        ...
        for i in temp:
            ...
            count += 1
            if count == 30 or i == len(temp)-1:
                AlegriaDeBrincar.insertDB(sql+stringInsert)
                stringInsert = ""
                count = 0
