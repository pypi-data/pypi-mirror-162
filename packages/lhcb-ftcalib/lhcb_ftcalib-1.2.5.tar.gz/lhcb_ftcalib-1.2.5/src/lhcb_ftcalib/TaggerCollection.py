...
class TaggerCollection:
    ...
    def _prepare_combination(self, name, calibrated, next_selection=None, tagger_subset=None, append=False):
        ...
        if calibrated and constants.propagate_errors:
            ...
            etas = np.array([ np.array(tagger.stats._full_data.eta) for tagger, cal in zip(self._taggers, indeed_calibrated) ]).T
            npars = np.array([tagger.func.npar for tagger in self._taggers])
            if constants.ignore_mistag_asymmetry_for_apply:
                vec_gradients = np.array([tagger.func.gradient_averaged(tagger.stats.params.params_average, etas[:, i]) for i, tagger in enumerate(self._taggers)], dtype=object)
                covariances = block_diag(*[t.stats.params.covariance_average for t in self._taggers])
            else:
                npars *= 2
                vec_gradients = np.array([tagger.func.gradient(tagger.stats.params.params_flavour, etas[:, i], decs[:, i]) for i, tagger in enumerate(self._taggers)], dtype=object)
                covariances = block_diag(*[t.stats.params.covariance_flavour for t in self._taggers])
            gradients = np.empty((len(decs), len(self._taggers), np.sum(npars)))
            gradients = compute_gradient(npars, vec_gradients, ntaggers=len(self._taggers), nevents=len(decs))
        elif calibrated:
