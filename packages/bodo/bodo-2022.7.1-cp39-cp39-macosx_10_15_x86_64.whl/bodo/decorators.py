...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        bsx__hasga = self.overloads[sig]
        ushp__bmr = bsx__hasga.metadata.get('distributed_diagnostics', None)
        if ushp__bmr is None:
            fbh__rnoz = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(fbh__rnoz)
        ushp__bmr.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        hyqqh__xhb = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(hyqqh__xhb)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        wyvr__mna = options.pop('distributed')
        pipeline_class = (pipeline_class if wyvr__mna else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        tifz__vgpua = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if tifz__vgpua else
            pipeline_class)
    ciwb__nodvx = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(ciwb__nodvx, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(ciwb__nodvx)
        else:
            return master_mode_wrapper(ciwb__nodvx)
    else:
        return ciwb__nodvx
