...
def box_regex_flag(typ, val, c):
    hsypq__xwwx = c.box(types.int64, val)
    bedkp__nie = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(bedkp__nie, (hsypq__xwwx,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    hsypq__xwwx = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, hsypq__xwwx)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    bzv__fibki = context.get_python_api(builder)
    ahvc__zgtv = builder.icmp_signed('==', val, bzv__fibki.borrow_none())
    with builder.if_then(ahvc__zgtv):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    scax__pltf = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(scax__pltf)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        padgu__rdfqg = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if padgu__rdfqg > 1:
            typ = types.List(types.Tuple([string_type] * padgu__rdfqg))
...
def overload_match_group(m, *args):
    ...
    jnl__rrqa = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            wfn__zxgo = args[0]
            with numba.objmode(out=jnl__rrqa):
                out = m.group(wfn__zxgo)
            return out
        return _match_group_impl_one
    ingd__werex = 'tuple_str_{}'.format(len(args))
    setattr(types, ingd__werex, types.Tuple([jnl__rrqa] * len(args)))
    wads__iyt = ', '.join('group{}'.format(jxgk__yszt + 1) for jxgk__yszt in
        range(len(args)))
    temri__bwumn = 'def _match_group_impl(m, *args):\n'
    temri__bwumn += '  ({}) = args\n'.format(wads__iyt)
    temri__bwumn += "  with numba.objmode(out='{}'):\n".format(ingd__werex)
    temri__bwumn += '    out = m.group({})\n'.format(wads__iyt)
    temri__bwumn += '  return out\n'
    wvyut__bqz = {}
    exec(temri__bwumn, globals(), wvyut__bqz)
    zuchg__ymgi = wvyut__bqz['_match_group_impl']
    return zuchg__ymgi
...
def overload_match_groups(m, default=None):
    wbf__eehmi = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=wbf__eehmi):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(lditw__txi is None for lditw__txi in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=jnl__rrqa):
