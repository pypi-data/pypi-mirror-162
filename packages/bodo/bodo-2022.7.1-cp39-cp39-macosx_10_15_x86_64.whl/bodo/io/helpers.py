...
def lower_pyarrow_table_schema(context, builder, ty, pyval):
    ylwt__xhyug = context.get_python_api(builder)
    return ylwt__xhyug.unserialize(ylwt__xhyug.serialize_object(pyval))
...
def get_arrow_timestamp_type(pa_ts_typ):
    emg__apu = 'ns', 'us', 'ms', 's'
    if pa_ts_typ.unit not in emg__apu:
        return types.Array(bodo.datetime64ns, 1, 'C'), False
    elif pa_ts_typ.tz is not None:
        zpwk__bven = pa_ts_typ.to_pandas_dtype().tz
        qwb__yicg = bodo.libs.pd_datetime_arr_ext.get_pytz_type_info(zpwk__bven
            )
        return bodo.DatetimeArrayType(qwb__yicg), True
    else:
...
def _get_numba_typ_from_pa_typ(pa_typ: pa.Field, is_index,
    ...
    if isinstance(pa_typ.type, pa.ListType):
        hasfq__thcsi, pgknr__whjje = _get_numba_typ_from_pa_typ(pa_typ.type
            .value_field, is_index, nullable_from_metadata, category_info)
        return ArrayItemArrayType(hasfq__thcsi), pgknr__whjje
    if isinstance(pa_typ.type, pa.StructType):
        ihz__xmr = []
        jqn__bfxb = []
        pgknr__whjje = True
        for smzbd__wjf in pa_typ.flatten():
            jqn__bfxb.append(smzbd__wjf.name.split('.')[-1])
            wer__esmkc, rfv__piy = _get_numba_typ_from_pa_typ(smzbd__wjf,
                is_index, nullable_from_metadata, category_info)
            ihz__xmr.append(wer__esmkc)
            pgknr__whjje = pgknr__whjje and rfv__piy
        return StructArrayType(tuple(ihz__xmr), tuple(jqn__bfxb)), pgknr__whjje
    ...
    if isinstance(pa_typ.type, pa.DictionaryType):
        ...
        hze__szo = _pyarrow_numba_type_map[pa_typ.type.index_type]
        fwfvi__hqlo = PDCategoricalDtype(category_info[pa_typ.name], bodo.
            string_type, pa_typ.type.ordered, int_type=hze__szo)
        return CategoricalArrayType(fwfvi__hqlo), True
    if isinstance(pa_typ.type, pa.lib.TimestampType):
        ...
        hwvv__zfhbz = _pyarrow_numba_type_map[pa_typ.type]
        pgknr__whjje = True
    else:
        raise BodoError('Arrow data type {} not supported yet'.format(
            pa_typ.type))
    if hwvv__zfhbz == datetime_date_type:
        return datetime_date_array_type, pgknr__whjje
    if hwvv__zfhbz == bytes_type:
        return binary_array_type, pgknr__whjje
    hasfq__thcsi = (string_array_type if hwvv__zfhbz == string_type else
        types.Array(hwvv__zfhbz, 1, 'C'))
    if hwvv__zfhbz == types.bool_:
        hasfq__thcsi = boolean_array
    xrg__dfaj = (use_nullable_int_arr if nullable_from_metadata is None else
        nullable_from_metadata)
    if xrg__dfaj and not is_index and isinstance(hwvv__zfhbz, types.Integer
        ) and pa_typ.nullable:
        hasfq__thcsi = IntegerArrayType(hwvv__zfhbz)
    return hasfq__thcsi, pgknr__whjje
