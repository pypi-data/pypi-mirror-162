...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        bwwaw__latcj = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(bwwaw__latcj)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        qsmlu__vel = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, qsmlu__vel)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        hlqvd__omv, = args
        jtx__ajjx = signature.return_type
        mnrd__hke = cgutils.create_struct_proxy(jtx__ajjx)(context, builder)
        mnrd__hke.obj = hlqvd__omv
        context.nrt.incref(builder, signature.args[0], hlqvd__omv)
        return mnrd__hke._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                nos__ozmok = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        bwwaw__latcj = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(bwwaw__latcj)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        qsmlu__vel = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, qsmlu__vel)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        hlqvd__omv, = args
        kwqbp__tvrzg = signature.return_type
        wllwz__deb = cgutils.create_struct_proxy(kwqbp__tvrzg)(context, builder
            )
        wllwz__deb.obj = hlqvd__omv
        context.nrt.incref(builder, signature.args[0], hlqvd__omv)
        return wllwz__deb._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                ylgzt__qenh = bodo.utils.conversion.coerce_to_ndarray(idx)
                vyz__ptjd = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    ylgzt__qenh]
                mrs__sukt = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    ylgzt__qenh]
                bwwaw__latcj = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(vyz__ptjd,
                    mrs__sukt, bwwaw__latcj)
            return impl
        if isinstance(idx, types.SliceType):

            def impl_slice(I, idx):
                S = I._obj
                vyz__ptjd = bodo.hiframes.pd_series_ext.get_series_data(S)[idx]
                mrs__sukt = bodo.hiframes.pd_series_ext.get_series_index(S)[idx
                    ]
                bwwaw__latcj = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(vyz__ptjd,
                    mrs__sukt, bwwaw__latcj)
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            if is_scalar_type(val):
                if I.stype.dtype == types.NPDatetime('ns'
                    ...
                    def impl_dt(I, idx, val):
                        s = integer_to_dt64(val.value)
                        ylgzt__qenh = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        nos__ozmok = (bodo.hiframes.datetime_timedelta_ext.
                            _to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(nos__ozmok))
                        ylgzt__qenh = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            ylgzt__qenh] = s
            ...
            def impl_arr(I, idx, val):
                ylgzt__qenh = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[ylgzt__qenh
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        bwwaw__latcj = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(bwwaw__latcj)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        qsmlu__vel = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, qsmlu__vel)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        hlqvd__omv, = args
        awrh__rvdbj = signature.return_type
        whpr__qidg = cgutils.create_struct_proxy(awrh__rvdbj)(context, builder)
        whpr__qidg.obj = hlqvd__omv
        context.nrt.incref(builder, signature.args[0], hlqvd__omv)
        return whpr__qidg._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            vyz__ptjd = bodo.hiframes.pd_series_ext.get_series_data(S)
            mrs__sukt = bodo.hiframes.pd_series_ext.get_series_index(S)
            lhh__tonmi = (idx - mrs__sukt._start) // mrs__sukt._step
            return vyz__ptjd[lhh__tonmi]
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    ...
                    ylgzt__qenh = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(vyz__ptjd[
                        ylgzt__qenh])
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    tzp__ipftt = bodo.hiframes.pd_series_ext.get_series_index(S
                        )
                    hybvk__mpxe = tzp__ipftt.get_loc(idx)
                    if hybvk__mpxe is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(hybvk__mpxe)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                ylgzt__qenh = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[ylgzt__qenh
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        lhj__jtz = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(esf__lcfor,
            int) for esf__lcfor in lhj__jtz):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            pggm__mequo = get_literal_value(idx)
            if pggm__mequo not in lhj__jtz:
                raise_bodo_error(
                    f"Series label-based getitem: '{pggm__mequo}' not in {lhj__jtz}"
                    )
            arr_ind = lhj__jtz.index(pggm__mequo)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    mnrd__hke = cgutils.create_struct_proxy(fromty)(context, builder, val)
    lhi__lmbuc = context.cast(builder, mnrd__hke.obj, fromty.stype, toty.stype)
    hjx__hwvfv = cgutils.create_struct_proxy(toty)(context, builder)
    hjx__hwvfv.obj = lhi__lmbuc
    return hjx__hwvfv._getvalue()
