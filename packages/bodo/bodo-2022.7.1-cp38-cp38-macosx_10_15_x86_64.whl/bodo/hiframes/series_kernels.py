...
def _column_filter_impl(B, ind):
    lzstt__yhv = bodo.hiframes.rolling.alloc_shift(len(B), B, (-1,))
    for dmi__offb in numba.parfors.parfor.internal_prange(len(lzstt__yhv)):
        if ind[dmi__offb]:
            lzstt__yhv[dmi__offb] = B[dmi__offb]
        else:
            bodo.libs.array_kernels.setna(lzstt__yhv, dmi__offb)
    return lzstt__yhv
...
def _series_dropna_str_alloc_impl_inner(B):
    ...
    gyzmu__wpe = len(B)
    aeclk__jbwlt = 0
    for dmi__offb in range(len(B)):
        if bodo.libs.str_arr_ext.str_arr_is_na(B, dmi__offb):
            aeclk__jbwlt += 1
    kgfep__trzvb = gyzmu__wpe - aeclk__jbwlt
    mvjx__kgkx = bodo.libs.str_arr_ext.num_total_chars(B)
    lzstt__yhv = bodo.libs.str_arr_ext.pre_alloc_string_array(kgfep__trzvb,
        mvjx__kgkx)
    bodo.libs.str_arr_ext.copy_non_null_offsets(lzstt__yhv, B)
    bodo.libs.str_arr_ext.copy_data(lzstt__yhv, B)
    bodo.libs.str_arr_ext.set_null_bits_to_value(lzstt__yhv, -1)
    return lzstt__yhv
...
def compute_skew(first_moment, second_moment, third_moment, count):
    if count < 3:
        return np.nan
    vyzy__rtopy = first_moment / count
    rqu__qebh = (third_moment - 3 * second_moment * vyzy__rtopy + 2 * count *
        vyzy__rtopy ** 3)
    zicl__dfcze = second_moment - vyzy__rtopy * first_moment
    s = count * (count - 1) ** 1.5 / (count - 2
        ) * rqu__qebh / zicl__dfcze ** 1.5
    s = s / (count - 1)
    return s
...
def compute_kurt(first_moment, second_moment, third_moment, fourth_moment,
    ...
    if count < 4:
        return np.nan
    vyzy__rtopy = first_moment / count
    kfer__tcovk = (fourth_moment - 4 * third_moment * vyzy__rtopy + 6 *
        second_moment * vyzy__rtopy ** 2 - 3 * count * vyzy__rtopy ** 4)
    zyvu__xcbc = second_moment - vyzy__rtopy * first_moment
    koffw__tfful = 3 * (count - 1) ** 2 / ((count - 2) * (count - 3))
    dmbkk__inz = count * (count + 1) * (count - 1) * kfer__tcovk
    tnppa__unlmy = (count - 2) * (count - 3) * zyvu__xcbc ** 2
    s = (count - 1) * (dmbkk__inz / tnppa__unlmy - koffw__tfful)
    s = s / (count - 1)
    return s
