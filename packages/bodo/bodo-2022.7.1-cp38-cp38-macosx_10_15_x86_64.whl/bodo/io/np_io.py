...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        spdsc__yhae = context.get_abi_sizeof(context.get_data_type(dtype.dtype)
            )
        return context.get_constant(types.intp, spdsc__yhae)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            gujga__ylff = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(gujga__ylff.dtype)
            file_write(unicode_to_utf8(fname), gujga__ylff.ctypes, 
                dtype_size * gujga__ylff.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            gujga__ylff = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(gujga__ylff.dtype)
            wqgr__cbtr = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                gujga__ylff.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), gujga__ylff.ctypes,
                start, count, wqgr__cbtr)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            knpex__wfar = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                knpex__wfar = min(knpex__wfar, count * dtype_size)
            if knpex__wfar < 0:
                return -1
            return knpex__wfar
        return impl
