...
def _global_except_hook(exctype, value, traceback):
    ...
    hosn__ghpvm = MPI.COMM_WORLD
    eimn__khknf = hosn__ghpvm.Get_rank()
    mzs__dwhy = 3.0
    dpue__pcd = True
    oyg__ety = hosn__ghpvm.Ibarrier()
    sjbda__hbbhv = time.time()
    while time.time() - sjbda__hbbhv < mzs__dwhy:
        time.sleep(0.1)
        if oyg__ety.Test():
            dpue__pcd = False
            break
    try:
        ...
        if dpue__pcd:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {eimn__khknf}. \n')
