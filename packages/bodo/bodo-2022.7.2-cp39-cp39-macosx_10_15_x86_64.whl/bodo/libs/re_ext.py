...
def box_regex_flag(typ, val, c):
    unjx__rjvae = c.box(types.int64, val)
    vmf__crdv = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(vmf__crdv, (unjx__rjvae,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    unjx__rjvae = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, unjx__rjvae)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    fra__eto = context.get_python_api(builder)
    fbrl__gajc = builder.icmp_signed('==', val, fra__eto.borrow_none())
    with builder.if_then(fbrl__gajc):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    jdav__xft = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(jdav__xft)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        crl__lkbrg = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if crl__lkbrg > 1:
            typ = types.List(types.Tuple([string_type] * crl__lkbrg))
...
def overload_match_group(m, *args):
    ...
    aaao__arl = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            ccms__qgvvp = args[0]
            with numba.objmode(out=aaao__arl):
                out = m.group(ccms__qgvvp)
            return out
        return _match_group_impl_one
    cpvbm__tclz = 'tuple_str_{}'.format(len(args))
    setattr(types, cpvbm__tclz, types.Tuple([aaao__arl] * len(args)))
    ttyj__puqa = ', '.join('group{}'.format(hlumh__fyon + 1) for
        hlumh__fyon in range(len(args)))
    tfx__dlqx = 'def _match_group_impl(m, *args):\n'
    tfx__dlqx += '  ({}) = args\n'.format(ttyj__puqa)
    tfx__dlqx += "  with numba.objmode(out='{}'):\n".format(cpvbm__tclz)
    tfx__dlqx += '    out = m.group({})\n'.format(ttyj__puqa)
    tfx__dlqx += '  return out\n'
    hhqls__qplk = {}
    exec(tfx__dlqx, globals(), hhqls__qplk)
    qvh__sebp = hhqls__qplk['_match_group_impl']
    return qvh__sebp
...
def overload_match_groups(m, default=None):
    wlf__rfmn = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=wlf__rfmn):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(lsicp__ymx is None for lsicp__ymx in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=aaao__arl):
