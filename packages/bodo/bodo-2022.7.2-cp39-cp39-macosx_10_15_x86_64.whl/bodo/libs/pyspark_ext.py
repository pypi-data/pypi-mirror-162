...
def box_spark_session(typ, val, c):
    ejiwa__ualx = c.context.insert_const_string(c.builder.module, 'pyspark')
    vma__wdjcu = c.pyapi.import_module_noblock(ejiwa__ualx)
    zfh__crbsl = c.pyapi.object_getattr_string(vma__wdjcu, 'sql')
    rls__skk = c.pyapi.object_getattr_string(zfh__crbsl, 'SparkSession')
    oxw__gdse = c.pyapi.object_getattr_string(rls__skk, 'builder')
    vrki__ysiv = c.pyapi.call_method(oxw__gdse, 'getOrCreate', ())
    c.pyapi.decref(vma__wdjcu)
    c.pyapi.decref(zfh__crbsl)
    c.pyapi.decref(rls__skk)
    c.pyapi.decref(oxw__gdse)
    return vrki__ysiv
...
class RowType(types.BaseNamedTuple):
    ...
    def __init__(self, types, fields):
        ...
        xkht__cgnm = 'Row({})'.format(', '.join(
            f'{okwe__wdugl}:{ypans__flr}' for okwe__wdugl, ypans__flr in
            zip(self.fields, self.types)))
        super(RowType, self).__init__(xkht__cgnm)
...
class RowModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        qznf__usc = [(okwe__wdugl, ypans__flr) for okwe__wdugl, ypans__flr in
            zip(fe_type.fields, fe_type.types)]
        super(RowModel, self).__init__(dmm, fe_type, qznf__usc)
...
def typeof_row(val, c):
    ...
    return RowType(tuple(numba.typeof(ycb__lvf) for ycb__lvf in val), fields)


@box(RowType)
def box_row(typ, val, c):
    vtjtf__pkawy = c.pyapi.unserialize(c.pyapi.serialize_object(pyspark.sql
        .types.Row))
    if all(okwe__wdugl.startswith(ANON_SENTINEL) for okwe__wdugl in typ.fields
        ):
        phn__epmmb = [c.box(ypans__flr, c.builder.extract_value(val, i)) for
            i, ypans__flr in enumerate(typ.types)]
        jot__gvedc = c.pyapi.call_function_objargs(vtjtf__pkawy, phn__epmmb)
        for obj in phn__epmmb:
            c.pyapi.decref(obj)
        c.pyapi.decref(vtjtf__pkawy)
        return jot__gvedc
    args = c.pyapi.tuple_pack([])
    phn__epmmb = []
    saa__zat = []
    for i, ypans__flr in enumerate(typ.types):
        fztf__xttdh = c.builder.extract_value(val, i)
        obj = c.box(ypans__flr, fztf__xttdh)
        saa__zat.append((typ.fields[i], obj))
        phn__epmmb.append(obj)
    kws = c.pyapi.dict_pack(saa__zat)
    jot__gvedc = c.pyapi.call(vtjtf__pkawy, args, kws)
    for obj in phn__epmmb:
        c.pyapi.decref(obj)
    c.pyapi.decref(vtjtf__pkawy)
    c.pyapi.decref(args)
...
class RowConstructor(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        ucg__nnyve = ', '.join(f'arg{i}' for i in range(len(args)))
        ret__zhag = ', '.join(f"{mdy__omk} = ''" for mdy__omk in kws)
        func_text = f'def row_stub({ucg__nnyve}{ret__zhag}):\n'
        func_text += '    pass\n'
        dyvir__etdu = {}
        exec(func_text, {}, dyvir__etdu)
        jof__cvh = dyvir__etdu['row_stub']
        pip__ari = numba.core.utils.pysignature(jof__cvh)
        if args:
            pbka__ueee = RowType(args, tuple(f'{ANON_SENTINEL}{i}' for i in
                range(len(args))))
            return signature(pbka__ueee, *args).replace(pysig=pip__ari)
        kws = dict(kws)
        pbka__ueee = RowType(tuple(kws.values()), tuple(kws.keys()))
        return signature(pbka__ueee, *kws.values()).replace(pysig=pip__ari)
...
class SparkDataFrameModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        qznf__usc = [('df', fe_type.df)]
        super(SparkDataFrameModel, self).__init__(dmm, fe_type, qznf__usc)
...
def overload_create_df(sp_session, data, schema=None, samplingRatio=None,
    ...
    enyl__xmz = data.dtype.fields
    prg__ktt = len(data.dtype.types)
    func_text = (
        'def impl(sp_session, data, schema=None, samplingRatio=None, verifySchema=True):\n'
        )
    func_text += f'  n = len(data)\n'
    cro__azllg = []
    for i, ypans__flr in enumerate(data.dtype.types):
        our__cnj = dtype_to_array_type(ypans__flr)
        func_text += (
            f'  A{i} = bodo.utils.utils.alloc_type(n, arr_typ{i}, (-1,))\n')
        cro__azllg.append(our__cnj)
    func_text += f'  for i in range(n):\n'
    func_text += f'    r = data[i]\n'
    for i in range(prg__ktt):
        func_text += (
            f'    A{i}[i] = bodo.utils.conversion.unbox_if_timestamp(r[{i}])\n'
            )
    hbaf__atjyt = '({}{})'.format(', '.join(f'A{i}' for i in range(prg__ktt
        )), ',' if len(enyl__xmz) == 1 else '')
    func_text += (
        '  index = bodo.hiframes.pd_index_ext.init_range_index(0, n, 1, None)\n'
        )
    func_text += f"""  pdf = bodo.hiframes.pd_dataframe_ext.init_dataframe({hbaf__atjyt}, index, __col_name_meta_value_create_df)
"""
    func_text += f'  pdf = bodo.scatterv(pdf)\n'
    func_text += f'  return bodo.libs.pyspark_ext.init_spark_df(pdf)\n'
    dyvir__etdu = {}
    kczn__ybv = {'bodo': bodo, '__col_name_meta_value_create_df':
        ColNamesMetaType(tuple(enyl__xmz))}
    for i in range(prg__ktt):
        kczn__ybv[f'arr_typ{i}'] = cro__azllg[i]
    exec(func_text, kczn__ybv, dyvir__etdu)
    impl = dyvir__etdu['impl']
    return impl
...
def overload_df_to_rows(df):
    ...
    bwmu__ohdel = ', '.join(f'{c}=A{i}[i]' for i, c in enumerate(df.columns))
    func_text += f'    out.append(Row({bwmu__ohdel}))\n'
    func_text += '  return out\n'
    dyvir__etdu = {}
    kczn__ybv = {'bodo': bodo, 'Row': pyspark.sql.types.Row}
    exec(func_text, kczn__ybv, dyvir__etdu)
    impl = dyvir__etdu['impl']
    return impl
...
def _gen_df_select(spark_df, cols, avoid_stararg=False):
    ...
    for col in cols:
        ...
        data, hjge__cac = _gen_col_code(col, df_type)
        func_text += hjge__cac
...
def _gen_init_spark_df(func_text, out_data, out_col_names):
    hbaf__atjyt = '({}{})'.format(', '.join(out_data), ',' if len(out_data) ==
        1 else '')
    nuocr__doq = '0' if not out_data else f'len({out_data[0]})'
    func_text += f'  n = {nuocr__doq}\n'
    func_text += (
        '  index = bodo.hiframes.pd_index_ext.init_range_index(0, n, 1, None)\n'
        )
    func_text += f"""  pdf = bodo.hiframes.pd_dataframe_ext.init_dataframe({hbaf__atjyt}, index, __col_name_meta_value_init_spark_df)
"""
    func_text += f'  return bodo.libs.pyspark_ext.init_spark_df(pdf)\n'
    dyvir__etdu = {}
    kczn__ybv = {'bodo': bodo, 'np': np,
...
def overload_show(spark_df, n=20, truncate=True, vertical=False):
    hvn__chax = dict(truncate=truncate, vertical=vertical)
    auj__mid = dict(truncate=True, vertical=False)
    check_unsupported_args('SparkDataFrameType.show', hvn__chax, auj__mid)
...
def overload_with_column(spark_df, colName, col):
    ...
    nhpaw__ent = spark_df.df.columns
    ajthr__yncow = nhpaw__ent if col_name in nhpaw__ent else nhpaw__ent + (
        col_name,)
    kjjt__melkr, gkn__qvjme = _gen_col_code(col, spark_df.df)
    out_data = [(kjjt__melkr if c == col_name else
        f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {nhpaw__ent.index(c)})'
        ) for c in ajthr__yncow]
    func_text = 'def impl(spark_df, colName, col):\n'
    func_text += '  df = spark_df._df\n'
    func_text += gkn__qvjme
    return _gen_init_spark_df(func_text, out_data, ajthr__yncow)
...
def overload_with_column_renamed(spark_df, existing, new):
    ...
    xmws__ikoia = get_overload_const_str(existing)
    djdud__rib = get_overload_const_str(new)
    nhpaw__ent = spark_df.df.columns
    ajthr__yncow = tuple(djdud__rib if c == xmws__ikoia else c for c in
        nhpaw__ent)
    out_data = [f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i})'
         for i in range(len(nhpaw__ent))]
...
def overload_dataframe_columns(spark_df):
    iiyix__nlmqb = list(str(mdy__omk) for mdy__omk in spark_df.df.columns)
    func_text = 'def impl(spark_df):\n'
    func_text += f'  return {iiyix__nlmqb}\n'
    dyvir__etdu = {}
    exec(func_text, {}, dyvir__etdu)
    impl = dyvir__etdu['impl']
    return impl
...
def init_col_from_name(typingctx, col=None):
    ...
    wcvj__ijzt = get_overload_const_str(col)
    mdso__voywk = ColumnType(ExprType('col', (wcvj__ijzt,)))

    def codegen(context, builder, signature, args):
        return context.get_constant_null(mdso__voywk)
    return mdso__voywk(col), codegen
...
def init_f_sum(typingctx, col=None):
    mdso__voywk = ColumnType(ExprType('sum', (col.expr,)))
...
def _gen_col_code_expr(expr, df_type):
    ...
    if expr.op == 'sum':
        tnmqs__auf, vujc__awo = _gen_col_code_expr(expr.children[0], df_type)
        i = ir_utils.next_label()
        func_text = f"""  A{i} = np.asarray([bodo.libs.array_ops.array_op_sum({tnmqs__auf}, True, 0)])
"""
        return f'A{i}', vujc__awo + func_text


def _gen_col_code_colname(col_name, df_type):
    bezz__yekb = df_type.columns.index(col_name)
    i = ir_utils.next_label()
    func_text = (
        f'  A{i} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {bezz__yekb})\n'
        )
