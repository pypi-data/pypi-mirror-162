...
def decode_if_dict_array_overload(A):
    if isinstance(A, types.BaseTuple):
        vvbfe__bxh = len(A.types)
        wsh__bkhzu = 'def f(A):\n'
        juwa__rvqe = ','.join(f'decode_if_dict_array(A[{i}])' for i in
            range(vvbfe__bxh))
        wsh__bkhzu += '  return ({}{})\n'.format(juwa__rvqe, ',' if 
            vvbfe__bxh == 1 else '')
        sfxis__koxqg = {}
        exec(wsh__bkhzu, {'decode_if_dict_array': decode_if_dict_array},
            sfxis__koxqg)
        impl = sfxis__koxqg['f']
        return impl
    if isinstance(A, types.List):
        ...
        def impl(A):
            ...
            for a in A:
                vvbfe__bxh += 1
            gemdi__mpcfn = []
            for i in range(vvbfe__bxh):
                gemdi__mpcfn.append(decode_if_dict_array(A[i]))
            return gemdi__mpcfn
        return impl
    ...
    if isinstance(A, bodo.SeriesType):
        ...
        def impl(A):
            flng__rjzw = bodo.hiframes.pd_series_ext.get_series_data(A)
            wezgj__esu = bodo.hiframes.pd_series_ext.get_series_index(A)
            name = bodo.hiframes.pd_series_ext.get_series_name(A)
            ccrl__vpzyt = decode_if_dict_array(flng__rjzw)
            return bodo.hiframes.pd_series_ext.init_series(ccrl__vpzyt,
                wezgj__esu, name)
        return impl
...
def to_str_arr_if_dict_array(t):
    ...
    if isinstance(t, bodo.TableType):
        rac__qke = tuple(to_str_arr_if_dict_array(t) for t in t.arr_types)
        return bodo.TableType(rac__qke, t.has_runtime_cols)
...
class BodoError(NumbaError):
    ...
    def __init__(self, msg, loc=None, locs_in_msg=None):
        ...
        rrvth__snso = numba.core.errors.termcolor().errmsg
        super(BodoError, self).__init__(rrvth__snso(msg), loc)
...
def raise_bodo_error(msg, loc=None):
    if bodo.transforms.typing_pass.in_partial_typing:
        ...
        lsuyx__tbb = [] if loc is None else [loc]
        raise BodoError(msg, locs_in_msg=lsuyx__tbb)
...
class FileInfo:
    ...
    def get_schema(self, fname):
        vifss__mkbds = self.get_full_filename(fname)
        return self._get_schema(vifss__mkbds)
...
class FilenameModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        tteq__gli = dmm.lookup(bodo.typeof(fe_type.fname))
        ogq__syxbd = [(a, xwhx__wefi) for a, xwhx__wefi in zip(tteq__gli.
            _fields, tteq__gli._members)]
        super().__init__(dmm, fe_type, ogq__syxbd)
...
def is_initial_value_type(t):
    if not isinstance(t, types.InitialValue) or t.initial_value is None:
        return False
    gnhvz__qmx = t.initial_value
    if isinstance(gnhvz__qmx, dict):
        gnhvz__qmx = gnhvz__qmx.values()
    return not any(isinstance(ubtep__kvc, (types.Poison, numba.core.
        interpreter._UNKNOWN_VALUE)) for ubtep__kvc in gnhvz__qmx)
...
def is_overload_constant_dict(val):
    return isinstance(val, types.LiteralStrKeyDict) and all(is_literal_type
        (ubtep__kvc) for ubtep__kvc in val.types
...
def is_overload_bool_list(val):
    return is_overload_constant_list(val) and all(is_overload_constant_bool
        (ubtep__kvc) for ubtep__kvc in get_overload_const_list(val))
...
def get_overload_const(val):
    ...
    if isinstance(val, types.LiteralList):
        nuxh__qze = []
        for ubtep__kvc in val.literal_value:
            hpdl__gbjb = get_overload_const(ubtep__kvc)
            if hpdl__gbjb == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                nuxh__qze.append(hpdl__gbjb)
        return nuxh__qze
    ...
    if isinstance(val, types.BaseTuple):
        nuxh__qze = []
        for ubtep__kvc in val.types:
            hpdl__gbjb = get_overload_const(ubtep__kvc)
            if hpdl__gbjb == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                nuxh__qze.append(hpdl__gbjb)
        return tuple(nuxh__qze)
...
def ensure_constant_arg(fname, arg_name, val, const_type):
    hpdl__gbjb = get_overload_const(val)
    uidmg__feprg = _const_type_repr.get(const_type, str(const_type))
    if not isinstance(hpdl__gbjb, const_type):
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant {uidmg__feprg} not {val}"
            )


def ensure_constant_values(fname, arg_name, val, const_values):
    hpdl__gbjb = get_overload_const(val)
    if hpdl__gbjb not in const_values:
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant value in {const_values} not '{hpdl__gbjb}'"
            )
...
def check_unsupported_args(fname, args_dict, arg_defaults_dict,
    ...
    qzjf__hflg = False
    for a in args_dict:
        uzzck__bkcxk = get_overload_const(args_dict[a])
        qmypg__hiys = arg_defaults_dict[a]
        if (uzzck__bkcxk is NOT_CONSTANT or uzzck__bkcxk is not None and 
            qmypg__hiys is None or uzzck__bkcxk is None and qmypg__hiys is not
            None or uzzck__bkcxk != qmypg__hiys or uzzck__bkcxk is not
            _no_input and qmypg__hiys is _no_input or uzzck__bkcxk is
            _no_input and qmypg__hiys is not _no_input):
            rdzj__rufgn = (
                f'{fn_str}: {a} parameter only supports default value {qmypg__hiys}'
                )
            qzjf__hflg = True
            break
    if qzjf__hflg and package_name == 'pandas':
        if module_name == 'IO':
            rdzj__rufgn += """
...
def get_overload_constant_dict(val):
    if isinstance(val, types.LiteralStrKeyDict):
        return {get_literal_value(czfoc__yrmm): get_literal_value(
            ubtep__kvc) for czfoc__yrmm, ubtep__kvc in val.literal_value.
            items()}
    ...
    if isinstance(val, types.DictType):
        ...
        return val.initial_value
    ycvl__jgs = [get_overload_const(ubtep__kvc) for ubtep__kvc in val.types[1:]
        ]
    return {ycvl__jgs[2 * i]: ycvl__jgs[2 * i + 1] for i in range(len(
        ycvl__jgs) // 2)}
...
def get_overload_const_list(val):
    ...
    if is_initial_value_list_type(val):
        return val.initial_value
    if isinstance(val, types.LiteralList):
        return [get_literal_value(ubtep__kvc) for ubtep__kvc in val.
            literal_value]
...
def parse_dtype(dtype, func_name=None):
    ...
    if isinstance(dtype, types.Number) or dtype == bodo.string_type:
        return dtype
    try:
        szvg__szs = get_overload_const_str(dtype)
        if szvg__szs.startswith('Int') or szvg__szs.startswith('UInt'):
            return bodo.libs.int_arr_ext.typeof_pd_int_dtype(pd.api.types.
                pandas_dtype(szvg__szs), None)
        if szvg__szs == 'boolean':
            return bodo.libs.bool_arr_ext.boolean_dtype
        if szvg__szs == 'str':
            return bodo.string_type
        return numba.np.numpy_support.from_dtype(np.dtype(szvg__szs))
    except:
        pass
...
def get_index_names(t, func_name, default_name):
    ...
    wine__jdi = '{}: index name should be a constant string'.format(func_name)
    if isinstance(t, MultiIndexType):
        ztyud__wzwi = []
        for i, xuvla__sbnlv in enumerate(t.names_typ):
            if xuvla__sbnlv == types.none:
                ztyud__wzwi.append('level_{}'.format(i))
                continue
            if not is_overload_constant_str(xuvla__sbnlv):
                raise BodoError(wine__jdi)
            ztyud__wzwi.append(get_overload_const_str(xuvla__sbnlv))
        return tuple(ztyud__wzwi)
...
if types.SliceLiteral in numba.core.pythonapi._boxers.functions:
    ...
    def box_slice_literal(typ, val, c):
        kfr__corke = typ.literal_value
        qxe__dagn = []
        for xrr__pho in ('start', 'stop', 'step'):
            msd__chfs = getattr(typ.literal_value, xrr__pho)
            jmh__rfot = c.pyapi.make_none(
                ) if msd__chfs is None else c.pyapi.from_native_value(types
                .literal(msd__chfs), msd__chfs, c.env_manager)
            qxe__dagn.append(jmh__rfot)
        rjral__pbaq = c.pyapi.unserialize(c.pyapi.serialize_object(slice))
        bav__nurwy = c.pyapi.call_function_objargs(rjral__pbaq, qxe__dagn)
        for a in qxe__dagn:
            c.pyapi.decref(a)
        c.pyapi.decref(rjral__pbaq)
        return bav__nurwy
...
def box_list_literal(typ, val, c):
    djpkh__bokdh = typ.literal_value
    isda__gwnw = [c.pyapi.from_native_value(types.literal(ubtep__kvc),
        ubtep__kvc, c.env_manager) for ubtep__kvc in djpkh__bokdh]
    iqiq__oeesg = c.pyapi.list_pack(isda__gwnw)
    for a in isda__gwnw:
        c.pyapi.decref(a)
    return iqiq__oeesg
...
def list_literal_to_list(context, builder, fromty, toty, val):
    ...
    cfn__jvfo = types.List(toty.dtype)
...
def is_literal_type(t):
    ...
    return isinstance(t, (types.Literal, types.Omitted)) and not isinstance(t,
        ...
        is_literal_type(ubtep__kvc) for ubtep__kvc in t.types
...
def get_literal_value(t):
    ...
    if isinstance(t, types.Literal):
        if isinstance(t, types.LiteralStrKeyDict):
            return {get_literal_value(czfoc__yrmm): get_literal_value(
                ubtep__kvc) for czfoc__yrmm, ubtep__kvc in t.literal_value.
                items()}
        if isinstance(t, types.LiteralList):
            return [get_literal_value(ubtep__kvc) for ubtep__kvc in t.
                literal_value]
        return t.literal_value
    if isinstance(t, types.Omitted):
        return t.value
    if isinstance(t, types.BaseTuple):
        return tuple(get_literal_value(ubtep__kvc) for ubtep__kvc in t.types)
...
def dtype_to_array_type(dtype):
    ...
    if isinstance(dtype, (types.Number, types.Boolean, types.NPDatetime,
        types.NPTimedelta)):
        flng__rjzw = types.Array(dtype, 1, 'C')
        if vwx__ltxpp:
            return to_nullable_type(flng__rjzw)
        return flng__rjzw
...
def types_equality_exists(t1, t2):
    hqpu__hlkdx = numba.core.registry.cpu_target.typing_context
    try:
        hqpu__hlkdx.resolve_function_type(operator.eq, (t1, t2), {})
        return True
    except:
        return False
...
def is_hashable_type(t):
    whitelist_types = (types.UnicodeType, types.StringLiteral, types.
        ...
        PandasTimestampType)
    lliy__oqaj = (types.bool_, bodo.datetime64ns, bodo.timedelta64ns, bodo.
        pd_timedelta_type)
    if isinstance(t, whitelist_types) or t in lliy__oqaj:
        return True
    hqpu__hlkdx = numba.core.registry.cpu_target.typing_context
    try:
        hqpu__hlkdx.resolve_function_type(hash, (t,), {})
        return True
    except:
        return False
...
def to_nullable_type(t):
    ...
    if isinstance(t, DataFrameType):
        oez__qqbzi = tuple(to_nullable_type(t) for t in t.data)
        return DataFrameType(oez__qqbzi, t.index, t.columns, t.dist, t.
            is_table_format)
...
def is_common_scalar_dtype(scalar_types):
    puq__plm, mlywl__ikut = get_common_scalar_dtype(scalar_types)
    return mlywl__ikut
...
def get_common_scalar_dtype(scalar_types):
    ...
    try:
        syn__eojup = np.find_common_type([numba.np.numpy_support.as_dtype(t
            ) for t in scalar_types], [])
        if syn__eojup != object:
            return numba.np.numpy_support.from_dtype(syn__eojup), True
    except numba.core.errors.NumbaNotImplementedError as zgam__arcdk:
        pass
    ...
    sqlu__egj = itertools.groupby(scalar_types)
    if next(sqlu__egj, True) and not next(sqlu__egj, False):
...
def get_nullable_and_non_nullable_types(array_of_types):
    ...
    for typ in array_of_types:
        if typ == bodo.libs.bool_arr_ext.boolean_array:
            lygih__izkny.append(types.Array(types.bool_, 1, 'C'))
        elif isinstance(typ, bodo.libs.int_arr_ext.IntegerArrayType):
            lygih__izkny.append(types.Array(typ.dtype, 1, 'C'))
        elif isinstance(typ, types.Array):
            if typ.dtype == types.bool_:
                lygih__izkny.append(bodo.libs.bool_arr_ext.boolean_array)
            if isinstance(typ.dtype, types.Integer):
                lygih__izkny.append(bodo.libs.int_arr_ext.IntegerArrayType(
                    typ.dtype))
        lygih__izkny.append(typ)
    return lygih__izkny
...
def _gen_objmode_overload(func, output_type, attr_name=None, is_function=
    ...
    if is_function:
        xvnxd__rmd = getfullargspec(func)
        assert xvnxd__rmd.varargs is None, 'varargs not supported'
        assert xvnxd__rmd.varkw is None, 'varkw not supported'
        defaults = [] if xvnxd__rmd.defaults is None else xvnxd__rmd.defaults
        pik__sgzeb = len(xvnxd__rmd.args) - len(defaults)
        args = xvnxd__rmd.args[1:] if attr_name else xvnxd__rmd.args[:]
        scab__ivr = []
        for i, aprpq__kiv in enumerate(xvnxd__rmd.args):
            if i < pik__sgzeb:
                scab__ivr.append(aprpq__kiv)
            elif str(defaults[i - pik__sgzeb]) != '<deprecated parameter>':
                scab__ivr.append(aprpq__kiv + '=' + str(defaults[i -
                    pik__sgzeb]))
            else:
                args.remove(aprpq__kiv)
        if xvnxd__rmd.kwonlyargs is not None:
            for aprpq__kiv in xvnxd__rmd.kwonlyargs:
                args.append(f'{aprpq__kiv}={aprpq__kiv}')
                scab__ivr.append(
                    f'{aprpq__kiv}={str(xvnxd__rmd.kwonlydefaults[aprpq__kiv])}'
                    )
        sig = ', '.join(scab__ivr)
        args = ', '.join(args)
    else:
    ...
    dyp__atqi = f'self.{attr_name}' if attr_name else f'{func_name}'
    wsh__bkhzu = f'def overload_impl({sig}):\n'
    wsh__bkhzu += f'    def impl({sig}):\n'
    if single_rank:
        wsh__bkhzu += f'        if bodo.get_rank() == 0:\n'
        wtxox__szaq = '    '
    else:
        wtxox__szaq = ''
    wsh__bkhzu += (
        f"        {wtxox__szaq}with numba.objmode(res='{type_name}'):\n")
    if is_function:
        wsh__bkhzu += f'            {wtxox__szaq}res = {dyp__atqi}({args})\n'
    else:
        wsh__bkhzu += f'            {wtxox__szaq}res = {dyp__atqi}\n'
    wsh__bkhzu += f'        return res\n'
    wsh__bkhzu += f'    return impl\n'
    sfxis__koxqg = {}
    sxph__srsx = globals()
    if not attr_name:
        sxph__srsx[func_name] = func
    exec(wsh__bkhzu, sxph__srsx, sfxis__koxqg)
    skdj__fmbs = sfxis__koxqg['overload_impl']
    return skdj__fmbs


def gen_objmode_func_overload(func, output_type=None, single_rank=False):
    try:
        skdj__fmbs = _gen_objmode_overload(func, output_type, is_function=
            True, single_rank=single_rank)
        overload(func, no_unliteral=True)(skdj__fmbs)
    except Exception as zgam__arcdk:
        pass
...
def gen_objmode_method_overload(obj_type, method_name, method, output_type=
    ...
    try:
        skdj__fmbs = _gen_objmode_overload(method, output_type, method_name,
            True, single_rank)
        overload_method(obj_type, method_name, no_unliteral=True)(skdj__fmbs)
    except Exception as zgam__arcdk:
        pass
...
def gen_objmode_attr_overload(obj_type, attr_name, attr, output_type=None,
    single_rank=False):
    try:
        skdj__fmbs = _gen_objmode_overload(attr, output_type, attr_name, 
            False, single_rank)
        overload_attribute(obj_type, attr_name, no_unliteral=True)(skdj__fmbs)
    except Exception as zgam__arcdk:
        pass
...
class NumTypeStaticGetItem(AbstractTemplate):
    ...
    def generic(self, args, kws):
        val, msueg__crqr = args
        if isinstance(msueg__crqr, slice) and (isinstance(val, types.
            NumberClass) or isinstance(val, types.TypeRef) and isinstance(
            val.instance_type, (types.NPDatetime, types.NPTimedelta))):
            return signature(types.TypeRef(val.instance_type[msueg__crqr]),
                *args)
...
def construct_pysig(arg_names, defaults):
    wsh__bkhzu = f'def stub('
    for aprpq__kiv in arg_names:
        wsh__bkhzu += aprpq__kiv
        if aprpq__kiv in defaults:
            if isinstance(defaults[aprpq__kiv], str):
                wsh__bkhzu += f"='{defaults[aprpq__kiv]}'"
            else:
                wsh__bkhzu += f'={defaults[aprpq__kiv]}'
        wsh__bkhzu += ', '
    wsh__bkhzu += '):\n'
    wsh__bkhzu += '    pass\n'
    sfxis__koxqg = {}
    exec(wsh__bkhzu, {}, sfxis__koxqg)
    uaej__yqmyc = sfxis__koxqg['stub']
    return numba.core.utils.pysignature(uaej__yqmyc)


def fold_typing_args(func_name, args, kws, arg_names, defaults,
    unsupported_arg_names=()):
    kws = dict(kws)
    flee__kitvx = len(arg_names)
    lce__vbhh = len(args) + len(kws)
    if lce__vbhh > flee__kitvx:
        atmd__kub = 'argument' if flee__kitvx == 1 else 'arguments'
        zzmv__xypd = 'was' if lce__vbhh == 1 else 'were'
        raise BodoError(
            f'{func_name}(): Too many arguments specified. Function takes {flee__kitvx} {atmd__kub}, but {lce__vbhh} {zzmv__xypd} provided.'
            )
    cid__aafwo = bodo.utils.typing.construct_pysig(arg_names, defaults)
    try:
        cjmu__mooa = bodo.utils.transform.fold_argument_types(cid__aafwo,
            args, kws)
    except Exception as vzz__uxggi:
        raise_bodo_error(f'{func_name}(): {vzz__uxggi}')
    if unsupported_arg_names:
        ...
        for i, arg_name in enumerate(arg_names):
            if arg_name in unsupported_arg_names:
                ...
                xkgl__wkgk[arg_name] = cjmu__mooa[i]
                fcuj__keg[arg_name] = defaults[arg_name]
        check_unsupported_args(func_name, xkgl__wkgk, fcuj__keg)
    return cid__aafwo, cjmu__mooa
...
class BodoArrayIteratorModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ogq__syxbd = [('index', types.EphemeralPointer(types.uintp)), (
            'array', fe_type.arr_type)]
        super(BodoArrayIteratorModel, self).__init__(dmm, fe_type, ogq__syxbd)
...
def iternext_bodo_array(context, builder, sig, args, result):
    [sil__scx] = sig.args
    [roy__vzm] = args
    ncxqy__xvzoz = context.make_helper(builder, sil__scx, value=roy__vzm)
    vux__scc = signature(types.intp, sil__scx.arr_type)
    kpacf__yeryy = context.compile_internal(builder, lambda a: len(a),
        vux__scc, [ncxqy__xvzoz.array])
    wezgj__esu = builder.load(ncxqy__xvzoz.index)
    smvm__raf = builder.icmp_signed('<', wezgj__esu, kpacf__yeryy)
    result.set_valid(smvm__raf)
    with builder.if_then(smvm__raf):
        rcft__uov = signature(sil__scx.yield_type, sil__scx.arr_type, types
            .intp)
        value = context.compile_internal(builder, lambda a, i: a[i],
            rcft__uov, [ncxqy__xvzoz.array, wezgj__esu])
        result.yield_(value)
        gfcn__gavcs = cgutils.increment_index(builder, wezgj__esu)
        builder.store(gfcn__gavcs, ncxqy__xvzoz.index)


def index_typ_from_dtype_name_arr(elem_dtype, name, arr_typ):
    dprex__rvswt = type(get_index_type_from_dtype(elem_dtype))
    if name is None:
        imt__wuz = None
    else:
        imt__wuz = types.StringLiteral(name)
    if dprex__rvswt == bodo.hiframes.pd_index_ext.NumericIndexType:
        tbuc__bxb = dprex__rvswt(elem_dtype, imt__wuz, arr_typ)
    elif dprex__rvswt == bodo.hiframes.pd_index_ext.CategoricalIndexType:
        tbuc__bxb = dprex__rvswt(bodo.CategoricalArrayType(elem_dtype),
            imt__wuz, arr_typ)
    else:
        tbuc__bxb = dprex__rvswt(imt__wuz, arr_typ)
    return tbuc__bxb
...
def check_objmode_output_type(ret_tup, ret_type):
    return tuple(_check_objmode_type(ubtep__kvc, t) for ubtep__kvc, t in
...
def _fix_objmode_df_type(val, val_typ, typ):
    ...
    if val_typ != typ:
        ukw__uyngz = pd.Index(val_typ.columns)
        thms__xyzqn = pd.Index(typ.columns)
        qqvmz__dpmz = ukw__uyngz.argsort()
        ofr__iyc = thms__xyzqn.argsort()
        tggd__yboeg = val_typ.copy(data=tuple(np.array(val_typ.data)[
            qqvmz__dpmz]), columns=tuple(ukw__uyngz[qqvmz__dpmz]))
        jwm__wkef = typ.copy(data=tuple(np.array(typ.data)[ofr__iyc]),
            columns=tuple(thms__xyzqn[ofr__iyc]))
        if tggd__yboeg == jwm__wkef:
            val_typ = typ
