...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        jxjk__uni = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, jxjk__uni)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            raiwp__jqowy = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(raiwp__jqowy.dtype)
            file_write(unicode_to_utf8(fname), raiwp__jqowy.ctypes, 
                dtype_size * raiwp__jqowy.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            raiwp__jqowy = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(raiwp__jqowy.dtype)
            qlxbx__zkx = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                raiwp__jqowy.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), raiwp__jqowy.
                ctypes, start, count, qlxbx__zkx)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            hfwn__jzzpa = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                hfwn__jzzpa = min(hfwn__jzzpa, count * dtype_size)
            if hfwn__jzzpa < 0:
                return -1
            return hfwn__jzzpa
        return impl
