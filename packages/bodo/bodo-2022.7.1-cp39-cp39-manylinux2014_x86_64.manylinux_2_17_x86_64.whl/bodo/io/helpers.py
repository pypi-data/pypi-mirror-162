...
def lower_pyarrow_table_schema(context, builder, ty, pyval):
    jymkw__isq = context.get_python_api(builder)
    return jymkw__isq.unserialize(jymkw__isq.serialize_object(pyval))
...
def get_arrow_timestamp_type(pa_ts_typ):
    giidi__zyzh = 'ns', 'us', 'ms', 's'
    if pa_ts_typ.unit not in giidi__zyzh:
        return types.Array(bodo.datetime64ns, 1, 'C'), False
    elif pa_ts_typ.tz is not None:
        osd__hqv = pa_ts_typ.to_pandas_dtype().tz
        ewmy__lqi = bodo.libs.pd_datetime_arr_ext.get_pytz_type_info(osd__hqv)
        return bodo.DatetimeArrayType(ewmy__lqi), True
    else:
...
def _get_numba_typ_from_pa_typ(pa_typ: pa.Field, is_index,
    ...
    if isinstance(pa_typ.type, pa.ListType):
        cgc__mmm, mhe__jbjji = _get_numba_typ_from_pa_typ(pa_typ.type.
            value_field, is_index, nullable_from_metadata, category_info)
        return ArrayItemArrayType(cgc__mmm), mhe__jbjji
    if isinstance(pa_typ.type, pa.StructType):
        ojqa__tzm = []
        ffuk__jsnvf = []
        mhe__jbjji = True
        for gpop__mahf in pa_typ.flatten():
            ffuk__jsnvf.append(gpop__mahf.name.split('.')[-1])
            wzfew__xyt, ogcp__hboy = _get_numba_typ_from_pa_typ(gpop__mahf,
                is_index, nullable_from_metadata, category_info)
            ojqa__tzm.append(wzfew__xyt)
            mhe__jbjji = mhe__jbjji and ogcp__hboy
        return StructArrayType(tuple(ojqa__tzm), tuple(ffuk__jsnvf)
            ), mhe__jbjji
    ...
    if isinstance(pa_typ.type, pa.DictionaryType):
        ...
        ogis__icks = _pyarrow_numba_type_map[pa_typ.type.index_type]
        yvs__crpm = PDCategoricalDtype(category_info[pa_typ.name], bodo.
            string_type, pa_typ.type.ordered, int_type=ogis__icks)
        return CategoricalArrayType(yvs__crpm), True
    if isinstance(pa_typ.type, pa.lib.TimestampType):
        ...
        rhzjg__iczkn = _pyarrow_numba_type_map[pa_typ.type]
        mhe__jbjji = True
    else:
        raise BodoError('Arrow data type {} not supported yet'.format(
            pa_typ.type))
    if rhzjg__iczkn == datetime_date_type:
        return datetime_date_array_type, mhe__jbjji
    if rhzjg__iczkn == bytes_type:
        return binary_array_type, mhe__jbjji
    cgc__mmm = (string_array_type if rhzjg__iczkn == string_type else types
        .Array(rhzjg__iczkn, 1, 'C'))
    if rhzjg__iczkn == types.bool_:
        cgc__mmm = boolean_array
    fpe__hveyl = (use_nullable_int_arr if nullable_from_metadata is None else
        nullable_from_metadata)
    if fpe__hveyl and not is_index and isinstance(rhzjg__iczkn, types.Integer
        ) and pa_typ.nullable:
        cgc__mmm = IntegerArrayType(rhzjg__iczkn)
    return cgc__mmm, mhe__jbjji
