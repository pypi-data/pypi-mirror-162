...
def box_spark_session(typ, val, c):
    pdojx__jbtnd = c.context.insert_const_string(c.builder.module, 'pyspark')
    chzmu__wyae = c.pyapi.import_module_noblock(pdojx__jbtnd)
    icnns__ctpg = c.pyapi.object_getattr_string(chzmu__wyae, 'sql')
    iimw__nydo = c.pyapi.object_getattr_string(icnns__ctpg, 'SparkSession')
    hreya__zjt = c.pyapi.object_getattr_string(iimw__nydo, 'builder')
    tqmi__lkt = c.pyapi.call_method(hreya__zjt, 'getOrCreate', ())
    c.pyapi.decref(chzmu__wyae)
    c.pyapi.decref(icnns__ctpg)
    c.pyapi.decref(iimw__nydo)
    c.pyapi.decref(hreya__zjt)
    return tqmi__lkt
...
class RowType(types.BaseNamedTuple):
    ...
    def __init__(self, types, fields):
        ...
        pjz__ikb = 'Row({})'.format(', '.join(f'{clv__bvska}:{isgdw__stp}' for
            clv__bvska, isgdw__stp in zip(self.fields, self.types)))
        super(RowType, self).__init__(pjz__ikb)
...
class RowModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        xthrs__mqmax = [(clv__bvska, isgdw__stp) for clv__bvska, isgdw__stp in
            zip(fe_type.fields, fe_type.types)]
        super(RowModel, self).__init__(dmm, fe_type, xthrs__mqmax)
...
def typeof_row(val, c):
    ...
    return RowType(tuple(numba.typeof(svdho__vyhx) for svdho__vyhx in val),
        fields)


@box(RowType)
def box_row(typ, val, c):
    utpuc__tey = c.pyapi.unserialize(c.pyapi.serialize_object(pyspark.sql.
        types.Row))
    if all(clv__bvska.startswith(ANON_SENTINEL) for clv__bvska in typ.fields):
        jyj__vmxzv = [c.box(isgdw__stp, c.builder.extract_value(val, i)) for
            i, isgdw__stp in enumerate(typ.types)]
        ekzqa__mczrz = c.pyapi.call_function_objargs(utpuc__tey, jyj__vmxzv)
        for obj in jyj__vmxzv:
            c.pyapi.decref(obj)
        c.pyapi.decref(utpuc__tey)
        return ekzqa__mczrz
    args = c.pyapi.tuple_pack([])
    jyj__vmxzv = []
    rdjb__nov = []
    for i, isgdw__stp in enumerate(typ.types):
        srmz__xzwb = c.builder.extract_value(val, i)
        obj = c.box(isgdw__stp, srmz__xzwb)
        rdjb__nov.append((typ.fields[i], obj))
        jyj__vmxzv.append(obj)
    kws = c.pyapi.dict_pack(rdjb__nov)
    ekzqa__mczrz = c.pyapi.call(utpuc__tey, args, kws)
    for obj in jyj__vmxzv:
        c.pyapi.decref(obj)
    c.pyapi.decref(utpuc__tey)
    c.pyapi.decref(args)
...
class RowConstructor(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        ztkj__rvrng = ', '.join(f'arg{i}' for i in range(len(args)))
        ffocy__tmear = ', '.join(f"{pcl__cvysa} = ''" for pcl__cvysa in kws)
        func_text = f'def row_stub({ztkj__rvrng}{ffocy__tmear}):\n'
        func_text += '    pass\n'
        mwc__mxgyn = {}
        exec(func_text, {}, mwc__mxgyn)
        rsaq__zjxe = mwc__mxgyn['row_stub']
        nduf__gvno = numba.core.utils.pysignature(rsaq__zjxe)
        if args:
            vkfnw__ovnhk = RowType(args, tuple(f'{ANON_SENTINEL}{i}' for i in
                range(len(args))))
            return signature(vkfnw__ovnhk, *args).replace(pysig=nduf__gvno)
        kws = dict(kws)
        vkfnw__ovnhk = RowType(tuple(kws.values()), tuple(kws.keys()))
        return signature(vkfnw__ovnhk, *kws.values()).replace(pysig=nduf__gvno)
...
class SparkDataFrameModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        xthrs__mqmax = [('df', fe_type.df)]
        super(SparkDataFrameModel, self).__init__(dmm, fe_type, xthrs__mqmax)
...
def overload_create_df(sp_session, data, schema=None, samplingRatio=None,
    ...
    rvkt__qadch = data.dtype.fields
    ere__gdduw = len(data.dtype.types)
    func_text = (
        'def impl(sp_session, data, schema=None, samplingRatio=None, verifySchema=True):\n'
        )
    func_text += f'  n = len(data)\n'
    dawn__nodkp = []
    for i, isgdw__stp in enumerate(data.dtype.types):
        exf__ssk = dtype_to_array_type(isgdw__stp)
        func_text += (
            f'  A{i} = bodo.utils.utils.alloc_type(n, arr_typ{i}, (-1,))\n')
        dawn__nodkp.append(exf__ssk)
    func_text += f'  for i in range(n):\n'
    func_text += f'    r = data[i]\n'
    for i in range(ere__gdduw):
        func_text += (
            f'    A{i}[i] = bodo.utils.conversion.unbox_if_timestamp(r[{i}])\n'
            )
    ykqh__tdd = '({}{})'.format(', '.join(f'A{i}' for i in range(ere__gdduw
        )), ',' if len(rvkt__qadch) == 1 else '')
    func_text += (
        '  index = bodo.hiframes.pd_index_ext.init_range_index(0, n, 1, None)\n'
        )
    func_text += f"""  pdf = bodo.hiframes.pd_dataframe_ext.init_dataframe({ykqh__tdd}, index, __col_name_meta_value_create_df)
"""
    func_text += f'  pdf = bodo.scatterv(pdf)\n'
    func_text += f'  return bodo.libs.pyspark_ext.init_spark_df(pdf)\n'
    mwc__mxgyn = {}
    fxakx__tsh = {'bodo': bodo, '__col_name_meta_value_create_df':
        ColNamesMetaType(tuple(rvkt__qadch))}
    for i in range(ere__gdduw):
        fxakx__tsh[f'arr_typ{i}'] = dawn__nodkp[i]
    exec(func_text, fxakx__tsh, mwc__mxgyn)
    impl = mwc__mxgyn['impl']
    return impl
...
def overload_df_to_rows(df):
    ...
    vqvoc__ebm = ', '.join(f'{c}=A{i}[i]' for i, c in enumerate(df.columns))
    func_text += f'    out.append(Row({vqvoc__ebm}))\n'
    func_text += '  return out\n'
    mwc__mxgyn = {}
    fxakx__tsh = {'bodo': bodo, 'Row': pyspark.sql.types.Row}
    exec(func_text, fxakx__tsh, mwc__mxgyn)
    impl = mwc__mxgyn['impl']
    return impl
...
def _gen_df_select(spark_df, cols, avoid_stararg=False):
    ...
    for col in cols:
        ...
        data, iaaxb__hkjdv = _gen_col_code(col, df_type)
        func_text += iaaxb__hkjdv
        out_data.append(data)
    return _gen_init_spark_df(func_text, out_data, out_col_names)


def _gen_init_spark_df(func_text, out_data, out_col_names):
    ykqh__tdd = '({}{})'.format(', '.join(out_data), ',' if len(out_data) ==
        1 else '')
    wenw__fxza = '0' if not out_data else f'len({out_data[0]})'
    func_text += f'  n = {wenw__fxza}\n'
    func_text += (
        '  index = bodo.hiframes.pd_index_ext.init_range_index(0, n, 1, None)\n'
        )
    func_text += f"""  pdf = bodo.hiframes.pd_dataframe_ext.init_dataframe({ykqh__tdd}, index, __col_name_meta_value_init_spark_df)
"""
    func_text += f'  return bodo.libs.pyspark_ext.init_spark_df(pdf)\n'
    mwc__mxgyn = {}
    fxakx__tsh = {'bodo': bodo, 'np': np,
...
def overload_show(spark_df, n=20, truncate=True, vertical=False):
    pujrr__kfh = dict(truncate=truncate, vertical=vertical)
    toylx__ummxd = dict(truncate=True, vertical=False)
    check_unsupported_args('SparkDataFrameType.show', pujrr__kfh, toylx__ummxd)
...
def overload_with_column(spark_df, colName, col):
    ...
    urzlq__mjotk = spark_df.df.columns
    siah__ivpyy = (urzlq__mjotk if col_name in urzlq__mjotk else 
        urzlq__mjotk + (col_name,))
    mak__aqb, yhfl__tlwjp = _gen_col_code(col, spark_df.df)
    out_data = [(mak__aqb if c == col_name else
        f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {urzlq__mjotk.index(c)})'
        ) for c in siah__ivpyy]
    func_text = 'def impl(spark_df, colName, col):\n'
    func_text += '  df = spark_df._df\n'
    func_text += yhfl__tlwjp
    return _gen_init_spark_df(func_text, out_data, siah__ivpyy)
...
def overload_with_column_renamed(spark_df, existing, new):
    ...
    hig__ensr = get_overload_const_str(existing)
    axlwe__hgbz = get_overload_const_str(new)
    urzlq__mjotk = spark_df.df.columns
    siah__ivpyy = tuple(axlwe__hgbz if c == hig__ensr else c for c in
        urzlq__mjotk)
    out_data = [f'bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {i})'
         for i in range(len(urzlq__mjotk))]
...
def overload_dataframe_columns(spark_df):
    sbmg__lge = list(str(pcl__cvysa) for pcl__cvysa in spark_df.df.columns)
    func_text = 'def impl(spark_df):\n'
    func_text += f'  return {sbmg__lge}\n'
    mwc__mxgyn = {}
    exec(func_text, {}, mwc__mxgyn)
    impl = mwc__mxgyn['impl']
    return impl
...
def init_col_from_name(typingctx, col=None):
    ...
    hmc__exw = get_overload_const_str(col)
    cksn__wanhh = ColumnType(ExprType('col', (hmc__exw,)))

    def codegen(context, builder, signature, args):
        return context.get_constant_null(cksn__wanhh)
    return cksn__wanhh(col), codegen
...
def init_f_sum(typingctx, col=None):
    cksn__wanhh = ColumnType(ExprType('sum', (col.expr,)))
...
def _gen_col_code_expr(expr, df_type):
    ...
    if expr.op == 'sum':
        roq__fjl, qprll__prkeb = _gen_col_code_expr(expr.children[0], df_type)
        i = ir_utils.next_label()
        func_text = f"""  A{i} = np.asarray([bodo.libs.array_ops.array_op_sum({roq__fjl}, True, 0)])
"""
        return f'A{i}', qprll__prkeb + func_text


def _gen_col_code_colname(col_name, df_type):
    tumpn__ginmj = df_type.columns.index(col_name)
    i = ir_utils.next_label()
    func_text = (
        f'  A{i} = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(df, {tumpn__ginmj})\n'
        )
