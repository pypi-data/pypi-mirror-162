...
def _install_mpl_types():
    ovh__fchhg = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for wgj__wapzd, necr__jdjxj in ovh__fchhg:
        install_py_obj_class(types_name=wgj__wapzd, python_type=necr__jdjxj,
            module=this_module)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    niigv__ytpc = ', '.join(f'e{lhsl__odwts}' for lhsl__odwts in range(len(
        args)))
    if niigv__ytpc:
        niigv__ytpc += ', '
    nsv__ynqvw = ', '.join(f"{ghoj__eal} = ''" for ghoj__eal in kws.keys())
    utlw__xvlak = 'matplotlib_obj, ' if obj_typ is not None else ''
    shvkx__pul = f'def mpl_stub({utlw__xvlak} {niigv__ytpc} {nsv__ynqvw}):\n'
    shvkx__pul += '    pass\n'
    hys__lccw = {}
    exec(shvkx__pul, {}, hys__lccw)
    vtw__ethv = hys__lccw['mpl_stub']
    zdoz__jrqoe = numba.core.utils.pysignature(vtw__ethv)
    ijy__gwztj = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(ijy__gwztj)).replace(pysig=
        zdoz__jrqoe)


def generate_axes_typing(mod_name, nrows, ncols):
    klftm__jxhwv = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(klftm__jxhwv.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(klftm__jxhwv.format(mod_name, 'ncols'))
    xwppq__ollm = get_overload_const_int(nrows)
    mtvo__mej = get_overload_const_int(ncols)
    if xwppq__ollm < 1:
        raise BodoError(klftm__jxhwv.format(mod_name, 'nrows'))
    if mtvo__mej < 1:
        raise BodoError(klftm__jxhwv.format(mod_name, 'ncols'))
    if xwppq__ollm == 1 and mtvo__mej == 1:
        brbo__kpd = types.mpl_axes_type
    else:
        if mtvo__mej == 1:
            vtm__jorh = types.mpl_axes_type
        else:
            vtm__jorh = types.Tuple([types.mpl_axes_type] * mtvo__mej)
        brbo__kpd = types.Tuple([vtm__jorh] * xwppq__ollm)
    return brbo__kpd


def generate_pie_return_type(args, kws):
    cjsz__kkaq = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if cjsz__kkaq == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    cnzij__qlkat = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(cnzij__qlkat):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(cnzij__qlkat):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        dhpwm__kumd = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, dhpwm__kumd]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    dhpwm__kumd = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols
        )
    wgj__wapzd = str(dhpwm__kumd)
    if not hasattr(types, wgj__wapzd):
        wgj__wapzd = f'objmode_type{ir_utils.next_label()}'
        setattr(types, wgj__wapzd, dhpwm__kumd)
    shvkx__pul = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{wgj__wapzd}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    hys__lccw = {}
    exec(shvkx__pul, {'numba': numba, 'np': np}, hys__lccw)
    impl = hys__lccw['impl']
    return impl
