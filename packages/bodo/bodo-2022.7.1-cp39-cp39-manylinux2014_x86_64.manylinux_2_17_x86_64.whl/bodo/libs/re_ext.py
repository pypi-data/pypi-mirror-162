...
def box_regex_flag(typ, val, c):
    qdgqq__pra = c.box(types.int64, val)
    dgw__qrxl = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(dgw__qrxl, (qdgqq__pra,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    qdgqq__pra = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, qdgqq__pra)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    vzjnx__fima = context.get_python_api(builder)
    gnf__tmf = builder.icmp_signed('==', val, vzjnx__fima.borrow_none())
    with builder.if_then(gnf__tmf):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    nrymw__xhi = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(nrymw__xhi)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        zyy__qjwy = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if zyy__qjwy > 1:
            typ = types.List(types.Tuple([string_type] * zyy__qjwy))
...
def overload_match_group(m, *args):
    ...
    hhehx__vzq = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            pkyn__twkvo = args[0]
            with numba.objmode(out=hhehx__vzq):
                out = m.group(pkyn__twkvo)
            return out
        return _match_group_impl_one
    kgr__rgs = 'tuple_str_{}'.format(len(args))
    setattr(types, kgr__rgs, types.Tuple([hhehx__vzq] * len(args)))
    ytl__abnrz = ', '.join('group{}'.format(jdyk__idhu + 1) for jdyk__idhu in
        range(len(args)))
    voa__zlin = 'def _match_group_impl(m, *args):\n'
    voa__zlin += '  ({}) = args\n'.format(ytl__abnrz)
    voa__zlin += "  with numba.objmode(out='{}'):\n".format(kgr__rgs)
    voa__zlin += '    out = m.group({})\n'.format(ytl__abnrz)
    voa__zlin += '  return out\n'
    fondg__cmwcm = {}
    exec(voa__zlin, globals(), fondg__cmwcm)
    lik__wbrxv = fondg__cmwcm['_match_group_impl']
    return lik__wbrxv
...
def overload_match_groups(m, default=None):
    gjdv__vvh = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=gjdv__vvh):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(azin__epklw is None for azin__epklw in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=hhehx__vzq):
