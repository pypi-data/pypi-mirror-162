...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    xsfg__lqhe = pq_node.out_vars[0].name
    ctxa__pjlh = pq_node.out_vars[1].name
    if xsfg__lqhe not in lives and ctxa__pjlh not in lives:
        return None
    elif xsfg__lqhe not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.out_used_cols = []
    elif ctxa__pjlh not in lives:
