...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        axknd__agkj = []
        eoest__eixin = h5g_get_num_objs(obj_id)
        for bnw__fjqw in range(eoest__eixin):
            xedx__uuumw = h5g_get_objname_by_idx(obj_id, bnw__fjqw)
            axknd__agkj.append(xedx__uuumw)
        return axknd__agkj
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    osri__fcvog = parse_dtype(dtype)
    kviv__thl = np.int32(numba_to_c_type(osri__fcvog))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            ytjx__twxo = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    gvce__jefay = tuple_to_ptr(starts)
    fovlf__lmir = tuple_to_ptr(counts)
    bpi__cjcs = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, gvce__jefay,
        fovlf__lmir, is_parallel, out_arr.ctypes, bpi__cjcs)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    fovlf__lmir = tuple_to_ptr(counts)
    bpi__cjcs = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, gvce__jefay,
        fovlf__lmir, is_parallel, out_arr.ctypes, bpi__cjcs)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        qkk__jyrdm = types.Array(dtype, ndim, 'C')
        afeg__tzaza = args[3]
        qkk__jyrdm = self.context.resolve_function_type(operator.getitem, [
            qkk__jyrdm, afeg__tzaza], {}).return_type.copy(layout='C')
        return signature(qkk__jyrdm, *args)
...
def get_filter_read_indices(bool_arr):
    qcpk__sff = bool_arr.nonzero()[0]
    yvokd__gay = bodo.libs.distributed_api.get_rank()
    hczf__dgzs = bodo.libs.distributed_api.get_size()
    jyke__gnfna = np.empty(hczf__dgzs, np.int64)
    wyg__pxh = len(bool_arr)
    bodo.libs.distributed_api.allgather(jyke__gnfna, wyg__pxh)
    kgj__wzfp = jyke__gnfna.cumsum()[yvokd__gay] - wyg__pxh
    qcpk__sff += kgj__wzfp
    ppy__pyooy = bodo.libs.distributed_api.dist_reduce(len(qcpk__sff), np.
        int32(sum_op))
    vkk__ooj = bodo.libs.distributed_api.gatherv(qcpk__sff)
    if yvokd__gay == 0:
        dabx__wdk = vkk__ooj
    else:
        dabx__wdk = np.empty(ppy__pyooy, qcpk__sff.dtype)
    bodo.libs.distributed_api.bcast(dabx__wdk)
    sipi__mojb = bodo.libs.distributed_api.get_start(ppy__pyooy, hczf__dgzs,
        yvokd__gay)
    jnh__uao = bodo.libs.distributed_api.get_end(ppy__pyooy, hczf__dgzs,
        yvokd__gay)
    return dabx__wdk[sipi__mojb:jnh__uao]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        rnojo__wncr = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], rnojo__wncr)
        return builder.bitcast(rnojo__wncr, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    fovlf__lmir = tuple_to_ptr(counts)
    bpi__cjcs = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, gvce__jefay,
        fovlf__lmir, is_parallel, out_arr.ctypes, bpi__cjcs, read_indices.
