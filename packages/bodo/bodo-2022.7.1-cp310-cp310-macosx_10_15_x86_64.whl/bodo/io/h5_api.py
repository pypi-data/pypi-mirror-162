...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        kuort__wlhv = []
        cnjrp__pufj = h5g_get_num_objs(obj_id)
        for akvug__tixui in range(cnjrp__pufj):
            cmpci__wggze = h5g_get_objname_by_idx(obj_id, akvug__tixui)
            kuort__wlhv.append(cmpci__wggze)
        return kuort__wlhv
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    utds__ngsd = parse_dtype(dtype)
    izrdp__rjz = np.int32(numba_to_c_type(utds__ngsd))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            pnqd__rfox = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    dch__ihfz = tuple_to_ptr(starts)
    vglm__wghc = tuple_to_ptr(counts)
    tyz__jysx = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, dch__ihfz, vglm__wghc,
        is_parallel, out_arr.ctypes, tyz__jysx)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    vglm__wghc = tuple_to_ptr(counts)
    tyz__jysx = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, dch__ihfz, vglm__wghc,
        is_parallel, out_arr.ctypes, tyz__jysx)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        twsh__imxs = types.Array(dtype, ndim, 'C')
        khk__lfor = args[3]
        twsh__imxs = self.context.resolve_function_type(operator.getitem, [
            twsh__imxs, khk__lfor], {}).return_type.copy(layout='C')
        return signature(twsh__imxs, *args)
...
def get_filter_read_indices(bool_arr):
    pqnd__out = bool_arr.nonzero()[0]
    rre__rwxvy = bodo.libs.distributed_api.get_rank()
    pyb__injuc = bodo.libs.distributed_api.get_size()
    auy__eke = np.empty(pyb__injuc, np.int64)
    uxf__onjpl = len(bool_arr)
    bodo.libs.distributed_api.allgather(auy__eke, uxf__onjpl)
    chx__ztoeo = auy__eke.cumsum()[rre__rwxvy] - uxf__onjpl
    pqnd__out += chx__ztoeo
    zql__jlsh = bodo.libs.distributed_api.dist_reduce(len(pqnd__out), np.
        int32(sum_op))
    bcarw__krmk = bodo.libs.distributed_api.gatherv(pqnd__out)
    if rre__rwxvy == 0:
        qklii__pog = bcarw__krmk
    else:
        qklii__pog = np.empty(zql__jlsh, pqnd__out.dtype)
    bodo.libs.distributed_api.bcast(qklii__pog)
    ehfpv__rkh = bodo.libs.distributed_api.get_start(zql__jlsh, pyb__injuc,
        rre__rwxvy)
    ywxa__gsn = bodo.libs.distributed_api.get_end(zql__jlsh, pyb__injuc,
        rre__rwxvy)
    return qklii__pog[ehfpv__rkh:ywxa__gsn]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        psf__rgy = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], psf__rgy)
        return builder.bitcast(psf__rgy, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    vglm__wghc = tuple_to_ptr(counts)
    tyz__jysx = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, dch__ihfz,
        vglm__wghc, is_parallel, out_arr.ctypes, tyz__jysx, read_indices.
