...
def _global_except_hook(exctype, value, traceback):
    ...
    ooire__hidw = MPI.COMM_WORLD
    ynrw__kgrl = ooire__hidw.Get_rank()
    esu__qmj = 3.0
    zoov__bsnt = True
    kisvg__mtpa = ooire__hidw.Ibarrier()
    wwve__vyrjh = time.time()
    while time.time() - wwve__vyrjh < esu__qmj:
        time.sleep(0.1)
        if kisvg__mtpa.Test():
            zoov__bsnt = False
            break
    try:
        ...
        if zoov__bsnt:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {ynrw__kgrl}. \n')
