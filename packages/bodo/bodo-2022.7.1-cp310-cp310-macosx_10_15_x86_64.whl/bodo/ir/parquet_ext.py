...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    tui__qap = pq_node.out_vars[0].name
    fop__lypum = pq_node.out_vars[1].name
    if tui__qap not in lives and fop__lypum not in lives:
        return None
    elif tui__qap not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.out_used_cols = []
    elif fop__lypum not in lives:
