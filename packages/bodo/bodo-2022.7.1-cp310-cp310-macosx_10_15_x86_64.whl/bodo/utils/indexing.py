...
def get_new_null_mask_bool_index(old_mask, ind, n):
    cbyrk__kizza = n + 7 >> 3
    ugud__qskmr = np.empty(cbyrk__kizza, np.uint8)
    szpq__dcg = 0
    for oxv__fklbp in range(len(ind)):
        if ind[oxv__fklbp]:
            vevo__jhhed = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
                oxv__fklbp)
            bodo.libs.int_arr_ext.set_bit_to_arr(ugud__qskmr, szpq__dcg,
                vevo__jhhed)
            szpq__dcg += 1
    return ugud__qskmr
...
def array_getitem_bool_index(A, ind):
    ...
    wbxpo__uos = A._data[ind]
    n = len(wbxpo__uos)
    ugud__qskmr = get_new_null_mask_bool_index(old_mask, ind, n)
    return wbxpo__uos, ugud__qskmr
...
def array_getitem_int_index(A, ind):
    jdmt__ureqi = bodo.utils.conversion.coerce_to_ndarray(ind)
    old_mask = A._null_bitmap
    wbxpo__uos = A._data[jdmt__ureqi]
    n = len(wbxpo__uos)
    ugud__qskmr = get_new_null_mask_int_index(old_mask, jdmt__ureqi, n)
    return wbxpo__uos, ugud__qskmr


@register_jitable
def get_new_null_mask_slice_index(old_mask, ind, n):
    gqhik__cpaa = numba.cpython.unicode._normalize_slice(ind, n)
    zei__iifo = numba.cpython.unicode._slice_span(gqhik__cpaa)
    cbyrk__kizza = zei__iifo + 7 >> 3
    ugud__qskmr = np.empty(cbyrk__kizza, np.uint8)
    szpq__dcg = 0
    for oxv__fklbp in range(gqhik__cpaa.start, gqhik__cpaa.stop,
        gqhik__cpaa.step):
        vevo__jhhed = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
            oxv__fklbp)
        bodo.libs.int_arr_ext.set_bit_to_arr(ugud__qskmr, szpq__dcg,
...
def array_getitem_slice_index(A, ind):
    n = len(A._data)
    old_mask = A._null_bitmap
    wbxpo__uos = np.ascontiguousarray(A._data[ind])
    ugud__qskmr = get_new_null_mask_slice_index(old_mask, ind, n)
    return wbxpo__uos, ugud__qskmr
...
def array_setitem_int_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            n = len(val._data)
            for oxv__fklbp in range(n):
                A._data[idx[oxv__fklbp]] = val._data[oxv__fklbp]
                vevo__jhhed = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val.
                    _null_bitmap, oxv__fklbp)
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap, idx[
                    oxv__fklbp], vevo__jhhed)
        return impl_arr
    if bodo.utils.typing.is_scalar_type(val):

        def impl_scalar(A, idx, val):
            for oxv__fklbp in idx:
                A._data[oxv__fklbp] = val
...
def array_setitem_bool_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            tip__lnrng = 0
            for oxv__fklbp in range(n):
                if not bodo.libs.array_kernels.isna(idx, oxv__fklbp) and idx[
                    oxv__fklbp]:
                    A._data[oxv__fklbp] = val._data[tip__lnrng]
                    vevo__jhhed = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val
                        ._null_bitmap, tip__lnrng)
                    bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                        oxv__fklbp, vevo__jhhed)
                    tip__lnrng += 1
        return impl_arr
...
def setitem_slice_index_null_bits(dst_bitmap, src_bitmap, idx, n):
    gqhik__cpaa = numba.cpython.unicode._normalize_slice(idx, n)
    tip__lnrng = 0
    for oxv__fklbp in range(gqhik__cpaa.start, gqhik__cpaa.stop,
        gqhik__cpaa.step):
        vevo__jhhed = bodo.libs.int_arr_ext.get_bit_bitmap_arr(src_bitmap,
            tip__lnrng)
        bodo.libs.int_arr_ext.set_bit_to_arr(dst_bitmap, oxv__fklbp,
            vevo__jhhed)
        tip__lnrng += 1
...
def array_setitem_slice_index_overload(A, idx, val):
    ...
    if bodo.utils.typing.is_scalar_type(val):
        ...
        def impl_scalar(A, idx, val):
            gqhik__cpaa = numba.cpython.unicode._normalize_slice(idx, len(A))
            for oxv__fklbp in range(gqhik__cpaa.start, gqhik__cpaa.stop,
                gqhik__cpaa.step):
                A._data[oxv__fklbp] = val
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
    ...
    if not bodo.utils.utils.is_array_typ(A, False):
        ...
        if isinstance(idx, types.Integer):
            ...
            def setitem_none_int_arr(A, idx, val):
                ...
                for oxv__fklbp in idx:
                    bodo.libs.array_kernels.setna(A, oxv__fklbp)
            ...
            if A == bodo.string_array_type:
                ...
                def string_arr_impl(A, idx, val):
                    n = len(A)
                    idx = bodo.utils.conversion.coerce_to_ndarray(idx)
                    aedqr__qrzn = bodo.libs.str_arr_ext.pre_alloc_string_array(
                        n, -1)
                    for oxv__fklbp in numba.parfors.parfor.internal_prange(n):
                        if idx[oxv__fklbp] or bodo.libs.array_kernels.isna(A,
                            oxv__fklbp):
                            aedqr__qrzn[oxv__fklbp] = ''
                            bodo.libs.str_arr_ext.str_arr_set_na(aedqr__qrzn,
                                oxv__fklbp)
                        else:
                            aedqr__qrzn[oxv__fklbp] = A[oxv__fklbp]
            ...
            def setitem_none_bool_arr(A, idx, val):
                ...
                n = len(idx)
                for oxv__fklbp in range(n):
                    if not bodo.libs.array_kernels.isna(idx, oxv__fklbp
                        ) and idx[oxv__fklbp]:
                        bodo.libs.array_kernels.setna(A, oxv__fklbp)
            ...
            def setitem_optional_int_arr(A, idx, val):
                ...
                for oxv__fklbp in idx:
                    if val is None:
                        bodo.libs.array_kernels.setna(A, oxv__fklbp)
                        continue
                    A[oxv__fklbp] = bodo.utils.indexing.unoptional(val)
...
def unoptional(typingctx, val_t=None):
    ...
    def codegen(context, builder, signature, args):
        ajbqy__mtvur = context.make_helper(builder, val_t, args[0])
        tlv__qvzc = ajbqy__mtvur.data
        context.nrt.incref(builder, val_t.type, tlv__qvzc)
        return tlv__qvzc
