...
class TupleArrayType(types.ArrayCompatible):
    ...
    @property
    def dtype(self):
        return types.BaseTuple.from_types(tuple(paef__bnncp.dtype for
            paef__bnncp in self.data))
...
class TupleArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        azjb__pwdvc = [('data', StructArrayType(fe_type.data))]
        models.StructModel.__init__(self, dmm, fe_type, azjb__pwdvc)
...
def init_tuple_arr(typingctx, data_typ=None):
    ...
    lpoj__eym = TupleArrayType(data_typ.data)

    def codegen(context, builder, sig, args):
        zpbq__lmmz, = args
        rvmka__ppr = context.make_helper(builder, lpoj__eym)
        rvmka__ppr.data = zpbq__lmmz
        context.nrt.incref(builder, data_typ, zpbq__lmmz)
        return rvmka__ppr._getvalue()
    return lpoj__eym(data_typ), codegen


@unbox(TupleArrayType)
def unbox_tuple_array(typ, val, c):
    data_typ = StructArrayType(typ.data)
    zarat__dezp = unbox_struct_array(data_typ, val, c, is_tuple_array=True)
    zpbq__lmmz = zarat__dezp.value
    rvmka__ppr = c.context.make_helper(c.builder, typ)
    rvmka__ppr.data = zpbq__lmmz
    csxo__jvfkj = zarat__dezp.is_error
    return NativeValue(rvmka__ppr._getvalue(), is_error=csxo__jvfkj)


@box(TupleArrayType)
def box_tuple_arr(typ, val, c):
    data_typ = StructArrayType(typ.data)
    rvmka__ppr = c.context.make_helper(c.builder, typ, val)
    arr = box_struct_arr(data_typ, rvmka__ppr.data, c, is_tuple_array=True)
    return arr
...
def tuple_arr_getitem(arr, ind):
    ...
    if isinstance(ind, types.Integer):
        tdln__oxbmp = 'def impl(arr, ind):\n'
        tfijq__azbt = ','.join(f'get_data(arr._data)[{mkh__fdsu}][ind]' for
            mkh__fdsu in range(len(arr.data)))
        tdln__oxbmp += f'  return ({tfijq__azbt})\n'
        dbvlg__jbsb = {}
        exec(tdln__oxbmp, {'get_data': bodo.libs.struct_arr_ext.get_data},
            dbvlg__jbsb)
        amwqx__uvtx = dbvlg__jbsb['impl']
        return amwqx__uvtx
...
def tuple_arr_setitem(arr, ind, val):
    ...
    if val == types.none or isinstance(val, types.optional):
        return
    if isinstance(ind, types.Integer):
        iojzw__gakv = len(arr.data)
        tdln__oxbmp = 'def impl(arr, ind, val):\n'
        tdln__oxbmp += '  data = get_data(arr._data)\n'
        tdln__oxbmp += '  null_bitmap = get_null_bitmap(arr._data)\n'
        tdln__oxbmp += '  set_bit_to_arr(null_bitmap, ind, 1)\n'
        for mkh__fdsu in range(iojzw__gakv):
            tdln__oxbmp += f'  data[{mkh__fdsu}][ind] = val[{mkh__fdsu}]\n'
        dbvlg__jbsb = {}
        exec(tdln__oxbmp, {'get_data': bodo.libs.struct_arr_ext.get_data,
