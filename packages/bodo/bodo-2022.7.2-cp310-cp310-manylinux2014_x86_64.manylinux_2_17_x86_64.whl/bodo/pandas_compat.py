...
if pandas_version < (1, 4):
    ...
    def _set_noconvert_columns(self):
        ...
        byb__egqgb = {xni__iezd: yazpz__tjpvc for yazpz__tjpvc, xni__iezd in
            enumerate(self.orig_names)}
        ccztn__kpslw = [byb__egqgb[xni__iezd] for xni__iezd in self.names]
        vdk__pntba = self._set_noconvert_dtype_columns(ccztn__kpslw, self.names
            )
        for rahk__kkqmk in vdk__pntba:
            self._reader.set_noconvert(rahk__kkqmk)
