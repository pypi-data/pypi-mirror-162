...
def box_regex_flag(typ, val, c):
    poc__mijoq = c.box(types.int64, val)
    srg__chmqw = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(srg__chmqw, (poc__mijoq,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    poc__mijoq = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, poc__mijoq)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    joqa__eyoe = context.get_python_api(builder)
    vfo__gkage = builder.icmp_signed('==', val, joqa__eyoe.borrow_none())
    with builder.if_then(vfo__gkage):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    byxwg__mvwey = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(byxwg__mvwey)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        ijngh__mys = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if ijngh__mys > 1:
            typ = types.List(types.Tuple([string_type] * ijngh__mys))
...
def overload_match_group(m, *args):
    ...
    bpeb__euybj = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            rgfad__tyzqq = args[0]
            with numba.objmode(out=bpeb__euybj):
                out = m.group(rgfad__tyzqq)
            return out
        return _match_group_impl_one
    vnxeb__kdmli = 'tuple_str_{}'.format(len(args))
    setattr(types, vnxeb__kdmli, types.Tuple([bpeb__euybj] * len(args)))
    hhrb__xomvt = ', '.join('group{}'.format(uyp__ifau + 1) for uyp__ifau in
        range(len(args)))
    hsky__lgde = 'def _match_group_impl(m, *args):\n'
    hsky__lgde += '  ({}) = args\n'.format(hhrb__xomvt)
    hsky__lgde += "  with numba.objmode(out='{}'):\n".format(vnxeb__kdmli)
    hsky__lgde += '    out = m.group({})\n'.format(hhrb__xomvt)
    hsky__lgde += '  return out\n'
    ylvzi__gref = {}
    exec(hsky__lgde, globals(), ylvzi__gref)
    czzwh__brf = ylvzi__gref['_match_group_impl']
    return czzwh__brf
...
def overload_match_groups(m, default=None):
    tait__pdb = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=tait__pdb):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(mzw__xizxx is None for mzw__xizxx in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=bpeb__euybj):
