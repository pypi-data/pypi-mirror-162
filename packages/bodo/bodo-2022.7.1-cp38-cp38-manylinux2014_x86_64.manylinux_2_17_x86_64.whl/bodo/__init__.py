...
def _global_except_hook(exctype, value, traceback):
    ...
    drm__tyoy = MPI.COMM_WORLD
    tqga__yca = drm__tyoy.Get_rank()
    nbv__dfkvw = 3.0
    fwxrg__rvsk = True
    upuhq__ygl = drm__tyoy.Ibarrier()
    vhbkk__zzzlz = time.time()
    while time.time() - vhbkk__zzzlz < nbv__dfkvw:
        time.sleep(0.1)
        if upuhq__ygl.Test():
            fwxrg__rvsk = False
            break
    try:
        ...
        if fwxrg__rvsk:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {tqga__yca}. \n')
