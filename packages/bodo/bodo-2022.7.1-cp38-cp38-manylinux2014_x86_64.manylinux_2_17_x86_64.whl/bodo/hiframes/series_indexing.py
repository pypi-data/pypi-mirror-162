...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        bzly__fkzkq = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(bzly__fkzkq)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        dic__zez = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, dic__zez)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        pjf__rqov, = args
        aewfo__zcm = signature.return_type
        fdeg__scce = cgutils.create_struct_proxy(aewfo__zcm)(context, builder)
        fdeg__scce.obj = pjf__rqov
        context.nrt.incref(builder, signature.args[0], pjf__rqov)
        return fdeg__scce._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                vzdc__nlm = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        bzly__fkzkq = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(bzly__fkzkq)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        dic__zez = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, dic__zez)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        pjf__rqov, = args
        irj__wotrx = signature.return_type
        oha__hizp = cgutils.create_struct_proxy(irj__wotrx)(context, builder)
        oha__hizp.obj = pjf__rqov
        context.nrt.incref(builder, signature.args[0], pjf__rqov)
        return oha__hizp._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                fhs__lgb = bodo.utils.conversion.coerce_to_ndarray(idx)
                qeou__jzk = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    fhs__lgb]
                ixeu__wrmkb = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    fhs__lgb]
                bzly__fkzkq = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(qeou__jzk,
                    ixeu__wrmkb, bzly__fkzkq)
            return impl
        if isinstance(idx, types.SliceType):

            def impl_slice(I, idx):
                S = I._obj
                qeou__jzk = bodo.hiframes.pd_series_ext.get_series_data(S)[idx]
                ixeu__wrmkb = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    idx]
                bzly__fkzkq = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(qeou__jzk,
    ...
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        vzdc__nlm = (bodo.hiframes.datetime_timedelta_ext.
                            _to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(vzdc__nlm))
                        fhs__lgb = bodo.utils.conversion.coerce_to_ndarray(idx)
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            fhs__lgb] = s
                    return impl_dt
            ...
            def impl_arr(I, idx, val):
                fhs__lgb = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[fhs__lgb
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        bzly__fkzkq = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(bzly__fkzkq)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        dic__zez = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, dic__zez)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        pjf__rqov, = args
        rube__izcw = signature.return_type
        nvf__rfcvy = cgutils.create_struct_proxy(rube__izcw)(context, builder)
        nvf__rfcvy.obj = pjf__rqov
        context.nrt.incref(builder, signature.args[0], pjf__rqov)
        return nvf__rfcvy._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if is_list_like_index_type(idx) and idx.dtype == types.bool_:
        ...
        def impl(I, idx):
            S = I._obj
            fhs__lgb = bodo.utils.conversion.coerce_to_ndarray(idx)
            qeou__jzk = bodo.hiframes.pd_series_ext.get_series_data(S)[fhs__lgb
                ]
            ixeu__wrmkb = bodo.hiframes.pd_series_ext.get_series_index(S)[
                fhs__lgb]
            bzly__fkzkq = bodo.hiframes.pd_series_ext.get_series_name(S)
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            qeou__jzk = bodo.hiframes.pd_series_ext.get_series_data(S)
            ixeu__wrmkb = bodo.hiframes.pd_series_ext.get_series_index(S)
            hohah__wpyq = (idx - ixeu__wrmkb._start) // ixeu__wrmkb._step
            return qeou__jzk[hohah__wpyq]
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    ...
                    fhs__lgb = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(qeou__jzk[
                        fhs__lgb])
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    xynxh__uiuj = bodo.hiframes.pd_series_ext.get_series_index(
                        S)
                    med__oyu = xynxh__uiuj.get_loc(idx)
                    if med__oyu is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(med__oyu)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                fhs__lgb = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[fhs__lgb
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        pfjd__ccl = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(oxhqq__fwg,
            int) for oxhqq__fwg in pfjd__ccl):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            vohcd__gmbk = get_literal_value(idx)
            if vohcd__gmbk not in pfjd__ccl:
                raise_bodo_error(
                    f"Series label-based getitem: '{vohcd__gmbk}' not in {pfjd__ccl}"
                    )
            arr_ind = pfjd__ccl.index(vohcd__gmbk)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    fdeg__scce = cgutils.create_struct_proxy(fromty)(context, builder, val)
    muokr__gsg = context.cast(builder, fdeg__scce.obj, fromty.stype, toty.stype
        )
    aoc__wml = cgutils.create_struct_proxy(toty)(context, builder)
    aoc__wml.obj = muokr__gsg
    return aoc__wml._getvalue()
