...
def get_new_null_mask_bool_index(old_mask, ind, n):
    tsdt__nlmcb = n + 7 >> 3
    tytfw__jpypi = np.empty(tsdt__nlmcb, np.uint8)
    okb__jbyc = 0
    for hfbk__spi in range(len(ind)):
        if ind[hfbk__spi]:
            uvbqt__ddog = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
                hfbk__spi)
            bodo.libs.int_arr_ext.set_bit_to_arr(tytfw__jpypi, okb__jbyc,
                uvbqt__ddog)
            okb__jbyc += 1
    return tytfw__jpypi
...
def array_getitem_bool_index(A, ind):
    ...
    vrgw__guujq = A._data[ind]
    n = len(vrgw__guujq)
    tytfw__jpypi = get_new_null_mask_bool_index(old_mask, ind, n)
    return vrgw__guujq, tytfw__jpypi
...
def array_getitem_int_index(A, ind):
    ddf__doz = bodo.utils.conversion.coerce_to_ndarray(ind)
    old_mask = A._null_bitmap
    vrgw__guujq = A._data[ddf__doz]
    n = len(vrgw__guujq)
    tytfw__jpypi = get_new_null_mask_int_index(old_mask, ddf__doz, n)
    return vrgw__guujq, tytfw__jpypi


@register_jitable
def get_new_null_mask_slice_index(old_mask, ind, n):
    jca__fcbz = numba.cpython.unicode._normalize_slice(ind, n)
    bdn__rgs = numba.cpython.unicode._slice_span(jca__fcbz)
    tsdt__nlmcb = bdn__rgs + 7 >> 3
    tytfw__jpypi = np.empty(tsdt__nlmcb, np.uint8)
    okb__jbyc = 0
    for hfbk__spi in range(jca__fcbz.start, jca__fcbz.stop, jca__fcbz.step):
        uvbqt__ddog = bodo.libs.int_arr_ext.get_bit_bitmap_arr(old_mask,
            hfbk__spi)
        bodo.libs.int_arr_ext.set_bit_to_arr(tytfw__jpypi, okb__jbyc,
            uvbqt__ddog)
...
def array_getitem_slice_index(A, ind):
    n = len(A._data)
    old_mask = A._null_bitmap
    vrgw__guujq = np.ascontiguousarray(A._data[ind])
    tytfw__jpypi = get_new_null_mask_slice_index(old_mask, ind, n)
    return vrgw__guujq, tytfw__jpypi
...
def array_setitem_int_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            n = len(val._data)
            for hfbk__spi in range(n):
                A._data[idx[hfbk__spi]] = val._data[hfbk__spi]
                uvbqt__ddog = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val.
                    _null_bitmap, hfbk__spi)
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap, idx[
                    hfbk__spi], uvbqt__ddog)
        return impl_arr
    if bodo.utils.typing.is_scalar_type(val):

        def impl_scalar(A, idx, val):
            for hfbk__spi in idx:
                A._data[hfbk__spi] = val
...
def array_setitem_bool_index_overload(A, idx, val):
    if bodo.utils.utils.is_array_typ(val
        ...
        def impl_arr(A, idx, val):
            ...
            mncd__vlj = 0
            for hfbk__spi in range(n):
                if not bodo.libs.array_kernels.isna(idx, hfbk__spi) and idx[
                    hfbk__spi]:
                    A._data[hfbk__spi] = val._data[mncd__vlj]
                    uvbqt__ddog = bodo.libs.int_arr_ext.get_bit_bitmap_arr(val
                        ._null_bitmap, mncd__vlj)
                    bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                        hfbk__spi, uvbqt__ddog)
                    mncd__vlj += 1
        return impl_arr
...
def setitem_slice_index_null_bits(dst_bitmap, src_bitmap, idx, n):
    jca__fcbz = numba.cpython.unicode._normalize_slice(idx, n)
    mncd__vlj = 0
    for hfbk__spi in range(jca__fcbz.start, jca__fcbz.stop, jca__fcbz.step):
        uvbqt__ddog = bodo.libs.int_arr_ext.get_bit_bitmap_arr(src_bitmap,
            mncd__vlj)
        bodo.libs.int_arr_ext.set_bit_to_arr(dst_bitmap, hfbk__spi, uvbqt__ddog
            )
        mncd__vlj += 1
...
def array_setitem_slice_index_overload(A, idx, val):
    ...
    if bodo.utils.typing.is_scalar_type(val):
        ...
        def impl_scalar(A, idx, val):
            jca__fcbz = numba.cpython.unicode._normalize_slice(idx, len(A))
            for hfbk__spi in range(jca__fcbz.start, jca__fcbz.stop,
                jca__fcbz.step):
                A._data[hfbk__spi] = val
                bodo.libs.int_arr_ext.set_bit_to_arr(A._null_bitmap,
                    hfbk__spi, 1)
    ...
    if not bodo.utils.utils.is_array_typ(A, False):
        ...
        if isinstance(idx, types.Integer):
            ...
            def setitem_none_int_arr(A, idx, val):
                ...
                for hfbk__spi in idx:
                    bodo.libs.array_kernels.setna(A, hfbk__spi)
            ...
            if A == bodo.string_array_type:
                ...
                def string_arr_impl(A, idx, val):
                    n = len(A)
                    idx = bodo.utils.conversion.coerce_to_ndarray(idx)
                    alw__jwim = bodo.libs.str_arr_ext.pre_alloc_string_array(n,
                        -1)
                    for hfbk__spi in numba.parfors.parfor.internal_prange(n):
                        if idx[hfbk__spi] or bodo.libs.array_kernels.isna(A,
                            hfbk__spi):
                            alw__jwim[hfbk__spi] = ''
                            bodo.libs.str_arr_ext.str_arr_set_na(alw__jwim,
                                hfbk__spi)
                        else:
                            alw__jwim[hfbk__spi] = A[hfbk__spi]
            ...
            def setitem_none_bool_arr(A, idx, val):
                ...
                n = len(idx)
                for hfbk__spi in range(n):
                    if not bodo.libs.array_kernels.isna(idx, hfbk__spi
                        ) and idx[hfbk__spi]:
                        bodo.libs.array_kernels.setna(A, hfbk__spi)
            ...
            def setitem_optional_int_arr(A, idx, val):
                ...
                for hfbk__spi in idx:
                    if val is None:
                        bodo.libs.array_kernels.setna(A, hfbk__spi)
                        continue
                    A[hfbk__spi] = bodo.utils.indexing.unoptional(val)
...
def unoptional(typingctx, val_t=None):
    ...
    def codegen(context, builder, signature, args):
        fmt__vmg = context.make_helper(builder, val_t, args[0])
        kfgi__asl = fmt__vmg.data
        context.nrt.incref(builder, val_t.type, kfgi__asl)
        return kfgi__asl
