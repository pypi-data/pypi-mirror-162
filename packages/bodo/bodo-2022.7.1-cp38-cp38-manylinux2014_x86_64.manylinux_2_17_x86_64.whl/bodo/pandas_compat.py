...
if pandas_version < (1, 4):
    ...
    def _set_noconvert_columns(self):
        ...
        xxr__upxa = {kloll__roizy: simgd__kbco for simgd__kbco,
            kloll__roizy in enumerate(self.orig_names)}
        vtmj__xauhc = [xxr__upxa[kloll__roizy] for kloll__roizy in self.names]
        fky__qytn = self._set_noconvert_dtype_columns(vtmj__xauhc, self.names)
        for onr__gqy in fky__qytn:
            self._reader.set_noconvert(onr__gqy)
