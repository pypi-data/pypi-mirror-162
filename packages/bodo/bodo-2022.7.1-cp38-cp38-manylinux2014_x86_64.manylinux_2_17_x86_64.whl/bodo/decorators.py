...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        nik__guqr = self.overloads[sig]
        yhvn__rii = nik__guqr.metadata.get('distributed_diagnostics', None)
        if yhvn__rii is None:
            ldhns__vxerf = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(ldhns__vxerf)
        yhvn__rii.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        wipcv__tlen = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(wipcv__tlen)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        fkze__jjtua = options.pop('distributed')
        pipeline_class = (pipeline_class if fkze__jjtua else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        uoigv__qzji = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if uoigv__qzji else
            pipeline_class)
    key__mjwgk = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(key__mjwgk, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(key__mjwgk)
        else:
            return master_mode_wrapper(key__mjwgk)
    else:
        return key__mjwgk
