...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        biq__unftv = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, biq__unftv)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            mgwoz__wtuk = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(mgwoz__wtuk.dtype)
            file_write(unicode_to_utf8(fname), mgwoz__wtuk.ctypes, 
                dtype_size * mgwoz__wtuk.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            mgwoz__wtuk = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(mgwoz__wtuk.dtype)
            kzcg__mpwz = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                mgwoz__wtuk.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), mgwoz__wtuk.ctypes,
                start, count, kzcg__mpwz)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            mukw__empv = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                mukw__empv = min(mukw__empv, count * dtype_size)
            if mukw__empv < 0:
                return -1
            return mukw__empv
        return impl
