...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        qejs__qqm = []
        zxqda__wohxd = h5g_get_num_objs(obj_id)
        for xqqbe__dej in range(zxqda__wohxd):
            umx__jahn = h5g_get_objname_by_idx(obj_id, xqqbe__dej)
            qejs__qqm.append(umx__jahn)
        return qejs__qqm
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    yhl__sobax = parse_dtype(dtype)
    qmac__twl = np.int32(numba_to_c_type(yhl__sobax))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            vuyl__jxrbi = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ishow__jsrue = tuple_to_ptr(starts)
    pqbdx__mayqa = tuple_to_ptr(counts)
    tvlh__udga = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, ishow__jsrue,
        pqbdx__mayqa, is_parallel, out_arr.ctypes, tvlh__udga)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    pqbdx__mayqa = tuple_to_ptr(counts)
    tvlh__udga = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, ishow__jsrue,
        pqbdx__mayqa, is_parallel, out_arr.ctypes, tvlh__udga)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        jbtov__rmht = types.Array(dtype, ndim, 'C')
        cifyn__neu = args[3]
        jbtov__rmht = self.context.resolve_function_type(operator.getitem,
            [jbtov__rmht, cifyn__neu], {}).return_type.copy(layout='C')
        return signature(jbtov__rmht, *args)
...
def get_filter_read_indices(bool_arr):
    xqn__mee = bool_arr.nonzero()[0]
    epof__ouknm = bodo.libs.distributed_api.get_rank()
    sbz__orkdr = bodo.libs.distributed_api.get_size()
    wddj__ymnhz = np.empty(sbz__orkdr, np.int64)
    psvck__kktc = len(bool_arr)
    bodo.libs.distributed_api.allgather(wddj__ymnhz, psvck__kktc)
    qks__douc = wddj__ymnhz.cumsum()[epof__ouknm] - psvck__kktc
    xqn__mee += qks__douc
    cukls__hcjsk = bodo.libs.distributed_api.dist_reduce(len(xqn__mee), np.
        int32(sum_op))
    gqjb__yjxr = bodo.libs.distributed_api.gatherv(xqn__mee)
    if epof__ouknm == 0:
        krrql__lwko = gqjb__yjxr
    else:
        krrql__lwko = np.empty(cukls__hcjsk, xqn__mee.dtype)
    bodo.libs.distributed_api.bcast(krrql__lwko)
    fgu__jidej = bodo.libs.distributed_api.get_start(cukls__hcjsk,
        sbz__orkdr, epof__ouknm)
    ghdi__csau = bodo.libs.distributed_api.get_end(cukls__hcjsk, sbz__orkdr,
        epof__ouknm)
    return krrql__lwko[fgu__jidej:ghdi__csau]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        blea__qlrl = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], blea__qlrl)
        return builder.bitcast(blea__qlrl, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    pqbdx__mayqa = tuple_to_ptr(counts)
    tvlh__udga = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, ishow__jsrue,
        pqbdx__mayqa, is_parallel, out_arr.ctypes, tvlh__udga, read_indices
