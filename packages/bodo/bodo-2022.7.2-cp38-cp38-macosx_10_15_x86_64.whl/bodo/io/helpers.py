...
def lower_pyarrow_table_schema(context, builder, ty, pyval):
    eqf__gninh = context.get_python_api(builder)
    return eqf__gninh.unserialize(eqf__gninh.serialize_object(pyval))
...
def get_arrow_timestamp_type(pa_ts_typ):
    mxa__xhm = 'ns', 'us', 'ms', 's'
    if pa_ts_typ.unit not in mxa__xhm:
        return types.Array(bodo.datetime64ns, 1, 'C'), False
    elif pa_ts_typ.tz is not None:
        iiykb__dvok = pa_ts_typ.to_pandas_dtype().tz
        pbc__vau = bodo.libs.pd_datetime_arr_ext.get_pytz_type_info(iiykb__dvok
            )
        return bodo.DatetimeArrayType(pbc__vau), True
    else:
...
def _get_numba_typ_from_pa_typ(pa_typ: pa.Field, is_index,
    ...
    if isinstance(pa_typ.type, pa.ListType):
        nevl__fsf, szkdn__ogocn = _get_numba_typ_from_pa_typ(pa_typ.type.
            value_field, is_index, nullable_from_metadata, category_info)
        return ArrayItemArrayType(nevl__fsf), szkdn__ogocn
    if isinstance(pa_typ.type, pa.StructType):
        pvv__pdt = []
        cpe__brym = []
        szkdn__ogocn = True
        for yoey__kdlw in pa_typ.flatten():
            cpe__brym.append(yoey__kdlw.name.split('.')[-1])
            opbw__utb, thdc__his = _get_numba_typ_from_pa_typ(yoey__kdlw,
                is_index, nullable_from_metadata, category_info)
            pvv__pdt.append(opbw__utb)
            szkdn__ogocn = szkdn__ogocn and thdc__his
        return StructArrayType(tuple(pvv__pdt), tuple(cpe__brym)), szkdn__ogocn
    ...
    if isinstance(pa_typ.type, pa.DictionaryType):
        ...
        dif__zyn = _pyarrow_numba_type_map[pa_typ.type.index_type]
        dukwk__gmpn = PDCategoricalDtype(category_info[pa_typ.name], bodo.
            string_type, pa_typ.type.ordered, int_type=dif__zyn)
        return CategoricalArrayType(dukwk__gmpn), True
    if isinstance(pa_typ.type, pa.lib.TimestampType):
        ...
        aueee__fyswk = _pyarrow_numba_type_map[pa_typ.type]
        szkdn__ogocn = True
    else:
        raise BodoError('Arrow data type {} not supported yet'.format(
            pa_typ.type))
    if aueee__fyswk == datetime_date_type:
        return datetime_date_array_type, szkdn__ogocn
    if aueee__fyswk == bytes_type:
        return binary_array_type, szkdn__ogocn
    nevl__fsf = (string_array_type if aueee__fyswk == string_type else
        types.Array(aueee__fyswk, 1, 'C'))
    if aueee__fyswk == types.bool_:
        nevl__fsf = boolean_array
    zolqw__faawb = (use_nullable_int_arr if nullable_from_metadata is None else
        nullable_from_metadata)
    if zolqw__faawb and not is_index and isinstance(aueee__fyswk, types.Integer
        ) and pa_typ.nullable:
        nevl__fsf = IntegerArrayType(aueee__fyswk)
    return nevl__fsf, szkdn__ogocn
