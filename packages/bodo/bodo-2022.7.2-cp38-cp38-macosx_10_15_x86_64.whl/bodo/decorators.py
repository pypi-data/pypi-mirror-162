...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        oyv__gswis = self.overloads[sig]
        utdsy__wnrhx = oyv__gswis.metadata.get('distributed_diagnostics', None)
        if utdsy__wnrhx is None:
            pkkel__yjq = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(pkkel__yjq)
        utdsy__wnrhx.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        lbnwa__kbqxd = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(lbnwa__kbqxd)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        epcn__fpc = options.pop('distributed')
        pipeline_class = (pipeline_class if epcn__fpc else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        caqhx__oow = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if caqhx__oow else
            pipeline_class)
    iwszh__afz = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(iwszh__afz, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(iwszh__afz)
        else:
            return master_mode_wrapper(iwszh__afz)
    else:
        return iwszh__afz
