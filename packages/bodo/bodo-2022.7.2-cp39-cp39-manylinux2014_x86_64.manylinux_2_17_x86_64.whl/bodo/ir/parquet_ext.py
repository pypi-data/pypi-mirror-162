...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    sphwe__iim = pq_node.out_vars[0].name
    krts__jpeh = pq_node.out_vars[1].name
    if sphwe__iim not in lives and krts__jpeh not in lives:
        return None
    elif sphwe__iim not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.out_used_cols = []
    elif krts__jpeh not in lives:
