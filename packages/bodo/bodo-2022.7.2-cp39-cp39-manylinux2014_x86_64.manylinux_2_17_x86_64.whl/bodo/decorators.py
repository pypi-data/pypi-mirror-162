...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        jue__fpgui = self.overloads[sig]
        ouxz__eoc = jue__fpgui.metadata.get('distributed_diagnostics', None)
        if ouxz__eoc is None:
            yioez__mso = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(yioez__mso)
        ouxz__eoc.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        kqppi__wqvj = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(kqppi__wqvj)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        uayh__tjlfb = options.pop('distributed')
        pipeline_class = (pipeline_class if uayh__tjlfb else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        tydcf__syydb = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if tydcf__syydb else
            pipeline_class)
    iqr__sxq = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(iqr__sxq, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(iqr__sxq)
        else:
            return master_mode_wrapper(iqr__sxq)
    else:
        return iqr__sxq
