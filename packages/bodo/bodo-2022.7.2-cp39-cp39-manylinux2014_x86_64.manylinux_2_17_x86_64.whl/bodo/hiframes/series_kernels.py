...
def _column_filter_impl(B, ind):
    jzt__mni = bodo.hiframes.rolling.alloc_shift(len(B), B, (-1,))
    for bmes__xgqyt in numba.parfors.parfor.internal_prange(len(jzt__mni)):
        if ind[bmes__xgqyt]:
            jzt__mni[bmes__xgqyt] = B[bmes__xgqyt]
        else:
            bodo.libs.array_kernels.setna(jzt__mni, bmes__xgqyt)
    return jzt__mni
...
def _series_dropna_str_alloc_impl_inner(B):
    ...
    gdeem__xkhe = len(B)
    vjpm__tmgmz = 0
    for bmes__xgqyt in range(len(B)):
        if bodo.libs.str_arr_ext.str_arr_is_na(B, bmes__xgqyt):
            vjpm__tmgmz += 1
    win__xvl = gdeem__xkhe - vjpm__tmgmz
    lfhws__kas = bodo.libs.str_arr_ext.num_total_chars(B)
    jzt__mni = bodo.libs.str_arr_ext.pre_alloc_string_array(win__xvl,
        lfhws__kas)
    bodo.libs.str_arr_ext.copy_non_null_offsets(jzt__mni, B)
    bodo.libs.str_arr_ext.copy_data(jzt__mni, B)
    bodo.libs.str_arr_ext.set_null_bits_to_value(jzt__mni, -1)
    return jzt__mni
...
def compute_skew(first_moment, second_moment, third_moment, count):
    if count < 3:
        return np.nan
    ikcoe__cobid = first_moment / count
    zcukk__rbtlw = (third_moment - 3 * second_moment * ikcoe__cobid + 2 *
        count * ikcoe__cobid ** 3)
    yrzk__vspvo = second_moment - ikcoe__cobid * first_moment
    s = count * (count - 1) ** 1.5 / (count - 2
        ) * zcukk__rbtlw / yrzk__vspvo ** 1.5
    s = s / (count - 1)
    return s
...
def compute_kurt(first_moment, second_moment, third_moment, fourth_moment,
    ...
    if count < 4:
        return np.nan
    ikcoe__cobid = first_moment / count
    ednh__salsk = (fourth_moment - 4 * third_moment * ikcoe__cobid + 6 *
        second_moment * ikcoe__cobid ** 2 - 3 * count * ikcoe__cobid ** 4)
    zsf__chssr = second_moment - ikcoe__cobid * first_moment
    kqqv__foka = 3 * (count - 1) ** 2 / ((count - 2) * (count - 3))
    dicj__znivy = count * (count + 1) * (count - 1) * ednh__salsk
    xyvr__brxto = (count - 2) * (count - 3) * zsf__chssr ** 2
    s = (count - 1) * (dicj__znivy / xyvr__brxto - kqqv__foka)
    s = s / (count - 1)
    return s
