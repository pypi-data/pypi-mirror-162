...
def _install_mpl_types():
    ulpfz__ylh = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for ubw__gphx, efg__jyk in ulpfz__ylh:
        install_py_obj_class(types_name=ubw__gphx, python_type=efg__jyk,
            module=this_module)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    sal__edwgf = ', '.join(f'e{wyzr__mjm}' for wyzr__mjm in range(len(args)))
    if sal__edwgf:
        sal__edwgf += ', '
    rpdp__mrd = ', '.join(f"{kwqe__ijq} = ''" for kwqe__ijq in kws.keys())
    okc__cde = 'matplotlib_obj, ' if obj_typ is not None else ''
    sdpxv__zmci = f'def mpl_stub({okc__cde} {sal__edwgf} {rpdp__mrd}):\n'
    sdpxv__zmci += '    pass\n'
    maenc__zbo = {}
    exec(sdpxv__zmci, {}, maenc__zbo)
    dyqcy__ltk = maenc__zbo['mpl_stub']
    gfgna__pygkr = numba.core.utils.pysignature(dyqcy__ltk)
    mej__bhy = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(mej__bhy)).replace(pysig=
        gfgna__pygkr)


def generate_axes_typing(mod_name, nrows, ncols):
    dtlks__otocm = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(dtlks__otocm.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(dtlks__otocm.format(mod_name, 'ncols'))
    zxr__mjt = get_overload_const_int(nrows)
    xqd__vmelr = get_overload_const_int(ncols)
    if zxr__mjt < 1:
        raise BodoError(dtlks__otocm.format(mod_name, 'nrows'))
    if xqd__vmelr < 1:
        raise BodoError(dtlks__otocm.format(mod_name, 'ncols'))
    if zxr__mjt == 1 and xqd__vmelr == 1:
        ppkz__xgcqb = types.mpl_axes_type
    else:
        if xqd__vmelr == 1:
            hkx__rzz = types.mpl_axes_type
        else:
            hkx__rzz = types.Tuple([types.mpl_axes_type] * xqd__vmelr)
        ppkz__xgcqb = types.Tuple([hkx__rzz] * zxr__mjt)
    return ppkz__xgcqb


def generate_pie_return_type(args, kws):
    adxvj__wgp = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if adxvj__wgp == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    qhbo__trzok = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(qhbo__trzok):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(qhbo__trzok):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        hrmr__isla = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, hrmr__isla]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    hrmr__isla = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols)
    ubw__gphx = str(hrmr__isla)
    if not hasattr(types, ubw__gphx):
        ubw__gphx = f'objmode_type{ir_utils.next_label()}'
        setattr(types, ubw__gphx, hrmr__isla)
    sdpxv__zmci = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{ubw__gphx}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    maenc__zbo = {}
    exec(sdpxv__zmci, {'numba': numba, 'np': np}, maenc__zbo)
    impl = maenc__zbo['impl']
    return impl
