...
def _global_except_hook(exctype, value, traceback):
    ...
    eqw__wdflx = MPI.COMM_WORLD
    fgo__dbijp = eqw__wdflx.Get_rank()
    gdbhj__azm = 3.0
    dfysk__pvozr = True
    pjlfv__zjyaz = eqw__wdflx.Ibarrier()
    hhsm__zonnc = time.time()
    while time.time() - hhsm__zonnc < gdbhj__azm:
        time.sleep(0.1)
        if pjlfv__zjyaz.Test():
            dfysk__pvozr = False
            break
    try:
        ...
        if dfysk__pvozr:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {fgo__dbijp}. \n')
