...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        uqqzk__tzo = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, uqqzk__tzo)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            tut__tow = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(tut__tow.dtype)
            file_write(unicode_to_utf8(fname), tut__tow.ctypes, dtype_size *
                tut__tow.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            tut__tow = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(tut__tow.dtype)
            vwbfj__nxjx = (dtype_size * bodo.libs.distributed_api.
                get_tuple_prod(tut__tow.shape[1:]))
            _file_write_parallel(unicode_to_utf8(fname), tut__tow.ctypes,
                start, count, vwbfj__nxjx)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            tenss__bsk = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                tenss__bsk = min(tenss__bsk, count * dtype_size)
            if tenss__bsk < 0:
                return -1
            return tenss__bsk
        return impl
