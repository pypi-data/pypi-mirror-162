...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    mhu__tyz = pq_node.out_vars[0].name
    vgs__qrp = pq_node.out_vars[1].name
    if mhu__tyz not in lives and vgs__qrp not in lives:
        return None
    elif mhu__tyz not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.out_used_cols = []
    elif vgs__qrp not in lives:
