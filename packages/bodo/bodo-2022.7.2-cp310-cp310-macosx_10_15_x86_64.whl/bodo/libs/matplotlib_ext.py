...
def _install_mpl_types():
    fgzny__zsaia = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for qmc__zuhbu, uxal__wbxy in fgzny__zsaia:
        install_py_obj_class(types_name=qmc__zuhbu, python_type=uxal__wbxy,
            module=this_module)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    seao__ivlth = ', '.join(f'e{zalpm__ozdav}' for zalpm__ozdav in range(
        len(args)))
    if seao__ivlth:
        seao__ivlth += ', '
    apuhi__bdk = ', '.join(f"{gzi__all} = ''" for gzi__all in kws.keys())
    yvx__epq = 'matplotlib_obj, ' if obj_typ is not None else ''
    iyq__pih = f'def mpl_stub({yvx__epq} {seao__ivlth} {apuhi__bdk}):\n'
    iyq__pih += '    pass\n'
    zkd__jpt = {}
    exec(iyq__pih, {}, zkd__jpt)
    vfbav__tdm = zkd__jpt['mpl_stub']
    tdiy__zba = numba.core.utils.pysignature(vfbav__tdm)
    hevsl__qxz = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(hevsl__qxz)).replace(pysig=
        tdiy__zba)


def generate_axes_typing(mod_name, nrows, ncols):
    saixv__wfboi = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(saixv__wfboi.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(saixv__wfboi.format(mod_name, 'ncols'))
    qhfm__enqcr = get_overload_const_int(nrows)
    wmriz__ucqp = get_overload_const_int(ncols)
    if qhfm__enqcr < 1:
        raise BodoError(saixv__wfboi.format(mod_name, 'nrows'))
    if wmriz__ucqp < 1:
        raise BodoError(saixv__wfboi.format(mod_name, 'ncols'))
    if qhfm__enqcr == 1 and wmriz__ucqp == 1:
        bnos__gmci = types.mpl_axes_type
    else:
        if wmriz__ucqp == 1:
            rat__syhh = types.mpl_axes_type
        else:
            rat__syhh = types.Tuple([types.mpl_axes_type] * wmriz__ucqp)
        bnos__gmci = types.Tuple([rat__syhh] * qhfm__enqcr)
    return bnos__gmci


def generate_pie_return_type(args, kws):
    vrhj__llnty = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if vrhj__llnty == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    nva__pwthv = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(nva__pwthv):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(nva__pwthv):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        trl__xhz = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, trl__xhz]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    trl__xhz = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols)
    qmc__zuhbu = str(trl__xhz)
    if not hasattr(types, qmc__zuhbu):
        qmc__zuhbu = f'objmode_type{ir_utils.next_label()}'
        setattr(types, qmc__zuhbu, trl__xhz)
    iyq__pih = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        ...
        with numba.objmode(axes="{qmc__zuhbu}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    zkd__jpt = {}
    exec(iyq__pih, {'numba': numba, 'np': np}, zkd__jpt)
    impl = zkd__jpt['impl']
    return impl
