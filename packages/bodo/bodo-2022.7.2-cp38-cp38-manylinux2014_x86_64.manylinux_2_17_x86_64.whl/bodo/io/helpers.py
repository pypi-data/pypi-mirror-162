...
def lower_pyarrow_table_schema(context, builder, ty, pyval):
    rckx__gyxgi = context.get_python_api(builder)
    return rckx__gyxgi.unserialize(rckx__gyxgi.serialize_object(pyval))
...
def get_arrow_timestamp_type(pa_ts_typ):
    uatq__tsd = 'ns', 'us', 'ms', 's'
    if pa_ts_typ.unit not in uatq__tsd:
        return types.Array(bodo.datetime64ns, 1, 'C'), False
    elif pa_ts_typ.tz is not None:
        gkqc__woxy = pa_ts_typ.to_pandas_dtype().tz
        agpob__zfrt = bodo.libs.pd_datetime_arr_ext.get_pytz_type_info(
            gkqc__woxy)
        return bodo.DatetimeArrayType(agpob__zfrt), True
    else:
...
def _get_numba_typ_from_pa_typ(pa_typ: pa.Field, is_index,
    ...
    if isinstance(pa_typ.type, pa.ListType):
        jxyw__ojzf, tbeww__yiwgl = _get_numba_typ_from_pa_typ(pa_typ.type.
            value_field, is_index, nullable_from_metadata, category_info)
        return ArrayItemArrayType(jxyw__ojzf), tbeww__yiwgl
    if isinstance(pa_typ.type, pa.StructType):
        owwg__asrtx = []
        pyn__wqn = []
        tbeww__yiwgl = True
        for lxxgi__prnae in pa_typ.flatten():
            pyn__wqn.append(lxxgi__prnae.name.split('.')[-1])
            cqton__dyd, brwgt__xon = _get_numba_typ_from_pa_typ(lxxgi__prnae,
                is_index, nullable_from_metadata, category_info)
            owwg__asrtx.append(cqton__dyd)
            tbeww__yiwgl = tbeww__yiwgl and brwgt__xon
        return StructArrayType(tuple(owwg__asrtx), tuple(pyn__wqn)
            ), tbeww__yiwgl
    ...
    if isinstance(pa_typ.type, pa.DictionaryType):
        ...
        jzesy__mxo = _pyarrow_numba_type_map[pa_typ.type.index_type]
        kwplj__tggu = PDCategoricalDtype(category_info[pa_typ.name], bodo.
            string_type, pa_typ.type.ordered, int_type=jzesy__mxo)
        return CategoricalArrayType(kwplj__tggu), True
    if isinstance(pa_typ.type, pa.lib.TimestampType):
        ...
        rjap__auz = _pyarrow_numba_type_map[pa_typ.type]
        tbeww__yiwgl = True
    else:
        raise BodoError('Arrow data type {} not supported yet'.format(
            pa_typ.type))
    if rjap__auz == datetime_date_type:
        return datetime_date_array_type, tbeww__yiwgl
    if rjap__auz == bytes_type:
        return binary_array_type, tbeww__yiwgl
    jxyw__ojzf = (string_array_type if rjap__auz == string_type else types.
        Array(rjap__auz, 1, 'C'))
    if rjap__auz == types.bool_:
        jxyw__ojzf = boolean_array
    crep__zqpw = (use_nullable_int_arr if nullable_from_metadata is None else
        nullable_from_metadata)
    if crep__zqpw and not is_index and isinstance(rjap__auz, types.Integer
        ) and pa_typ.nullable:
        jxyw__ojzf = IntegerArrayType(rjap__auz)
    return jxyw__ojzf, tbeww__yiwgl
