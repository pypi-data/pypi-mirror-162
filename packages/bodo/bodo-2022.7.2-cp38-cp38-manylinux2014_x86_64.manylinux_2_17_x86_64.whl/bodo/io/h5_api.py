...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        iouhe__ocbwq = []
        sgjfv__otwh = h5g_get_num_objs(obj_id)
        for cvu__sbbt in range(sgjfv__otwh):
            ojjii__zptu = h5g_get_objname_by_idx(obj_id, cvu__sbbt)
            iouhe__ocbwq.append(ojjii__zptu)
        return iouhe__ocbwq
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    nojo__lxgtv = parse_dtype(dtype)
    njvbe__tko = np.int32(numba_to_c_type(nojo__lxgtv))
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            xdyjq__oezr = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    eol__tngc = tuple_to_ptr(starts)
    lucjh__eeyw = tuple_to_ptr(counts)
    otc__exfx = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, eol__tngc, lucjh__eeyw,
        is_parallel, out_arr.ctypes, otc__exfx)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    lucjh__eeyw = tuple_to_ptr(counts)
    otc__exfx = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, eol__tngc, lucjh__eeyw,
        is_parallel, out_arr.ctypes, otc__exfx)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        qzi__uhpf = types.Array(dtype, ndim, 'C')
        negj__xwj = args[3]
        qzi__uhpf = self.context.resolve_function_type(operator.getitem, [
            qzi__uhpf, negj__xwj], {}).return_type.copy(layout='C')
        return signature(qzi__uhpf, *args)
...
def get_filter_read_indices(bool_arr):
    wddyq__vlde = bool_arr.nonzero()[0]
    kdrfa__jzf = bodo.libs.distributed_api.get_rank()
    ivmrn__wpt = bodo.libs.distributed_api.get_size()
    gzwzu__cehga = np.empty(ivmrn__wpt, np.int64)
    oxf__hkf = len(bool_arr)
    bodo.libs.distributed_api.allgather(gzwzu__cehga, oxf__hkf)
    bbx__nbr = gzwzu__cehga.cumsum()[kdrfa__jzf] - oxf__hkf
    wddyq__vlde += bbx__nbr
    qfv__wil = bodo.libs.distributed_api.dist_reduce(len(wddyq__vlde), np.
        int32(sum_op))
    esjkf__mbeb = bodo.libs.distributed_api.gatherv(wddyq__vlde)
    if kdrfa__jzf == 0:
        bakh__oakl = esjkf__mbeb
    else:
        bakh__oakl = np.empty(qfv__wil, wddyq__vlde.dtype)
    bodo.libs.distributed_api.bcast(bakh__oakl)
    rwwn__oncae = bodo.libs.distributed_api.get_start(qfv__wil, ivmrn__wpt,
        kdrfa__jzf)
    zwv__hbp = bodo.libs.distributed_api.get_end(qfv__wil, ivmrn__wpt,
        kdrfa__jzf)
    return bakh__oakl[rwwn__oncae:zwv__hbp]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        zdthn__emve = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], zdthn__emve)
        return builder.bitcast(zdthn__emve, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    lucjh__eeyw = tuple_to_ptr(counts)
    otc__exfx = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, eol__tngc,
        lucjh__eeyw, is_parallel, out_arr.ctypes, otc__exfx, read_indices.
