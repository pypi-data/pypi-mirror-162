...
class PandasTimestampType(types.Type):
    ...
    def __init__(self, tz_val=None):
        self.tz = tz_val
        if tz_val is None:
            gnh__peh = 'PandasTimestampType()'
        else:
            gnh__peh = f'PandasTimestampType({tz_val})'
        super(PandasTimestampType, self).__init__(name=gnh__peh)
...
def check_tz_aware_unsupported(val, func_name):
    ...
    if isinstance(val, PandasTimestampType) and val.tz is not None:
        ...
        for wiym__srbrm in val.data:
            if isinstance(wiym__srbrm, bodo.DatetimeArrayType):
                raise BodoError(
...
class PandasTimestampModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        gbgiy__uwg = [('year', ts_field_typ), ('month', ts_field_typ), (
            ...
            ts_field_typ)]
        models.StructModel.__init__(self, dmm, fe_type, gbgiy__uwg)
...
def unbox_pandas_timestamp(typ, val, c):
    tlqac__hpxo = c.pyapi.object_getattr_string(val, 'year')
    mdd__eagjh = c.pyapi.object_getattr_string(val, 'month')
    azmn__fcpw = c.pyapi.object_getattr_string(val, 'day')
    jctx__uxy = c.pyapi.object_getattr_string(val, 'hour')
    ycxkn__llfcl = c.pyapi.object_getattr_string(val, 'minute')
    wxe__jzou = c.pyapi.object_getattr_string(val, 'second')
    oze__qxrj = c.pyapi.object_getattr_string(val, 'microsecond')
    wrk__liw = c.pyapi.object_getattr_string(val, 'nanosecond')
    yiihb__giu = c.pyapi.object_getattr_string(val, 'value')
    bup__cjbpy = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    bup__cjbpy.year = c.pyapi.long_as_longlong(tlqac__hpxo)
    bup__cjbpy.month = c.pyapi.long_as_longlong(mdd__eagjh)
    bup__cjbpy.day = c.pyapi.long_as_longlong(azmn__fcpw)
    bup__cjbpy.hour = c.pyapi.long_as_longlong(jctx__uxy)
    bup__cjbpy.minute = c.pyapi.long_as_longlong(ycxkn__llfcl)
    bup__cjbpy.second = c.pyapi.long_as_longlong(wxe__jzou)
    bup__cjbpy.microsecond = c.pyapi.long_as_longlong(oze__qxrj)
    bup__cjbpy.nanosecond = c.pyapi.long_as_longlong(wrk__liw)
    bup__cjbpy.value = c.pyapi.long_as_longlong(yiihb__giu)
    c.pyapi.decref(tlqac__hpxo)
    c.pyapi.decref(mdd__eagjh)
    c.pyapi.decref(azmn__fcpw)
    c.pyapi.decref(jctx__uxy)
    c.pyapi.decref(ycxkn__llfcl)
    c.pyapi.decref(wxe__jzou)
    c.pyapi.decref(oze__qxrj)
    c.pyapi.decref(wrk__liw)
    c.pyapi.decref(yiihb__giu)
    ypxil__nnkrm = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(bup__cjbpy._getvalue(), is_error=ypxil__nnkrm)


@box(PandasTimestampType)
def box_pandas_timestamp(typ, val, c):
    ewmk__razh = cgutils.create_struct_proxy(typ)(c.context, c.builder,
        value=val)
    tlqac__hpxo = c.pyapi.long_from_longlong(ewmk__razh.year)
    mdd__eagjh = c.pyapi.long_from_longlong(ewmk__razh.month)
    azmn__fcpw = c.pyapi.long_from_longlong(ewmk__razh.day)
    jctx__uxy = c.pyapi.long_from_longlong(ewmk__razh.hour)
    ycxkn__llfcl = c.pyapi.long_from_longlong(ewmk__razh.minute)
    wxe__jzou = c.pyapi.long_from_longlong(ewmk__razh.second)
    osemp__ouudp = c.pyapi.long_from_longlong(ewmk__razh.microsecond)
    gppt__mdfz = c.pyapi.long_from_longlong(ewmk__razh.nanosecond)
    evquv__idld = c.pyapi.unserialize(c.pyapi.serialize_object(pd.Timestamp))
    if typ.tz is None:
        res = c.pyapi.call_function_objargs(evquv__idld, (tlqac__hpxo,
            mdd__eagjh, azmn__fcpw, jctx__uxy, ycxkn__llfcl, wxe__jzou,
            osemp__ouudp, gppt__mdfz))
    else:
        if isinstance(typ.tz, int):
            ctjk__ptg = c.pyapi.long_from_longlong(lir.Constant(lir.IntType
                (64), typ.tz))
        else:
            abziw__fvl = c.context.insert_const_string(c.builder.module,
                str(typ.tz))
            ctjk__ptg = c.pyapi.string_from_string(abziw__fvl)
        args = c.pyapi.tuple_pack(())
        kwargs = c.pyapi.dict_pack([('year', tlqac__hpxo), ('month',
            mdd__eagjh), ('day', azmn__fcpw), ('hour', jctx__uxy), (
            'minute', ycxkn__llfcl), ('second', wxe__jzou), ('microsecond',
            osemp__ouudp), ('nanosecond', gppt__mdfz), ('tz', ctjk__ptg)])
        res = c.pyapi.call(evquv__idld, args, kwargs)
        c.pyapi.decref(args)
        c.pyapi.decref(kwargs)
        c.pyapi.decref(ctjk__ptg)
    c.pyapi.decref(tlqac__hpxo)
    c.pyapi.decref(mdd__eagjh)
    c.pyapi.decref(azmn__fcpw)
    c.pyapi.decref(jctx__uxy)
    c.pyapi.decref(ycxkn__llfcl)
    c.pyapi.decref(wxe__jzou)
    c.pyapi.decref(osemp__ouudp)
    c.pyapi.decref(gppt__mdfz)
    return res
...
def init_timestamp(typingctx, year, month, day, hour, minute, second,
    ...
    def codegen(context, builder, sig, args):
        (year, month, day, hour, minute, second, pad__uft, yntrb__ekoh,
            value, mol__sjlj) = args
        ...
        ts.minute = minute
        ts.second = second
        ts.microsecond = pad__uft
        ts.nanosecond = yntrb__ekoh
        ts.value = value
        return ts._getvalue()
...
def overload_pd_timestamp(ts_input=_no_input, freq=None, tz=None, unit=None,
    ...
    if isinstance(ts_input, types.Number):
        ...
        kmzts__ffib, precision = (pd._libs.tslibs.conversion.
        ...
        def impl_float(ts_input=_no_input, freq=None, tz=None, unit=None,
            year=None, month=None, day=None, hour=None, minute=None, second
            =None, microsecond=None, nanosecond=None, tzinfo=None):
            ivilr__lgbpa = np.int64(ts_input)
            cqdm__lhu = ts_input - ivilr__lgbpa
            if precision:
                cqdm__lhu = np.round(cqdm__lhu, precision)
            value = ivilr__lgbpa * kmzts__ffib + np.int64(cqdm__lhu *
                kmzts__ffib)
            return convert_val_to_timestamp(value, tz)
        return impl_float
    ...
    if isinstance(ts_input, numba.core.types.scalars.NPDatetime):
        ...
        def impl_date(ts_input=_no_input, freq=None, tz=None, unit=None,
            year=None, month=None, day=None, hour=None, minute=None, second
            =None, microsecond=None, nanosecond=None, tzinfo=None):
            value = np.int64(ts_input) * kmzts__ffib
...
def overload_week_number(ptt):
    ...
    def pd_week_number(ptt):
        mol__sjlj, cbmk__zxm, mol__sjlj = get_isocalendar(ptt.year, ptt.
            month, ptt.day)
        return cbmk__zxm
...
def overload_pd_timestamp_isocalendar(ptt):
    ...
    def impl(ptt):
        year, cbmk__zxm, xnz__xbui = get_isocalendar(ptt.year, ptt.month,
            ptt.day)
        return year, cbmk__zxm, xnz__xbui
    return impl
...
def overload_pd_timestamp_isoformat(ts, sep=None):
    if is_overload_none(sep):
        ...
        def timestamp_isoformat_impl(ts, sep=None):
            ...
            rjmqc__rfxv = str_2d(ts.hour) + ':' + str_2d(ts.minute
                ) + ':' + str_2d(ts.second)
...
def overload_pd_timestamp_day_name(ptt, locale=None):
    hfpun__qezyg = dict(locale=locale)
    xzm__dyk = dict(locale=None)
    check_unsupported_args('Timestamp.day_name', hfpun__qezyg, xzm__dyk,
        package_name='pandas', module_name='Timestamp')

    def impl(ptt, locale=None):
        fha__lmd = ('Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday',
            'Saturday', 'Sunday')
        mol__sjlj, mol__sjlj, mqg__mcw = ptt.isocalendar()
        return fha__lmd[mqg__mcw - 1]
    return impl
...
def overload_pd_timestamp_month_name(ptt, locale=None):
    hfpun__qezyg = dict(locale=locale)
    xzm__dyk = dict(locale=None)
    check_unsupported_args('Timestamp.month_name', hfpun__qezyg, xzm__dyk,
        package_name='pandas', module_name='Timestamp')

    def impl(ptt, locale=None):
        ghvjj__mbvr = ('January', 'February', 'March', 'April', 'May',
            'June', 'July', 'August', 'September', 'October', 'November',
            'December')
        return ghvjj__mbvr[ptt.month - 1]
    return impl
...
def overload_pd_timestamp_tz_localize(ptt, tz, ambiguous='raise',
    ...
    hfpun__qezyg = dict(ambiguous=ambiguous, nonexistent=nonexistent)
    inpa__asrv = dict(ambiguous='raise', nonexistent='raise')
    check_unsupported_args('Timestamp.tz_localize', hfpun__qezyg,
        inpa__asrv, package_name='pandas', module_name='Timestamp')
    if is_overload_none(tz):
...
def extract_year_days(typingctx, dt64_t=None):
    ...
    def codegen(context, builder, sig, args):
        yfnq__vhlqo = cgutils.alloca_once(builder, lir.IntType(64))
        builder.store(args[0], yfnq__vhlqo)
        year = cgutils.alloca_once(builder, lir.IntType(64))
        dvdox__ipr = cgutils.alloca_once(builder, lir.IntType(64))
        gubc__hvg = lir.FunctionType(lir.VoidType(), [lir.IntType(64).
            as_pointer(), lir.IntType(64).as_pointer(), lir.IntType(64).
            as_pointer()])
        wzyts__rdi = cgutils.get_or_insert_function(builder.module,
            gubc__hvg, name='extract_year_days')
        builder.call(wzyts__rdi, [yfnq__vhlqo, year, dvdox__ipr])
        return cgutils.pack_array(builder, [builder.load(yfnq__vhlqo),
            builder.load(year), builder.load(dvdox__ipr)])
...
def get_month_day(typingctx, year_t, days_t=None):
    ...
    def codegen(context, builder, sig, args):
        ...
        gubc__hvg = lir.FunctionType(lir.VoidType(), [lir.IntType(64), lir.
            IntType(64), lir.IntType(64).as_pointer(), lir.IntType(64).
            as_pointer()])
        wzyts__rdi = cgutils.get_or_insert_function(builder.module,
            gubc__hvg, name='get_month_day')
        builder.call(wzyts__rdi, [args[0], args[1], month, day])
...
def get_day_of_year(year, month, day):
    fend__htkz = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, 
        365, 0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335, 366]
    vrfwj__wzji = is_leap_year(year)
    gheo__lyk = fend__htkz[vrfwj__wzji * 13 + month - 1]
    qhtt__mloru = gheo__lyk + day
    return qhtt__mloru


@register_jitable
def get_day_of_week(y, m, d):
    eos__qysfz = [0, 3, 2, 5, 0, 3, 5, 1, 4, 6, 2, 4]
    y -= m < 3
    day = (y + y // 4 - y // 100 + y // 400 + eos__qysfz[m - 1] + d) % 7
    return (day + 6) % 7
...
def get_days_in_month(year, month):
    ...
    qhx__usgm = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31, 31, 29, 31,
        30, 31, 30, 31, 31, 30, 31, 30, 31]
    return qhx__usgm[12 * is_leap_year + month - 1]
...
def convert_val_to_timestamp(ts_input, tz=None, is_convert=True):
    uekmg__ejjp = dchut__xfuh = np.array([])
    vuzte__occbm = '0'
    if is_overload_constant_str(tz):
        abziw__fvl = get_overload_const_str(tz)
        ctjk__ptg = pytz.timezone(abziw__fvl)
        if isinstance(ctjk__ptg, pytz.tzinfo.DstTzInfo):
            uekmg__ejjp = np.array(ctjk__ptg._utc_transition_times, dtype=
                'M8[ns]').view('i8')
            dchut__xfuh = np.array(ctjk__ptg._transition_info)[:, 0]
            dchut__xfuh = (pd.Series(dchut__xfuh).dt.total_seconds() * 
                1000000000).astype(np.int64).values
            vuzte__occbm = (
                "deltas[np.searchsorted(trans, ts_input, side='right') - 1]")
        else:
            dchut__xfuh = np.int64(ctjk__ptg._utcoffset.total_seconds() * 
                1000000000)
            vuzte__occbm = 'deltas'
    elif is_overload_constant_int(tz):
        gzh__egl = get_overload_const_int(tz)
        vuzte__occbm = str(gzh__egl)
    ...
    if is_convert:
        keplm__qsxb = 'tz_ts_input'
        ucqxm__frr = 'ts_input'
    else:
        keplm__qsxb = 'ts_input'
        ucqxm__frr = 'tz_ts_input'
    cmg__araxs = 'def impl(ts_input, tz=None, is_convert=True):\n'
    cmg__araxs += f'  tz_ts_input = ts_input + {vuzte__occbm}\n'
    cmg__araxs += (
        f'  dt, year, days = extract_year_days(integer_to_dt64({keplm__qsxb}))\n'
        )
    cmg__araxs += '  month, day = get_month_day(year, days)\n'
    cmg__araxs += '  return init_timestamp(\n'
    cmg__araxs += '    year=year,\n'
    cmg__araxs += '    month=month,\n'
    cmg__araxs += '    day=day,\n'
    cmg__araxs += '    hour=dt // (60 * 60 * 1_000_000_000),\n'
    cmg__araxs += '    minute=(dt // (60 * 1_000_000_000)) % 60,\n'
    cmg__araxs += '    second=(dt // 1_000_000_000) % 60,\n'
    cmg__araxs += '    microsecond=(dt // 1000) % 1_000_000,\n'
    cmg__araxs += '    nanosecond=dt % 1000,\n'
    cmg__araxs += f'    value={ucqxm__frr},\n'
    cmg__araxs += '    tz=tz,\n'
    cmg__araxs += '  )\n'
    rclt__vpix = {}
    exec(cmg__araxs, {'np': np, 'pd': pd, 'trans': uekmg__ejjp, 'deltas':
        dchut__xfuh, 'integer_to_dt64': integer_to_dt64,
        'extract_year_days': extract_year_days, 'get_month_day':
        get_month_day, 'init_timestamp': init_timestamp, 'zero_if_none':
        zero_if_none}, rclt__vpix)
    impl = rclt__vpix['impl']
    return impl


@numba.njit(no_cpython_wrapper=True)
def convert_datetime64_to_timestamp(dt64):
    yfnq__vhlqo, year, dvdox__ipr = extract_year_days(dt64)
    month, day = get_month_day(year, dvdox__ipr)
    return init_timestamp(year=year, month=month, day=day, hour=yfnq__vhlqo //
        (60 * 60 * 1000000000), minute=yfnq__vhlqo // (60 * 1000000000) % 
        60, second=yfnq__vhlqo // 1000000000 % 60, microsecond=yfnq__vhlqo //
        1000 % 1000000, nanosecond=yfnq__vhlqo % 1000, value=dt64, tz=None)


@numba.njit(no_cpython_wrapper=True)
def convert_numpy_timedelta64_to_datetime_timedelta(dt64):
    uwhti__eex = (bodo.hiframes.datetime_timedelta_ext.
        cast_numpy_timedelta_to_int(dt64))
    pwj__twlv = uwhti__eex // (86400 * 1000000000)
    wrna__vzrg = uwhti__eex - pwj__twlv * 86400 * 1000000000
    sfvho__bskt = wrna__vzrg // 1000000000
    zgilp__pcsj = wrna__vzrg - sfvho__bskt * 1000000000
    qlb__ivvw = zgilp__pcsj // 1000
    return datetime.timedelta(pwj__twlv, sfvho__bskt, qlb__ivvw)
...
def convert_numpy_timedelta64_to_pd_timedelta(dt64):
    uwhti__eex = (bodo.hiframes.datetime_timedelta_ext.
        cast_numpy_timedelta_to_int(dt64))
    return pd.Timedelta(uwhti__eex)
...
def pandas_dict_string_array_to_datetime(arr, errors, dayfirst, yearfirst,
    ...
    dpri__yky = len(arr)
    ujh__ycvq = np.empty(dpri__yky, 'datetime64[ns]')
    wxd__fqdv = arr._indices
    jacf__lwcm = pandas_string_array_to_datetime(arr._data, errors,
        dayfirst, yearfirst, utc, format, exact, unit,
        infer_datetime_format, origin, cache).values
    for fkpms__khph in range(dpri__yky):
        if bodo.libs.array_kernels.isna(wxd__fqdv, fkpms__khph):
            bodo.libs.array_kernels.setna(ujh__ycvq, fkpms__khph)
            continue
        ujh__ycvq[fkpms__khph] = jacf__lwcm[wxd__fqdv[fkpms__khph]]
    return ujh__ycvq
...
def overload_to_datetime(arg_a, errors='raise', dayfirst=False, yearfirst=
    ...
    if isinstance(arg_a, bodo.hiframes.pd_series_ext.SeriesType):
        ...
        def impl_series(arg_a, errors='raise', dayfirst=False, yearfirst=
            ...
            ofhx__uclm = bodo.hiframes.pd_series_ext.get_series_index(arg_a)
            gnh__peh = bodo.hiframes.pd_series_ext.get_series_name(arg_a)
            iwsa__wan = bodo.utils.conversion.coerce_to_ndarray(pd.
            ...
            return bodo.hiframes.pd_series_ext.init_series(iwsa__wan,
                ofhx__uclm, gnh__peh)
        return impl_series
    if arg_a == bodo.hiframes.datetime_date_ext.datetime_date_array_type:
        pxywb__ydbmv = np.dtype('datetime64[ns]')
        iNaT = pd._libs.tslibs.iNaT
        ...
        def impl_date_arr(arg_a, errors='raise', dayfirst=False, yearfirst=
            ...
            infer_datetime_format=False, origin='unix', cache=True):
            dpri__yky = len(arg_a)
            ujh__ycvq = np.empty(dpri__yky, pxywb__ydbmv)
            for fkpms__khph in numba.parfors.parfor.internal_prange(dpri__yky):
                val = iNaT
                if not bodo.libs.array_kernels.isna(arg_a, fkpms__khph):
                    data = arg_a[fkpms__khph]
                    val = (bodo.hiframes.pd_timestamp_ext.
                        npy_datetimestruct_to_datetime(data.year, data.
                        month, data.day, 0, 0, 0, 0))
                ujh__ycvq[fkpms__khph
                    ] = bodo.hiframes.pd_timestamp_ext.integer_to_dt64(val)
            return bodo.hiframes.pd_index_ext.init_datetime_index(ujh__ycvq,
                None)
    ...
    if isinstance(arg_a, CategoricalArrayType
        ...
        def impl_cat_arr(arg_a, errors='raise', dayfirst=False, yearfirst=
            ...
            dpri__yky = len(arg_a)
            ujh__ycvq = np.empty(dpri__yky, pxywb__ydbmv)
            phjve__qaak = (bodo.hiframes.pd_categorical_ext.
                get_categorical_arr_codes(arg_a))
            jacf__lwcm = pandas_string_array_to_datetime(arg_a.dtype.
                categories.values, errors, dayfirst, yearfirst, utc, format,
                exact, unit, infer_datetime_format, origin, cache).values
            for fkpms__khph in numba.parfors.parfor.internal_prange(dpri__yky):
                c = phjve__qaak[fkpms__khph]
                if c == -1:
                    bodo.libs.array_kernels.setna(ujh__ycvq, fkpms__khph)
                    continue
                ujh__ycvq[fkpms__khph] = jacf__lwcm[c]
            return bodo.hiframes.pd_index_ext.init_datetime_index(ujh__ycvq,
                None)
        return impl_cat_arr
    if arg_a == bodo.dict_str_arr_type:
        ...
        def impl_dict_str_arr(arg_a, errors='raise', dayfirst=False,
            yearfirst=False, utc=None, format=None, exact=True, unit=None,
            infer_datetime_format=False, origin='unix', cache=True):
            ujh__ycvq = pandas_dict_string_array_to_datetime(arg_a, errors,
                dayfirst, yearfirst, utc, format, exact, unit,
...
def overload_to_timedelta(arg_a, unit='ns', errors='raise'):
    ...
    if isinstance(arg_a, types.Float):
        m, emh__yri = pd._libs.tslibs.conversion.precision_from_unit(unit)

        def impl_float_scalar(arg_a, unit='ns', errors='raise'):
            val = float_to_timedelta_val(arg_a, emh__yri, m)
            return pd.Timedelta(val)
        return impl_float_scalar
    if isinstance(arg_a, types.Integer):
        m, mol__sjlj = pd._libs.tslibs.conversion.precision_from_unit(unit)
    ...
    if is_iterable_type(arg_a) and not isinstance(arg_a, types.BaseTuple):
        m, emh__yri = pd._libs.tslibs.conversion.precision_from_unit(unit)
        jdaki__zfkim = np.dtype('timedelta64[ns]')
        if isinstance(arg_a.dtype, types.Float):

            def impl_float(arg_a, unit='ns', errors='raise'):
                dpri__yky = len(arg_a)
                ujh__ycvq = np.empty(dpri__yky, jdaki__zfkim)
                for fkpms__khph in numba.parfors.parfor.internal_prange(
                    dpri__yky):
                    val = iNaT
                    if not bodo.libs.array_kernels.isna(arg_a, fkpms__khph):
                        val = float_to_timedelta_val(arg_a[fkpms__khph],
                            emh__yri, m)
                    ujh__ycvq[fkpms__khph
        ...
        if arg_a.dtype == datetime_timedelta_type:
            ...
            def impl_datetime_timedelta(arg_a, unit='ns', errors='raise'):
                ...
                for fkpms__khph in numba.parfors.parfor.internal_prange(
                    ...
                    val = iNaT
                    if not bodo.libs.array_kernels.isna(arg_a, fkpms__khph):
                        rdnc__hii = arg_a[fkpms__khph]
                        val = (rdnc__hii.microseconds + 1000 * 1000 * (
                            rdnc__hii.seconds + 24 * 60 * 60 * rdnc__hii.days)
                            ) * 1000
                    ujh__ycvq[fkpms__khph
                        ] = bodo.hiframes.pd_timestamp_ext.integer_to_timedelta64(
                        val)
...
def float_to_timedelta_val(data, precision, multiplier):
    ivilr__lgbpa = np.int64(data)
    cqdm__lhu = data - ivilr__lgbpa
    if precision:
        cqdm__lhu = np.round(cqdm__lhu, precision)
    return ivilr__lgbpa * multiplier + np.int64(cqdm__lhu * multiplier)
...
def overload_freq_methods(method):
    ...
    def freq_overload(td, freq, ambiguous='raise', nonexistent='raise'):
        ...
        hfpun__qezyg = dict(ambiguous=ambiguous, nonexistent=nonexistent)
        rdq__jzte = dict(ambiguous='raise', nonexistent='raise')
        check_unsupported_args(f'Timestamp.{method}', hfpun__qezyg,
            rdq__jzte, package_name='pandas', module_name='Timestamp')
        oasx__lbrm = ["freq == 'D'", "freq == 'H'",
            "freq == 'min' or freq == 'T'", "freq == 'S'",
            "freq == 'ms' or freq == 'L'", "freq == 'U' or freq == 'us'",
            "freq == 'N'"]
        brh__qlyz = [24 * 60 * 60 * 1000000 * 1000, 60 * 60 * 1000000 * 
            1000, 60 * 1000000 * 1000, 1000000 * 1000, 1000 * 1000, 1000, 1]
        cmg__araxs = (
            "def impl(td, freq, ambiguous='raise', nonexistent='raise'):\n")
        for fkpms__khph, tearg__dnds in enumerate(oasx__lbrm):
            yvy__ojyiz = 'if' if fkpms__khph == 0 else 'elif'
            cmg__araxs += '    {} {}:\n'.format(yvy__ojyiz, tearg__dnds)
            cmg__araxs += '        unit_value = {}\n'.format(brh__qlyz[
                fkpms__khph])
        cmg__araxs += '    else:\n'
        cmg__araxs += (
...
"""
            ...
            if method == 'round':
                cmg__araxs += '    if unit_value == 1:\n'
                cmg__araxs += '        value = td.value\n'
                cmg__araxs += '    else:\n'
                cmg__araxs += (
                    '        quotient, remainder = np.divmod(td.value, unit_value)\n'
                    )
                cmg__araxs += """        mask = np.logical_or(remainder > (unit_value // 2), np.logical_and(remainder == (unit_value // 2), quotient % 2))
"""
                cmg__araxs += '        if mask:\n'
                cmg__araxs += '            quotient = quotient + 1\n'
                cmg__araxs += '        value = quotient * unit_value\n'
            cmg__araxs += '    return pd.Timestamp(value)\n'
        rclt__vpix = {}
        exec(cmg__araxs, {'np': np, 'pd': pd}, rclt__vpix)
        impl = rclt__vpix['impl']
        return impl
...
def _install_freq_methods():
    xyb__cewp = ['ceil', 'floor', 'round']
    for method in xyb__cewp:
        rvu__aoq = overload_freq_methods(method)
        overload_method(PDTimeDeltaType, method, no_unliteral=True)(rvu__aoq)
...
def compute_pd_timestamp(totmicrosec, nanosecond):
    ...
    zxxld__psqy = totmicrosec // 1000000
    second = zxxld__psqy % 60
    ngp__vykw = zxxld__psqy // 60
    minute = ngp__vykw % 60
    ezskl__bnxh = ngp__vykw // 60
    hour = ezskl__bnxh % 24
    ojbuf__zxel = ezskl__bnxh // 24
    year, month, day = _ord2ymd(ojbuf__zxel)
...
def overload_sub_operator_timestamp(lhs, rhs):
    if lhs == pd_timestamp_type and rhs == datetime_timedelta_type:
        ...
        def impl(lhs, rhs):
            rla__pja = lhs.toordinal()
            biw__dwbo = lhs.second + lhs.minute * 60 + lhs.hour * 3600
            hplpj__wuk = lhs.microsecond
            nanosecond = lhs.nanosecond
            kmuhs__fflwx = rhs.days
            ucp__ekit = rhs.seconds
            vaehd__hktn = rhs.microseconds
            eoocj__jjiz = rla__pja - kmuhs__fflwx
            pqe__sbhcd = biw__dwbo - ucp__ekit
            kir__zqi = hplpj__wuk - vaehd__hktn
            totmicrosec = 1000000 * (eoocj__jjiz * 86400 + pqe__sbhcd
                ) + kir__zqi
...
def overload_add_operator_timestamp(lhs, rhs):
    ...
    if lhs == pd_timestamp_type and rhs == pd_timedelta_type:
        ...
        def impl(lhs, rhs):
            ...
            biw__dwbo = lhs.second + lhs.minute * 60 + lhs.hour * 3600
            hplpj__wuk = lhs.microsecond
            jtc__lhbs = lhs.nanosecond
            vaehd__hktn = rhs.value // 1000
            jpvfw__tvban = rhs.nanoseconds
            kir__zqi = hplpj__wuk + vaehd__hktn
            totmicrosec = 1000000 * (rla__pja * 86400 + biw__dwbo) + kir__zqi
            hcwx__dfden = jtc__lhbs + jpvfw__tvban
            return compute_pd_timestamp(totmicrosec, hcwx__dfden)
        return impl
...
def strftime(ts, format):
    if isinstance(ts, DatetimeDateType):
        jdf__kqr = 'datetime.date'
    else:
        jdf__kqr = 'pandas.Timestamp'
    if types.unliteral(format) != types.unicode_type:
        raise BodoError(
            f"{jdf__kqr}.strftime(): 'strftime' argument must be a string")
...
def typeof_python_calendar(val, c):
    return types.Tuple([types.StringLiteral(rrvl__tys) for rrvl__tys in val])
...
def _install_pd_timestamp_unsupported():
    ...
    for jeiw__ghdw in timestamp_unsupported_attrs:
        zgx__iupv = 'pandas.Timestamp.' + jeiw__ghdw
        overload_attribute(PandasTimestampType, jeiw__ghdw)(
            create_unsupported_overload(zgx__iupv))
    for xul__kup in timestamp_unsupported_methods:
        zgx__iupv = 'pandas.Timestamp.' + xul__kup
        overload_method(PandasTimestampType, xul__kup)(
            create_unsupported_overload(zgx__iupv + '()'))
