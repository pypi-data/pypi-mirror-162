...
class Indexer (daemon_class.DaemonClass):
	...
	def __init__ (self, logpath, varpath, config):
		self.config = config
		...
		self.working = {}
		self.ts = []
		daemon_class.DaemonClass.__init__ (self, logpath, varpath)
...
def main ():
	cmd, config = parse_argv ("dv:o:t:i:e:", ["exclude=", "origin=", "threads=", "interval=", "resource-dir=", "help"], usage)
	...
	if cmd and not servicer.execute (cmd):
		return
	if cmd and servicer.status (False):
		return

	s = Indexer (LOGPATH, VARPATH, config)
	s.start ()
