...
def execute_mwaa_cli_command(mwaa_cli_token: Dict[str, Any], mwaa_cli_command: str):
    ...
    log.info("RCVD output: {}".format(mwaa_response))
    ...
    if mwaa_status_code != requests.codes.ok:
...
def run():
    ...
    parser.add_argument("-p", "--environment-prefix", type=str, help="the mwaa environment name prefix",
                        default="dataplatform-mwaa")
    ...
    trigger_dag_subparser.add_argument("-n", "--dag-name", type=str, help="airflow DAG name",
                                       default="import_variables_hourly_v1")
    ...
    mwaa_environment = "{}-{}".format(environment_prefix, environment)
    ...
    if subcommand == "trigger-dag":
        ...
        mwaa_cli_command = f'dags trigger {dag_name}'
