...
def _run_operation(cycle: dict):
    def exec(operation: dict):
        ...
        debugValues(cycle, model=MODEL, term=input_term_id,
                    ...
                    value=value,
                    coeff=coeff)
        logShouldRun(cycle, MODEL, input_term_id, True, key=KEY, operation=operation.get('@id'))
...
def _should_run_operation(cycle: dict):
    def exec(practice: dict):
        ...
        coeffs = get_lookup_value(term, LOOKUPS['operation'], model=MODEL, key=KEY)
        ...
        logShouldRun(cycle, MODEL, term.get('@id'), should_run, key=KEY)
        return [{
            **practice,
            'input': input
