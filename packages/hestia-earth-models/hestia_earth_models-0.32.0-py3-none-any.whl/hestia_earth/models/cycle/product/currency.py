...
def _run_with_revenue(product: dict, price: float):
    logShouldRun(product, MODEL, product.get('term', {}).get('@id'), True, key='revenue')
    logShouldRun(product, MODEL, product.get('term', {}).get('@id'), True, key='price')
...
def _run_product(date: str):
    def run(product: dict):
        ...
        return None if price is None else _run_with_revenue(product, price)
    return run


def _product_currency_not_default(product: dict): return product.get('currency', '') != DEFAULT_CURRENCY
