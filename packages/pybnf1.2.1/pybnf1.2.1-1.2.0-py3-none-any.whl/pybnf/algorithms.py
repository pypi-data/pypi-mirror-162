...
from .juliaMod import juliaModel
...
#Will need to be taken out when moving to parallel computing
from julia import Distributed as ds
...
class Algorithm(object):
    ...
    def __init__(self, config):
        ...
        if self.config.config['useJulia'] == 'True':
    ...
    def _initialize_models(self):
        ...
        for m in init_model_list:
            if isinstance(m, BNGLModel) and m.generates_network:
                ...
                print('Appending Final Model List')
                if self.config.config['useJulia'] == 'True':
                    print('Initalizing Julia Model')
                    netStr = ''.join(final_model_list[-1].netfile_lines)
                    newModel = juliaModel(netStr, m.name, m.bngl_file_text, self.config.config['objfunc'], self.config)
                    juliaList.append(newModel.juliaODE())
                    juliaModelList.append(m.name)
        ...
        if self.config.config['useJulia'] == 'True':
            jModel = '\n'.join(juliaList)
            jFunctions = newModel.juliaFunctions(juliaModelList)
            jDepend = newModel.juliaDependancies()
            jOutList = [jDepend, jModel, jFunctions]
            jOutStr = '\n'.join(jOutList)
            print2('Generating jl file')
            self.jlFile = newModel.saveJuliaFile(jOutStr, init_dir)
            return self.jlFile
    ...
    def run(self, client, resume=None, debug=False):
        ...
        if self.config.config['useJulia'] == 'False':
            if self.refine:
            ...
            print2('Initilizing Julia Model')
            # Modify the exp columms maybe move this to pset?
            for i in self.exp_data:
                for j in self.exp_data[i]:
                    for k,v in self.exp_data[i][j].cols.items():
                        self.exp_data[i][j].cols[k] = 1 + v
            jl.include(self.model_list)
            if self.refine:
                logger.debug('Setting up Simplex refinement of previous algorithm')
            ...
            julPset = []
            for p in psets:
                parameters = {x.name:x.value for x in p}
                jobs.append(jl.allProbs(parameters))
                expJobs.append(self.exp_data)
                julPset.append(p)
            logger.info('Submitting initial set of %d Jobs' % len(jobs))
            runFunction = jl.runAll
            futures = ds.pmap(runFunction, [jobs], [julPset], [expJobs])
            pool = iter(futures[0])
            backed_up = True
            while True:
                res = next(pool)
                ...
                if response == 'STOP':
                    ...
                    newPset = []
                    for i,ps in enumerate(response):
                        parameters = {x.name:x.value for x in ps}
                        jl.solveOdeNewPars(jobs[i],parameters)
                        newPset.append(ps)
                    if bool(response):
                        newJ = ds.pmap(runFunction, [jobs], [newPset], [expJobs])
                        logger.debug('Submitting %d new Jobs' % len(newPset))
                        pool = iter(newJ[0])
