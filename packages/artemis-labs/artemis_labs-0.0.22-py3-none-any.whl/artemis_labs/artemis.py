    ...
    def callback_handler(self, message):        
        ...
        if message['type'] != 'ping':
            ...
            if message['type'] == 'query':
                ...
                if self.dev:
                    dev_arg = ' dev'
                print([self.launch_command , self.runner_path, self.code_path, self.launch_command , dev_arg, 'nolaunch'])
                subprocess.Popen([self.launch_command , self.runner_path, self.code_path, self.launch_command , dev_arg, 'nolaunch'], creationflags=subprocess.CREATE_NO_WINDOW)
                os._exit(1)
            else:
