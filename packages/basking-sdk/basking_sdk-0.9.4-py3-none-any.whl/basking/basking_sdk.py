...
from .constant import (
    BASKING_API_URL_PROD,
    BASKING_API_URL_LOCAL,
    TIMEOUT_MESSAGE
)
...
class Basking:
    ...
    def __init__(
            self,
            auth_type='env_variables',
            api_stage='prod'
    ):
        valid_api_stages = ['prod', 'local']
        if api_stage not in valid_api_stages:
            raise ValueError(
                f'''{api_stage} not valid. Valid options: {valid_api_stages} '''
            )
        if api_stage == 'prod':
            self.basking_api_url = BASKING_API_URL_PROD
        elif api_stage == 'local':
            self.basking_api_url = BASKING_API_URL_LOCAL
        else:
            raise ValueError('cannot set basking_api_url')
        ...
        # cognito
        self._aws_region = self.auth_type = auth_type
        config = Config(signature_version=botocore.UNSIGNED)
        self._boto3_client = boto3.client('cognito-idp', self._aws_region, config=config)
        #   occupancy object
    ...
    def date_obj_to_timestamp_ms(self, date_obj, tz_str):
        ...
        try:
            ...
            self.log.debug(f"error in data_obj_timestamp -- > {e}")
    ...
    def _cognito_login_password_auth_(self):
        """
        ...
        response = self._boto3_client.initiate_auth(
            ...
            ClientId='7o5vqv7ukt9onolq1nvjt7m40e'  # add the dashboard auth.
        )
    ...
    def get_new_cognito_token_(self):
        """
        Internal method to get new cognito token

        :return: return new token
        """

        # return cognito_login()
        return self._cognito_login_password_auth_(
