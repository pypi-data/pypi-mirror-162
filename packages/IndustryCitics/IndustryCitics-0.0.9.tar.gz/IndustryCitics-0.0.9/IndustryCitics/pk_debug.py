...
def generate_report():
	...
	lastest_date = str(ret_industries['日期'].max())[:10]
	...
	data = \
	roe.groupby(['报告期','INDEX_NAME'])[['ROE','i_weight']]\
	.apply(lambda s:np.sum(s['ROE'] * s['i_weight'])/np.sum(s['i_weight']) * 100)\
	...
	fig.suptitle('更新日期：' + lastest_date, fontsize = 26, x=0.5, y=0.9)
	plt.savefig(lastest_date+':行业收益率与估值.png',dpi=256)
	plt.show()
	...
	est_trend(index_consensus, metric_names = ['一致预期净利润(FY1)','一致预期净利润(FY2)'],
		  display_name = '滑动一致预期净利润', axis_label = '净利润（亿元）', unit=1e8, prefix=lastest_date+':')

	# ROE一致预期
	print('*'* 40 + 'ROE一致预期' + '*' *40)
	est_trend(index_consensus, metric_names = ['一致预期ROE(FY1)','一致预期ROE(FY2)'],
		  display_name = '滑动一致预期ROE', axis_label = 'ROE(%)', prefix=lastest_date+':')
...
def est_trend(index_consensus, metric_names, display_name, axis_label, prefix = '', unit=1):
	...
	for i, ind in enumerate(industries_sort):
		...
		axes[i].grid(axis='y')
		axes[i].set_title(ind, fontsize=24)
		axes[i].set_xlabel('')
		axes[i].set_ylabel(axis_label)
	fig.tight_layout()
	plt.savefig(prefix+display_name+'.png',dpi = 256)
	plt.show()