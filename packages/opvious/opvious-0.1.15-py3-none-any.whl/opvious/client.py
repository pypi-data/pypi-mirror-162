...
class Client:
  ...
  async def run_attempt(
    ...
    start_data = await self._execute(_START_ATTEMPT_QUERY, {
      'input': {
        'formulationName': formulation_name,
    ...
    uuid = start_data['startAttempt']['uuid']
    while True:
      time.sleep(1)
      poll_data = await self._execute(_POLL_ATTEMPT_QUERY, {'uuid': uuid})
...
def _failed_outcome(data):
  return FailedOutcome(error_message=data['errorMessage'])

def _feasible_outcome(data):
  return FeasibleOutcome(
    is_optimal=data['isOptimal'],
    objective_value=data['objectiveValue'],
    absolute_gap=data['absoluteGap'],
    variable_results=[_result(v) for v in data['variableResults']]
  )
