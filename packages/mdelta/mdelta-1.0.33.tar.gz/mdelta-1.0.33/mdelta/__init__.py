...
class MultiTree:
    ...
    def leaves_nodeobj(self, ll=[], flag=1):  # ??ζ 0 ??????????
        if self is None:
            return None
        if flag == 1:
            if self.left is None:
    ...
    def leaves_label(self, ll=[], flag=1):  # ??ζ 0 ??????????
        if self is None:
            return None
        if flag == 1:
            if self.left is None:
...
def scoremat(TreeSeqFile: str,
    ...
    if Tqdm == 1:
        ...
        with tqdm(total=(root1.node_count()*root2.node_count()), desc='Matrix Node') as pbar:
            # ?????й??????(0,0)(0,1)(1,0)(1,1)(0,2)(2,0)(1,2)(2,1)(2,2)(0,3)(3,0)...(n,m)
    ...
    if top == 0:  # ????????
        ...
        print("root1_nodecount:", root1.node_count(),
              "root2_nodecount:", root2.node_count())
