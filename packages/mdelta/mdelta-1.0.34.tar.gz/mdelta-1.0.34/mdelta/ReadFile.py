...
def ReadTreeSeq_Name2Type(TreeSeqFilePath, Name2TypeFilePath):
    # ??????????????????
    file1 = open(TreeSeqFilePath, 'r', encoding="utf-8")
    content = file1.read()
    # print(content)
    file2 = open(Name2TypeFilePath, 'r', encoding="utf-8")
    file2.readline()  # ?????????
    while True:
...
def QuantitativeScoreFile(lllleaf, llllleaf, mav: float, miv: float, ScoreFile, matchScore=-999.):
    ...
    for row in islice(csv_reader, 1, None):  # ????????????????
        ...
        for i in itertools.product(list(typeXn_dict.keys()), list(typeXn_dict.keys())):
            ...
            score_dict[i[0] + '_' + i[1]] = reverseScore(sum((abs(float(a)**2-float(b)**2)**0.5) for a, b in zip(
                typeXn_dict[i[0]], typeXn_dict[i[1]])), math.ceil(len(row)**0.5) if matchScore == -999. else matchScore)
    return score_dict
