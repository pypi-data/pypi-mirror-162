    ...
    def __init__(self, grammar_filename):
        ...
        self._triggers = {
            trigger_type: [
                ...
                if "_".join("trigger", trigger_type) in v
            ]
    ...
    def parse(self, filehandle):
        ...
        for line in filehandle:
            ...
            matches = {
                trigger: [
                    (k, t)
                    for k, t in self._triggers[trigger]
                    if any(
                        [
                            trigger_check_methods[trigger](line, t)
                            for line in line_pp
                        ]
                    )
                ]
            ...
            section_match = {k for trigger_matches in matches for k in matches}
            ...
            if len(section_match) == 1:
                ...
                line_unmatched = self.determine_unmatched(matches, line_pp)
            ...
            if section_type == "table":
                ...
                if section_name not in output:
                    ...
                    table_header = self._parse_table(active_section, row)
                ...
                sep = active_section.get("separator", ",")
                output[section_name] = output.get(output[section_name], [])
                ...
                str_to_append = "\n" + line if section_name in output else line
                output[section_name] += str_to_append

        return output

    @staticmethod
    def determine_umatched_part(
