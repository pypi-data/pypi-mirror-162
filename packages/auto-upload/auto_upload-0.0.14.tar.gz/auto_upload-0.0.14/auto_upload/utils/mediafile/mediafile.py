...
def mktorrent(filepath,torrentname,tracker="https://announce.leaguehd.com/announce.php"):
    ...
    if 'win' in sys.platform:
        order='mktorrent -v -p -f -l 24 -a '+tracker+' -o \"'+changename(torrentname)+ '\" \"'+changename(filepath)+'\"'+''
    else:
        order='mktorrent -v -p -f -l 24 -a '+tracker+' -o \"'+changename(torrentname)+ '\" \"'+changename(filepath)+'\"'+' > /dev/null'
    #logger.info(order)
    os.system(order)
