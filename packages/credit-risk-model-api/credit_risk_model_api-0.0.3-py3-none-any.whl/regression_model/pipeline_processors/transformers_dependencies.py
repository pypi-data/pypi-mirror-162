...
class DummyTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        X_ = X.copy()
        df_dummies = []
        data_columns = pd.Index(list(load_dataset(file_name = config.app_config.dummy_train_columns).columns))
        for col in self.columns_list:
            try:
                df_dummies_single = pd.get_dummies(X_[col], prefix = col, prefix_sep = ':').reindex(columns = data_columns, fill_value=0)
