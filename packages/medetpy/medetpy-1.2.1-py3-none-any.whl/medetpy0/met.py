'''
Author: your name
Date: 2022-08-05 11:31:51
LastEditTime: 2022-08-05 18:06:21
LastEditors: your name
Description: 
FilePath: \medetpy\medetpy0\met.py
'''
'''
Need package:
    requests
...
def Online_inspection(data):
    '''进行在线验证，每次验证一个样本'''
    import requests
    res = requests.post(url='http://49.233.27.204:8879/online-inspection',data=data)
...
def dice(test=None, reference=None, confusion_matrix=None, nan_for_nonexisting=True, **kwargs):
    ...
    if test_empty and reference_empty:
        if nan_for_nonexisting:
            ...
            return 0.
    return Online_inspection(json.dumps({'type':'dice','tp':str(tp),'fp':str(fp),'tn':str(tn),'fn':str(fn),'kwargs':str(kwargs)}))
...
def jaccard(test=None, reference=None, confusion_matrix=None, nan_for_nonexisting=True, **kwargs):
    ...
    if test_empty and reference_empty:
        if nan_for_nonexisting:
            ...
            return 0.
    return Online_inspection(json.dumps({'type':'jac','tp':str(tp),'fp':str(fp),'tn':str(tn),'fn':str(fn),'kwargs':(kwargs)}))            
...
def precision(test=None, reference=None, confusion_matrix=None, nan_for_nonexisting=True, **kwargs):
    ...
    return Online_inspection(json.dumps({'type':'pr','tp':str(tp),'fp':str(fp),'tn':str(tn),'fn':str(fn),'kwargs':str(kwargs)}))
...
def sensitivity(test=None, reference=None, confusion_matrix=None, nan_for_nonexisting=True, **kwargs):
    ...
    if reference_empty:
        if nan_for_nonexisting:
            ...
            return 0.
    return Online_inspection(json.dumps({'type':'rr','tp':str(tp),'fp':str(fp),'tn':str(tn),'fn':str(fn),'kwargs':str(kwargs)}))
...
if __name__ == '__main__':
    data = json.dumps({'requirement': '{"test": "test", "reference": "reference"}'})
    res = Online_inspection(data)
    if res:
        print(res)