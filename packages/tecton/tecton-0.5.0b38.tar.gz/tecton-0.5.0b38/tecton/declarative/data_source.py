...
class SparkBatchConfig(BaseBatchConfig):
    ...
    def __init__(
        ...
        has_spark_arg = "spark" in params
        has_filter_context_arg = "filter_context" in params
        if not has_spark_arg or params[0] != "spark":
            raise AssertionError(f"spark is a required first argument of Data Source Function.")
        if supports_time_filtering and (not has_filter_context_arg or params[1] != "filter_context"):
            raise AssertionError(f"filter_context is a required second argument when supports_time_filtering is True.")
        if not supports_time_filtering and has_filter_context_arg:
            raise AssertionError(f"filter_context is not required when supports_time_filtering is False.")

        if set(params) - {"spark", "filter_context"}:
            raise AssertionError(f"spark and filter_context are the only allowed arguments of Data Source Function.")
...
class SparkStreamConfig(BaseStreamDSConfig):
    ...
    def __init__(self, data_source_function: Callable[[SparkSession], DataFrame]):
        ...
        if not has_spark_arg or params[0] != "spark":
            raise AssertionError(f"spark is a required first argument of Data Source Function.")
        if len(params) > 1:
            raise AssertionError(f"spark is the only allowed argument of Data Source Function.")
