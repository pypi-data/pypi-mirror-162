...
class SpotDeconvolution:
    ...
    def predict(self,st_data,return_prop = True, return_low_exp_celltypes = False):
        ...
        if return_low_exp_celltypes == False:
            n_col_thres = int(0.95*(predict.shape[1]))
            row_sum_less_threshold = np.sum(predict<=0.05,axis=1)
            for row in range(predict.shape[0]):
                if row_sum_less_threshold[row] < n_col_thres:
                    predict.loc[row,predict.iloc[row,:]<0.05]=0
            ...
            # predict = row_sum_less_threshold
