...
class DataStagConnection:
    ...
    def __init__(self, url: str = "", local=True, _request_client: "FlaskClient" = None):
        """
        Initializer

        :param url: The connection url of a remote server
        :param local: Defines if the local in memory database shall be used
        :param _request_client: For unit tests only. Uses Flask's internal test client to test the API.
        """
        self.local = local
        'Defines if a local storage is being used'
        self.vault: Optional[DataStagVault] = DataStagVault.local_vault
        ...
        self._cur_transaction: Optional["DataStagTransaction"] = None
        "The current transaction"
        self.lock = RLock()

    def start_transaction(self):
        """
        Starts a new transaction. All write operations will be bundled and executed in a single lock.
        ...
        with self.lock:
            ...
            if self.transaction_lock == 0:
                if self.local:
                    ...
                    ct: "DataStagTransaction" = self._cur_transaction
                    self._cur_transaction = None
                    self._execute_remote(ct.commands)  # Execute all commands in one go

    def push(self, name: str, data: Union[List[StagDataTypes], StagDataTypes], timeout_s: Optional[float] = None,
             index: int = -1) -> int:
        """
        ...
        :param index: The index at which the data shall be inserted. By default at the end.
        ...
        if self.local:
            ...
            return self._verify_result(self._execute_remote({self._COMMAND: self._COMMAND_PUSH,
                                                             self._NAME: name,
    ...
    def pop(self, name: str, default: StagDataReturnTypes = None, index: int = 0) -> StagDataReturnTypes:
        ...
        :param index: The index from which the data shall be received. By default at the front
        :return: The element which was removed
        """
        if self.local:
            return self.vault.pop(name, default=default, index=index)
        else:
            result = self._verify_ct_result(self._execute_remote({self._COMMAND: self._COMMAND_POP, self._NAME: name,
                                                                  self._INDEX: index}))
            if result is None:
                return default
            return result

    def lelements(self, name: str, start: int = 0, end: Optional[int] = None) -> List[StagDataTypes]:
        """
        ...
        if self.local:
            ...
            response: list = self._execute_remote({self._COMMAND: self._COMMAND_LELEMENTS,
                                                   self._NAME: name,
                                                   self._START: start,
                                                   self._END: end})
        ...
        if self.local:
            ...
            return self._verify_bool_result(
                self._execute_remote(
                    {self._COMMAND: self._COMMAND_SET, self._NAME: name, self._DATA: self.data_to_json(data),
                     self._TIME_OUT: timeout_s}))

    def set_ts(self, name: str, data: StagDataTypes, timeout_s: Optional[float] = None,
        ...
        :param data: The data to be added. See StagDataTypes for supported types
        :param timeout_s: The timeout for automatic deletion
        :param timestamp: A custom timestamp. If None is provided the current time will be used
        :return: True on success
        """
        with self.start_transaction():
            ...
            if timestamp is None:
                timestamp = time.time()
            return self.set(name + self._TIMESTAMP_IDENTIFIER, timestamp, timeout_s) and success
    ...
    def get_ts_modified(self, name: str, timestamp: float = 0.0) -> (float, StagDataReturnTypes):
        """
        Returns the object if it was modified since timestamp
        :param name: The element's name
        :param timestamp: The previous timestamp
        :return: The timestamp or counter, the data on success, otherwise the old timestamp, None
        """
    ...
    def get_ex(self, name: str, default: StagDataReturnTypes = None, version_counter=-1) -> (int, StagDataReturnTypes):
        ...
        if self.local:
            ...
            response: dict = self._execute_remote({self._COMMAND: self._COMMAND_GET_EX,
                                                   self._NAME: name,
                                                   self._VERSION_COUNTER: version_counter})
            result = self.json_to_data(response)
            return result[0], result[1]

    def add(self, name: str, value: [float, int] = 1, timeout_s: Optional[float] = None, default=0) -> Union[
        int, float]:
        """
        ...
        if self.local:
            ...
            return self._verify_result(
                self._execute_remote(
                    {self._COMMAND: self._COMMAND_ADD, self._NAME: name, self._VALUE: self.data_to_json(value),
                     self._TIME_OUT: timeout_s,
    ...
    def exists(self, name: str) -> bool:
        ...
        if self.local:
            ...
            response: int = self._verify_bool_result(
                self._verify_bool_result(self._execute_remote({self._COMMAND: self._COMMAND_EXISTS, self._NAME: name})))
    ...
    def delete(self, name: str) -> bool:
        ...
        if self.local:
            ...
            response: int = self._verify_bool_result(
                self._verify_bool_result(self._execute_remote({self._COMMAND: self._COMMAND_DELETE, self._NAME: name})))
            return response if response is not None else False
    ...
    def delete_multiple(self, search_masks: List[str], recursive: bool = False) -> int:
        ...
        if self.local:
            ...
            response: int = self._verify_int_result(
                self._verify_int_result(self._execute_remote({self._COMMAND: self._COMMAND_DELETE_MULTIPLE,
                                                              self._SEARCH_MASKS: search_masks,
                                                              self._RECURSIVE: recursive})))
            return response if response is not None else 0

    def find(self, mask: str, limit: int = 100, relative_names: bool = False, recursive: bool = False) -> List[
        StagDataTypes]:
        """
        ...
        """
        if self.local:
            return self.vault.find(mask=mask, limit=limit, relative_names=relative_names, recursive=recursive)
        else:
            response: list = self._execute_remote({self._COMMAND: self._COMMAND_FIND, self._MASK: mask,
    ...
    def get_values_by_name(self, mask: str, limit: int = 100, flat: bool = True):
        ...
        if self.local:
            ...
            response = self._execute_remote({self._COMMAND: self._COMMAND_GET_VALUES_BY_NAME, self._LIMIT: limit,
                                             self._MASK: mask,
                                             self._FLAT: flat})
            if response is None:
                return []
            if flat:
                ...
                return [{"name": element["name"],
                         "value": self.json_to_data(element["value"])} for element in response]
    ...
    def get_status(self, advanced: bool = False):
        ...
        if self.local:
            ...
            response = self._execute_remote({self._COMMAND: self._COMMAND_STATUS, self._ADVANCED: advanced})
    ...
    def collect_garbage(self) -> bool:
        ...
        :return: True if at least one time interval passed and something could get collected
        """
        if self.local:
            return self.vault.collect_garbage()
        else:
            return self._verify_bool_result(self._execute_remote({self._COMMAND: self._COMMAND_COLLECT_GARBAGE}))

    def _execute_remote(self, command: Union[dict, list]) -> Union[dict, list, str, float, int, bool, None]:
        """
        ...
        """
        with self.lock:
            if self._cur_transaction is not None:  # If a transaction is in progress, collect changes first
    ...
    @classmethod
    def _verify_result(cls, result, supported_types: list) -> Union[
        int, float, bool, str, dict, bytes, np.ndarray, None]:
        """
        Verifies if the result is of a given type
        :param result: The result
        :param supported_types: The list of types which are allowed for this time
    ...
    def _verify_ct_result(self, result: dict) -> int:
        ...
        """
        return self._verify_result(result, [int, float, bool, str, dict, bytes, np.ndarray])

    @classmethod
    def json_to_data(cls, data) -> StagDataReturnTypes:
        """
        Converts an element converted to JSOn back to it's original type. Special types, e.g. binary representations
        are flagged as those using a _TYPE and a _VALUE element in a dictionary.
    ...
    def data_to_json(cls, data: StagDataTypes) -> StagDataReturnTypes:
        ...
        :param data: The data
        :return: The JSON representation which can decoded on the receiver side again using json_to_data
        """
...
class DataStagTransaction:
    ...
    def __init__(self, connection: "DataStagConnection"):
        super(DataStagTransaction, self).__init__()
    ...
    def add_command(self, command: Union[dict, list]):
        """
