...
"Defines the valid parameter types to be passed into a function. Single parameters get wrapped into a dict with\
a single key named _value"
RemoteReturnTypes = Union[dict, str, float, bool, int, bytes]
"Defines the valid return types. Single values will be stored in the single key _resultValue in a dictionary"


class RemoteService:
    """
    Defines a service hosted on this machine. A service consists of one or multiple services sharing the same
    data.
    """
    ...
    def __init__(self, identifier: str, multithreading: bool = True):
        """
        Initializer
        :param identifier: The service's unique identifier in the form com.company.ai.inference.yolov2.
        A service's name is not allowed to be contained within another service's name.
        :param multithreading: Defines if this data can be accessed from multiple threads or is using thread
        local data (such as many ML libraries).
        """
    ...
    def register_function(self, function: RemoteFunction) -> bool:
        ...
        with self._lock:
            if self._started:
                raise Exception("Service already started, can not register additional functions.")
    ...
    def run_task(self, function_name: str, parameters: RemoteParameterTypes,
        ...
        :param parameters: The function's parameters. Either as base type for a single parameter or as dictionary
        :param unwrap: Defines if a single value result shall not be wrapped into a dictionary
    ...
    def unwrap(cls, result: dict) -> RemoteReturnTypes:
        ...
        :return: The single value if there is just one element, otherwise the dictionary
        """
    ...
    def initialize(self):
        """
        Overwrite this method with your initialization code. If this service is thread bound it's guaranteed to be
        executed on the later execution thread.
        """
        pass

    def deinitialize(self):
        """
        Overwrite this method with your deinitialization code. If this service is thread bound it's guaranteed to be
