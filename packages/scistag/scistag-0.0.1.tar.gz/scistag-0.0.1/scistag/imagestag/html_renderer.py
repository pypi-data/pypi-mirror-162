...
class HtmlRenderer:
    """
    The HtmlRenderer allows the conversion from HTML code to an image for example to create the thumbnail of a
    web page or to integrate HTML elements into a slide.

    Important: Needs te optional imgkit module and wkhtmltopdf (https://wkhtmltopdf.org/)
    """

    def __init__(self, framework: ImsFramework = ImsFramework.PIL):
        """
        Initializer
    ...
    def dummy_image(self) -> Image:
        ...
        return Image(result, framework=self.framework)

    def render(self, options: Optional[dict] = None) -> Image:
        """
        Renders the HTML code to an image
        :param options: One of the following optins can be provided:
        * html - The website's full HTML code (if set body & style are ignored)
        * body - Only the website's body. You can pass only the content code or the full <body> block
        * style - The site's style configuration (head.style)
        * width - The rendering width. Otherwise auto detected / clipped at 1024. None by default.
        * height - The rendering height. Otherwise auto detected / no limitation. None by default.
        * transparent - Defines if the page shall be rendered onto transparent background so an RGBA image will be
        created. False by default.
        * backgroundColor - The color as Color object. White by default.
        * trimWidth - Automatically reduce the image's width if the full extend is not used. True by default.
        * trimHeight - Defines if the height shall be trimmed. False by default
        ...
        background_color = options.get("backgroundColor", Color(1.0, 1.0, 1.0, 0.0 if transparent else 1.0))
        if not transparent and background_color.to_rgba()[3] < 1.0:
            transparent = True
        image_format = options.get("format", "bmp" if not transparent else "png")
        r, g, b, a = background_color.int_rgba()
        bg_color = f'rgba({r},{g},{b},{a / 255})'
        html_data = self._generate_html(bg_color, options)
        html_options = {"format": image_format, "quality": quality, "quiet": None}
        trim_width, trim_height = options.get("trimWidth", True), options.get("trimHeight", False)
        if "width" in options:
        ...
        if PIL_AVAILABLE and self.framework != ImsFramework.CV2:
            pil_image = PIL.Image.open(io.BytesIO(image))
            if trim_width or trim_height:
                image = self._trim(np.array(pil_image), transparent=transparent, trim_width=trim_width,
                                   trim_height=trim_height)
                return Image(image, framework=self.framework)
            return Image(pil_image, framework=self.framework)
        elif CV2_AVAILABLE:
            cv_image = cv.imdecode(np.frombuffer(image, dtype=np.uint8), cv.IMREAD_UNCHANGED)
            image = self._trim(np.array(cv_image), transparent=transparent, trim_width=trim_width,
                               trim_height=trim_height)
            return Image(cv.cvtColor(cv_image, cv.COLOR_BGRA2RGBA), framework=self.framework)
        else:
            print("Error - No framework available")
            return self.dummy_image()

    def _trim(self, image: np.ndarray, transparent: bool, trim_width=True, trim_height=False) -> np.ndarray:
        """
        Searches for the first and last non-empty rows and columns and reduces the image to the valid pixels.
        :param image: The original image
        :param transparent: Defines if the image was transparent and the alpha channel can be used
        ...
        if transparent and image.shape[2] == 4:
            ...
            alpha_channel = cv.cvtColor(image, cv.COLOR_BGR2GRAY)
        ...
        if trim_height:
            ...
            last_row = row_mismatch[0][-1] if len(row_mismatch) != 0 else image.shape[1] - 1
        else:
            first_row = 0
        ...
        if trim_width:
            ...
            last_col = col_mismatch[0][-1] if len(col_mismatch) != 0 else image.shape[0] - 1
        else:
            first_col = 0
    ...
    def _generate_html(bg_color, options):
        ...
        if html is not None:
            ...
            if body.startswith("<body>"):
                ...
                html_data += f'<body style="padding:0;margin:0;background-color:{bg_color};">' + body + \
                             '</body></html>'
        return html_data
