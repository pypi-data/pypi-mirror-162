...
class VideoSourceCamera(VideoSource):
    ...
    def __init__(self):
        ...
        self._lock = RLock()
        "Access lock"
        self.recent_image: Optional[Image] = None
    ...
    def add_redirect(self, target: Union[str, Callable[[float, Image], None]], timeout_s=5.0):
        """
        Redirects the camera's data into a local or remote DataStag storage
        :param target: If a string this is interpreted as a DataStag target identifier, otherwise a function to be
        :param timeout_s: The timeout in seconds for how long the local camera streams shall be kept in the vault
        called.
        """
        ...
        if isinstance(target, str):  # register local camera
            ...
            lc.set(target + self.LOCAL_CAMERA_STREAM_IDENTIFIER_TYPE, str(type(self)) + "/" + str(self.source))

    @classmethod
    def get_local_camera(cls, source: Union[int, str]) -> Optional[str]:
        """
    ...
    @classmethod
    def get_local_camera_name(cls, source: Union[int, str]) -> str:
        """
    ...
    @classmethod
    def get_local_camera_exists(cls, source: Union[int, str]) -> bool:
        """
        Returns if a camera with given source was registered
        :param source: The camera's source name, class specific. Should be globally unique
        :return: True if the camera was found
        """
        from ..datastag import DataStagConnection
        lc = DataStagConnection(local=True)
        local_name = cls.get_local_camera_name(source) + cls.LOCAL_CAMERA_STREAM_IDENTIFIER_TYPE
    ...
    def handle_redirections(self):
        ...
        for entry in redirections:
            ...
            if isinstance(element, str):
                import scistag.datastag
                import cv2 as cv
                _, image_data = cv.imencode(".png", recent_image.get_pixels_bgr())
                image_data: np.ndarray
                from ..datastag import DataStagConnection
                lc = DataStagConnection(local=True)
                lc.set_ts(element, image_data.tobytes(), timeout_s=timeout, timestamp=recent_timestamp)
    ...
    def _get_image_int(self, timestamp: Optional[float] = None) -> Tuple[float, Optional[Image]]:
        ...
        :param timestamp: If provided and the image was not modified no image will be returned.
        :return: The newest image if one is available and timestamp does not match the previous time stamp.
        """
        with self._lock:
    ...
    def get_camera_name(cls, index: int) -> str:
        ...
        :return: The identifier under which the camera can be stored in the local DataStag vault
        """
    ...
    def handle_initialize_camera(self):
        """
        Initializes the camera. Could be executed in another thread, lock data accordingly.
