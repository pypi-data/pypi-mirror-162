...
class CardAccount(TripleObject):
    ...
    requiredAttributes = [
        ...
        'externalID',
        'status',
        # TODO: file a ticket for fixing these
        'cardProgramID',
        'createdAt',
        'updatedAt',
    ]
    ...
    def offerActivations(self, includeExpired: bool = False, page:int = 0) -> list:
        ...
        **`coronado.exceptions`** module.
        ...
        if response.status_code == 200:
            result = [ self.__class__(offerActivation) for offerActivation in json.loads(response.content)['offer_activations'] ]
    ...
    def activateFor(self, offerIDs: list = None, offerCategory: object = None) -> object:
        ...
        **`coronado.exceptions`** module.
        ...
        if response.status_code == 200:
            result = [ TripleObject(item) for item in json.loads(response.content)['offer_activations'] ]
        elif response.status_code == 404:
            result = None
        else:
            e = errorFor(response.status_code, response.text)
            print('%s => %s' % (spec, str(e)))
            raise e
