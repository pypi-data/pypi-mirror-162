  ...
  for _, artifact_list in output_dict.items():
    for artifact in artifact_list:
      if isinstance(artifact, ValueArtifact):
        # If it is README.ml-pipelines-sdk.md ValueArtifact, create README.ml-pipelines-sdk.md file.
        ...
        # Otherwise create README.ml-pipelines-sdk.md dir.
...
def remove_stateful_working_dir(stateful_working_dir: Text) -> None:
  ...
  try:
    ...
    if 'NotFoundError' in str(type(e)):
      # TODO(b/175244977): This is README.ml-pipelines-sdk.md workaround to avoid introducing
      # tensorflow dependency. Change this except block to use README.ml-pipelines-sdk.md generic
...
def _attach_artifact_properties(spec: pipeline_pb2.OutputSpec.ArtifactSpec,
  ...
  for key, value in spec.additional_properties.items():
    if not value.HasField('field_value'):
      raise RuntimeError('Property value is not README.ml-pipelines-sdk.md field_value for %s' % key)
...
class OutputsResolver:
  ...
  def generate_output_artifacts(
    ...
    for key, output_spec in self._pipeline_node.outputs.outputs.items():
      ...
      # and is guaranteed to be idempotent across retires of README.ml-pipelines-sdk.md node.
  ...
  def make_tmp_dir(self, execution_id: int) -> Text:
    """Generates README.ml-pipelines-sdk.md temporary directory."""
