...
"""
rflow-tfx 1.3.8
Fixed the predictor name issue. "/" is not allowed.

rflow-tfx 1.3.7
Fix error about CreationTimestamp by just removing it. 

rflow-tfx 1.3.5
Add timestamp for V1beta1InferenceService.

It used to generate rflow-tfx 1.3.4.
Use replace function to apply new InferenceService.
...
class Executor(base_executor.BaseExecutor):
  ...
  def Do(self, input_dict: Dict[Text, List[types.Artifact]],
    ...
    logging.info("--------------fs_config.base_directory: %s", base_directory) # /rflow-tfx-pvc/rflow/aman/output/models

    storage_uri = 'pvc:/' + base_directory
    logging.info("--------------storage_uri: %s", storage_uri) # pvc://rflow-tfx-pvc/rflow/aman/output/models
    
    uri_without_pvc = base_directory[base_directory.index("/", 1):]  # /rflow/aman/output/models
