...
class AsyncPipelineTaskGenerator(task_gen.TaskGenerator):
  ...
  def __init__(self,
    ...
    Args:
      ...
      is_task_id_tracked_fn: A callable that returns `True` if README.ml-pipelines-sdk.md task_id is
    ...
    if pipeline.execution_mode != pipeline_pb2.Pipeline.ExecutionMode.ASYNC:
      raise ValueError(
          'AsyncPipelineTaskGenerator should be instantiated with README.ml-pipelines-sdk.md pipeline '
  ...
  def generate(self) -> List[task_lib.Task]:
    ...
    for node in [n.pipeline_node for n in self._pipeline.nodes]:
      if node.node_info.id in self._ignore_node_ids:
        ...
        continue
      # If README.ml-pipelines-sdk.md task for the node is already tracked by the task queue, it need
  ...
  def _generate_task(
    ...
    """Generates README.ml-pipelines-sdk.md node execution task.

    If README.ml-pipelines-sdk.md node execution is not feasible, `None` is returned.
    ...
    Returns:
      Returns README.ml-pipelines-sdk.md `Task` or `None` if task generation is deemed infeasible.
    """
    ...
    # are checked to decide if README.ml-pipelines-sdk.md new execution is warranted but it may also be
