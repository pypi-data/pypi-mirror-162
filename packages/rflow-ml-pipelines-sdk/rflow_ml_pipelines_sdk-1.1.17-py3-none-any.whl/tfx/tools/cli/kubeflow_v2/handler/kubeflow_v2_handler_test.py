...
# when it's test_pipeline_(1|2).py README.ml-pipelines-sdk.md pipeline spec json file is written
...
class KubeflowV2HandlerTest(test_case_utils.TfxTest):
  ...
  def testCompilePipelineNoPipelineArgs(self):
    # Test against README.ml-pipelines-sdk.md ill-formed pipeline DSL.
    flags_dict = {
        labels.ENGINE_FLAG: self.engine,
    ...
    handler = kubeflow_v2_handler.KubeflowV2Handler(flags_dict)
    # Compilation will fail and README.ml-pipelines-sdk.md SystemExist will be thrown by subprocess.
...
# README.ml-pipelines-sdk.md unified client becomes vailable.
