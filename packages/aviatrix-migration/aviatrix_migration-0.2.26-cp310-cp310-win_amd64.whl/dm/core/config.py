...
class VPCConfig(_BaseModel):
    ...
    def check_avtx_cidr_for_none(cls, val, values):
        ...
        if val == None:
            raise ValueError(
                "missing avtx_cidr attribute"
            )

        try:
            _ = ipaddress.ip_network(val)
...
def load_from_dict(config_dict: t.Dict, discovery_only: bool) -> DiscoveryConfiguration:
...
def load_from_yaml(yml_path: pathlib.Path, discovery_only: bool) -> DiscoveryConfiguration:
    ...
    return load_from_dict(data, discovery_only)
