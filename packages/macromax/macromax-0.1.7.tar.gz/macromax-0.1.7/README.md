...
When available, the [machine learning library PyTorch](https://pytorch.org/) is used instead for [(cloud/GPU) accelerated calculations](https://arxiv.org/abs/2208.01118).
...
Note that for PyTorch to work correctly, Nvidia drivers need to be up to date and match the installed CUDA version. At the time of writing, you can install PyTorch as follows: 
````sh
