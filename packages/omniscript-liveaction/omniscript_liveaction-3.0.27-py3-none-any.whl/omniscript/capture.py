...
class Capture(object):
    ...
    def get_packet_data(self, number):
        ...
        if number < self.first_packet:
            return None
        command = f'captures/{self.id.format(1)}/packets/{number}/'
    ...
    def refresh(self):
        ...
        """
        props = self._engine._issue_command(f'captures/{self.id.format(1)}/')
    ...
    def start(self):
        ...
        if self.id is None:
            ...
            return False
        command = f'running-captures/{self.id.format(1)}/'
