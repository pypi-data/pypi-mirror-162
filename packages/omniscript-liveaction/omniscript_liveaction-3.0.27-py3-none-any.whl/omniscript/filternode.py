...
from .invariant import (ID_FLAG_NO_BRACES, PATTERN_TYPE_ASCII,
    VALUE_FLAG_NETWORK_BYTE_ORDER, VALUE_OPERATOR_EQUAL, WIRELESS_BAND_ALL,
...
def store_omni_filter(element, operator, criteria):
    ...
    filter_node[_json_classid] = class_id.format(ID_FLAG_NO_BRACES)
...
class PluginNode(FilterNode):
    ...
    def __str__(self):
        return (f'{PluginNode._class_name}: '
                f'{" ".join(i.format(ID_FLAG_NO_BRACES) for i in self.ids)}')
    ...
    def _store(self, props):
        if not isinstance(props, dict):
            return
        props[PluginNode._json_ids] = ' '.join(id.format(ID_FLAG_NO_BRACES) for id in self.ids)
