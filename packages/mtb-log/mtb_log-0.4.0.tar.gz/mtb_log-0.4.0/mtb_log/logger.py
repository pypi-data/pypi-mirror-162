...
from mtb_log.list import showlist

from mtb_log.title import bigtitle, boxtitle
...
class ColoredLogger(logging.Logger):
    ...
    def _log_it(self, level: int, message, *args, **kwargs):
        ...
        if len(unsuported):
            print(f"\nmtb_log: UNSUPORTED ARG PROVIDED: {unsuported}\n")
...
class Manager(logging.Manager):
    ...
    def setLoggerClass(self, klass):
        ...
        if klass != ColoredLogger and not issubclass(klass, ColoredLogger):
            raise TypeError(
                f"logger not derived from mtb_log.ColoredLogger: {klass.__name__}"
            )
...
class _ColoredFormatter(logging.Formatter):
    ...
    def format(self, record: ColoredLogRecord):
        ...
        if hasattr(record, "simple") and record.simple:
            ...
            return ANSIColor.colored(f"{ico}{record.name}@{record.lineno} | {msg}", clr)
