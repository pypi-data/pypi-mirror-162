...
class DfManager:
    ...
    def get_setup_and_transformations_code(self):
        ...
        Currently, the full code contains setup code (e.g. from normalizating the df or from the data loader
        plugin) and transformations code.
        """
        setup_code = self._setup_code
        transformations_code = self.get_transformations_code()
        return setup_code + transformations_code
    ...
    def _update_jupyter_cell(self):
        ...
        if self.show_live_code_export:
            ...
            code += self.get_transformations_code()
            if code != "":
