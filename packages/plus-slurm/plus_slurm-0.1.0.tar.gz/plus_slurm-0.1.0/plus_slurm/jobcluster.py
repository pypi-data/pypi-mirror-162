...
class JobCluster(object):
    """
    This is the main class, the *controller* of plus_slurm. It collects all the jobs and takes care of submitting them
    ...
    def __init__(self, required_ram='2G', request_cpus=1, jobs_dir='jobs',
        ...
        self._jinja_env = jinja2.Environment(loader=jinja2.PackageLoader('plus_slurm', 'jinja2_templates'),
    ...
    def add_job(self, job, *args, **kwargs):
        ...
        **kwargs
        ...
        if not plus_slurm.job.Job in inspect.getmro(job):
    ...
    def old_run_local(self):
        ...
        for idx, cur_file in enumerate(sorted(glob.glob(queue_glob))):
            with open(os.path.join(self._output_folder, 'log', 'out.%d' % (idx,)), 'wt') as out, \
                    open(os.path.join(self._output_folder, 'log', 'err.%d' % (idx,)), 'wt') as err:
    ...
    def run_local(self):
        ...
        for idx, cur_file in enumerate(sorted(Path(self.output_folder, 'slurm').glob('*.json.gzip'))):
            with open(os.path.join(self._output_folder, 'log', 'out.%d' % (idx,)), 'wt') as out, \
                    open(os.path.join(self._output_folder, 'log', 'err.%d' % (idx,)), 'wt') as err:
                ...
                subprocess.run([self.executable, self.arguments], cwd=self.working_directory, stderr=err, stdout=out,
                    env=env)
    ...
    def submit(self, do_submit=True):
        ...
        if do_submit:
            output = subprocess.check_output([self._slurm_submit, submit_fname], stderr=subprocess.STDOUT, cwd=self.working_directory)
            print('Submit output:\n%s\n' % (output, ))
        else:
    ...
    def _prepare_jobs_directory(self):
        ...
        shutil.copytree(os.path.split(inspect.getsourcefile(inspect.getmodule(JobItem)))[0], os.path.join(json_folder, 'plus_slurm'))
