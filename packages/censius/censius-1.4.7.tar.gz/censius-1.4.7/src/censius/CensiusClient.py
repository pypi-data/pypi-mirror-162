...
import pandas as pd
from censius.schemas import get_model_health_schema,register_new_model_version_schema,register_model_schema, revise_model_schema, update_actual_schema, individual_log_schema, batch_log_schema , process_model_schema,register_dataset_schema,register_project_schema,log_explanations_schema
...
MONITORS_PROGRAMMATIC_BASE_URL="http://monitors-prod.us-east-1.elasticbeanstalk.com/v1/programmatic"
...
class CensiusClient(object):
    ...
    def get_model_health(self, *args,**kwargs):
        try:
            validate(instance=kwargs, schema=get_model_health_schema)
        ...
        headers = {
            ...
            "tenant_key":str(self.tenantKey)
        }

        response = requests.request("POST", GET_MODEL_HEALTH_URL(), headers=headers, data=payload)
