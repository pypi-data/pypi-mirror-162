...
def generate_equation(depvar, fxeff, rneff, catvars="fullrank"):
    rneff = [ '('+i+')' for i in rneff ]
    # https://bambinos.github.io/bambi/0.8.0/notebooks/getting_started.html#Coding-of-categorical-variables
    if catvars == "fullrank":
        eq = depvar + ' ~ 0 + ' + ' + '.join(fxeff+rneff)
    elif catvars == "reducedrank":
...
def estimate_lmm(mod, data=[], override=0):
    ...
    if not(os.path.exists(mod["current_sys_location"])) or (override==1):
        ...
        # because of datalad, it works better if the file that alrady exsts is explicitly deleted
        if (os.path.exists(mod["current_sys_location"])) and (override==1):
           file = pathlib.Path(mod["current_sys_location"])
           file.unlink()
