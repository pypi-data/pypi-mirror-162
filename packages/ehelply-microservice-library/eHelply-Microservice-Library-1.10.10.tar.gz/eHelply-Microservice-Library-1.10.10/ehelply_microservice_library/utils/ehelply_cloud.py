...
class CloudParticipantResponse(BaseModel):
    ...
    def without_sensitive_data(self, safe_participant: bool = False) -> 'CloudParticipantResponse':
        ...
        if response['role'] == ProjectMemberRoleEnum.key or safe_participant and len(response['active_participant']) > 8:
...
def is_allowed(
        ...
        allow_scopes: list[str] = None,
        allow_privileged_scopes: list[str] = None,
    ...
    is_allowed: bool = (project_uuid is not None and cloud_participant.role in allow_roles and cloud_participant.project_uuid == project_uuid) or (
            allow_privileged and cloud_participant.is_privileged)

    if not is_allowed and exception_if_not_allowed:
        raise HTTPException(
            status_code=400,
